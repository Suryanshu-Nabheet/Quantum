...
Quantum is organized into two developer products:
...
```bash
./scripts/setup.sh          # Install + build Agent Manager and IDE
./scripts/verify.sh         # Confirm both products are ready
```

Per-product setup scripts live in `agent/scripts/` and `ide/scripts/`. See [scripts/README.md](scripts/README.md).

## Guidelines

1. **Isolation Principle**: Keep product-specific dependencies, configs, and documentation cleanly contained inside each product directory.
