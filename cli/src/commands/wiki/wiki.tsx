...
Manage the Quantum project wiki stored in .quantum/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized Quantum wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `Quantum wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the Quantum wiki.`,
    '',
    `Title: ${result.title}`,
