...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'quantum-wiki-ingest-'))
  tempDirs.push(dir)
  return dir
}
...
test('ingestLocalWikiSource creates a source note and updates log/index', async () => {
  ...
  expect(result.sourceNote.startsWith('.quantum/wiki/sources/')).toBe(true)
  ...
  expect(index).toContain(result.sourceNote.replace('.quantum/wiki/', './'))
})
