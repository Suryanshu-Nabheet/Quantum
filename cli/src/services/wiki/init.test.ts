...
async function makeProjectDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'quantum-wiki-init-'))
  tempDirs.push(dir)
  return dir
}
...
test('initializeWiki creates the expected wiki scaffold', async () => {
  ...
  expect(result.createdFiles).toEqual([
    join('.quantum', 'wiki', 'schema.md'),
    join('.quantum', 'wiki', 'index.md'),
    join('.quantum', 'wiki', 'log.md'),
    join('.quantum', 'wiki', 'pages', 'architecture.md'),
  ])
