...
const inputSchema = lazySchema(() =>
  z.strictObject({
    ...
    durable: semanticBoolean(z.boolean().optional()).describe(
      'true = persist to .claude/scheduled_tasks.json and survive restarts. false (default) = in-memory only, dies when this Quantum session ends. Use true only when the user asks the task to survive across sessions.',
    ),
  }),
)
