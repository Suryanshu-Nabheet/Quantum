...
test('parseGitHubReleaseBody preserves snake_case identifiers', () => {
  expect(
    parseGitHubReleaseBody(
      '* add QUANTUM_DISABLE_TOOL_REMINDERS env var to suppress reminders',
    ),
  ).toEqual([
    'add QUANTUM_DISABLE_TOOL_REMINDERS env var to suppress reminders',
  ])
...
## [0.8.0](https://github.com/SuryanshuNabheet/quantum/compare/v0.7.0...v0.8.0) (2026-05-02)

### Features

* add thing

## [0.7.0](https://github.com/SuryanshuNabheet/quantum/compare/v0.6.0...v0.7.0) (2026-04-26)
...
test('getReleaseTagUrl normalizes build metadata to the public tag', () => {
  expect(getReleaseTagUrl('0.8.0+abc123')).toBe(
    'https://github.com/SuryanshuNabheet/quantum/releases/tag/v0.8.0',
  )
})
