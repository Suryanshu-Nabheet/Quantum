 ...
 * socket (e.g., "quantum-panel-a1b2c3d4") holds the shell session. Each Quantum
 * CLI instance gets its own isolated terminal panel that persists within the
 * session but is destroyed when the instance exits.
 *
 * Meta+J is bound to detach-client inside tmux, so pressing it returns to
 * Quantum CLI while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per Quantum CLI instance (based on session ID)
...
export function getTerminalPanelSocket(): string {
  ...
  return `quantum-panel-${sessionId.slice(0, 8)}`
}
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to Quantum CLI from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to Quantum ', ';',
