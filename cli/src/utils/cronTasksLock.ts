// Scheduler lease lock for .quantum/scheduled_tasks.json.
//
...
const LOCK_FILE_REL = join('.quantum', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .quantum/ doesn't exist yet — create it and retry once. In steady
