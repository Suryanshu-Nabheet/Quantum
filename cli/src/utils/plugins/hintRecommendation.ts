 ...
 *
 * State persists in GlobalConfig.quantumHints — a show-once record per
...
/**
 * Hard cap on `quantumHints.plugin[]` — bounds config growth. Each shown
...
export function maybeRecordPluginHint(hint: ClaudeCodeHint): void {
  ...
  const state = getGlobalConfig().quantumHints
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.quantumHints?.plugin ?? []
    if (existing.includes(pluginId)) return current
    return {
      ...current,
      quantumHints: {
        ...current.quantumHints,
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.quantumHints?.disabled) return current
    return {
      ...current,
      quantumHints: { ...current.quantumHints, disabled: true },
    }
  })
}
