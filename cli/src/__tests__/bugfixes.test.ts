/**
 * Tests for Bug Fixes applied to quantum.
 *
 * Covers:
...
describe('Regression checks', () => {
  test('duplicate plugin hooks are deduplicated before execution', async () => {
    ...
    try {
      ...
      const pluginMatched = matched.filter(hook => hook.pluginRoot)
      expect(pluginMatched).toHaveLength(2)
      expect(pluginMatched.map(hook => hook.pluginRoot)).toEqual([
