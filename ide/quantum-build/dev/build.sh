...
if [[ -s "${NVM_DIR}/nvm.sh" ]]; then
  ...
  . "${NVM_DIR}/nvm.sh"
  cd "${QUANTUM_ROOT}" && nvm use
fi

export APP_NAME="Quantum"
export ASSETS_REPOSITORY="Suryanshu-Nabheet/Quantum"
export BINARY_NAME="quantum"
export CI_BUILD="no"
export DEVELOPER_NAME="Suryanshu Nabheet"
export GH_REPO_PATH="Suryanshu-Nabheet/Quantum"
export ORG_NAME="Suryanshu-Nabheet"
export PRODUCT_TAGLINE="The open source AI code editor"
export QUANTUM_VERSION="0.0.1"
...
while getopts ":ilops" opt; do
  case "$opt" in
    i)
      export ASSETS_REPOSITORY="Suryanshu-Nabheet/Quantum"
      export BINARY_NAME="quantum-insiders"
...
# VS Code node-gyp requires Python 3.11; Homebrew's default python3 may be 3.14 (breaks native modules).
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  if [[ "${SKIP_SOURCE}" != "no" ]]; then
    ...
    while [[ -n "$( git log -1 | grep "QUANTUM HELPER" )" ]]; do
      git reset -q --hard HEAD~
    done
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    ...
    if [[ -f "${HOME}/.gyp/include.gypi" ]]; then
      mv ~/.gyp/include.gypi ~/.gyp/include.gypi.pre-quantum
    else
      echo "{}" > ~/.gyp/include.gypi.pre-quantum
    fi
  ...
  if [[ -f "./include_${OS_NAME}.gypi" ]]; then
    mv ~/.gyp/include.gypi.pre-quantum ~/.gyp/include.gypi
  fi
