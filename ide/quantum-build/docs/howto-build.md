...
Use the Node version from [`.nvmrc`](../.nvmrc) (`nvm use` in the repo root). `dev/build.sh` runs `nvm use` automatically when nvm is installed.
...
Here is the base script to build Quantum:
...
- Linux: https://github.com/Suryanshu-Nabheet/Quantum/blob/main/.github/workflows/stable-linux.yml
- macOS: https://github.com/Suryanshu-Nabheet/Quantum/blob/main/.github/workflows/stable-macos.yml
- Windows: https://github.com/Suryanshu-Nabheet/Quantum/blob/main/.github/workflows/stable-windows.yml
...
# verify the snap
review-tools.snap-review --allow-classic quantum*.snap
```
...
- when the script pauses at `Press any key when the conflict have been resolved...`, open `vscode` directory in **Quantum**
...
- open `vscode` directory in a new **Quantum**'s window
- fix all the `*.rej` files
- run `npm run watch`
- run `./script/code.sh` until everything is ok
- go back to the command line running `./dev/patch.sh`, press `enter` to validate the changes and it will update the patch

### <a id="icons"></a>icons/generate_quantum_assets.sh

Branding assets (app icons, favicons, installer BMPs) regenerate automatically when you run `prepare_vscode.sh` / `./build.sh` via `icons/prepare_icons.sh`.
...
Requirements: **ImageMagick 7** (`magick`); **macOS:** `iconutil` for `.icns`.

Two sources in **`assets/`** (scripts never overwrite them):

| File | Used for |
|------|-----------|
| `assets/Quantum.png` | **Light UI** — black mark (transparent). Letterpress-light, installer BMPs, docs |
| `assets/Quantum-dark.png` | **Dark UI** — white mark (transparent). Letterpress-dark, `code-icon.svg` |
| `assets/app-icon.png` | App icon — white mark on transparent square → `.ico`, `linux/code.png`, server PWA, macOS `.icns` plate |
| `assets/lockup-banner-light.png` | Horizontal icon + “Quantum” wordmark (WiX banner / docs) |

Use a **square** transparent `app-icon.png` (≥1024px ideal) and **≥1024px** marks. UI assets never upscale; app icons upscale from the master only when needed for dock sizes. macOS `.icns` output uses `icons/template_macos.png` as a rounded **black** plate with the white mark composited on top.
