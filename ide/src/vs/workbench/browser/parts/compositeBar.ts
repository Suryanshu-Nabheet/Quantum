...
import { HorizontalActivityBarToggleAction, HorizontalActivityBarToggleActionViewItem, IHorizontalActivityBarDropdownItem } from './activitybar/horizontalActivityBarActions.js';
...
import { canMoveViewContainerToLocation, isAgentView, isAuxiliaryBarReservedForAgent } from '../../contrib/agent/workbench/common/agentMode.js';
...
export class CompositeDragAndDrop implements ICompositeDragAndDrop {
	...
	private isDropBlocked(dragData: { type: 'view' | 'composite'; id: string }): boolean {
		if (!isAuxiliaryBarReservedForAgent(this.configurationService)) {
			return false;
		}
		if (dragData.type === 'view') {
			return isAgentView(dragData.id) || this.targetContainerLocation === ViewContainerLocation.AuxiliaryBar;
		}
		const container = this.viewDescriptorService.getViewContainerById(dragData.id);
		return !container || !canMoveViewContainerToLocation(container, this.targetContainerLocation, this.configurationService);
	}

	drop(data: CompositeDragAndDropData, targetCompositeId: string | undefined, originalEvent: DragEvent, before?: Before2D): void {
		const dragData = data.getData();
		if (this.isDropBlocked(dragData)) {
			return;
		}
	...
	private canDrop(data: CompositeDragAndDropData, targetCompositeId: string | undefined): boolean {
		...
		if (dragData.type === 'composite') {
			...
			return canMoveViewContainerToLocation(currentContainer, this.targetContainerLocation, this.configurationService);
		} else {
...
export interface ICompositeBarOptions {
	...
	readonly horizontalActivityBarMode?: boolean;
	readonly getHorizontalActivityBarDropdownItems?: () => IHorizontalActivityBarDropdownItem[];
	readonly getHorizontalActivityBarFooterActions?: () => IAction[];
}
...
export class CompositeBar extends Widget implements ICompositeBar {
	...
	private horizontalToggleAction = this._register(new MutableDisposable<HorizontalActivityBarToggleAction>());
	private horizontalToggleActionViewItem = this._register(new MutableDisposable<HorizontalActivityBarToggleActionViewItem>());
	...
	create(parent: HTMLElement): HTMLElement {
		...
		this.compositeSwitcherBar = this._register(new ActionBar(actionBarDiv, {
			actionViewItemProvider: (action, options) => {
				if (action instanceof HorizontalActivityBarToggleAction) {
					return this.horizontalToggleActionViewItem.value;
				}
	...
	areBadgesEnabled(compositeId: string): boolean {
		if (this.options.horizontalActivityBarMode) {
			return false;
		}
	...
	private updateCompositeSwitcher(donotTrigger?: boolean): void {
		...
		if (this.options.horizontalActivityBarMode) {
			this.updateHorizontalCompositeSwitcher(donotTrigger);
			return;
		}
	...
	private updateHorizontalCompositeSwitcher(donotTrigger?: boolean): void {
		const compositeSwitcherBar = this.compositeSwitcherBar!;
		const headerContainer = compositeSwitcherBar.getContainer().closest('.header-or-footer') as HTMLElement | undefined;
		const paneContainer = headerContainer?.closest('.pane-composite-part') as HTMLElement | undefined;

		const compositesToShow = this.model.visibleItems
		...
		if (!this.horizontalToggleAction.value) {
			this.horizontalToggleAction.value = this.instantiationService.createInstance(HorizontalActivityBarToggleAction, () => {
				this.horizontalToggleActionViewItem.value?.toggleMenu();
			});
			this.horizontalToggleActionViewItem.value = this.instantiationService.createInstance(
				HorizontalActivityBarToggleActionViewItem,
				{
				...
				this.horizontalToggleAction.value,
				() => paneContainer,
				() => headerContainer,
				() => this.options.getHorizontalActivityBarDropdownItems?.() ?? this.getDefaultHorizontalDropdownItems(),
				() => this.model.activeItem?.id,
				compositeId => this.options.getOnCompositeClickAction(compositeId),
				compositeId => this.options.getCompositePinnedAction(compositeId),
			);
		}

		const toggleIndex = compositeSwitcherBar.viewItems.findIndex(item => item.action === this.horizontalToggleAction.value);
		if (toggleIndex === -1) {
			compositeSwitcherBar.push(this.horizontalToggleAction.value!, { label: false, icon: true });
		} else if (toggleIndex !== compositesToShow.length) {
			compositeSwitcherBar.pull(toggleIndex);
			compositeSwitcherBar.push(this.horizontalToggleAction.value!, { label: false, icon: true, index: compositesToShow.length });
		}
	...
	private getDefaultHorizontalDropdownItems(): IHorizontalActivityBarDropdownItem[] {
		return this.model.visibleItems.map(item => ({
			id: item.id,
			name: item.name ?? this.getAction(item.id)?.label ?? item.id,
			keybindingId: item.activityAction.compositeBarActionItem.keybindingId,
			iconClassNames: item.activityAction.compositeBarActionItem.classNames,
			pinned: item.pinned,
		}));
	}
	...
	getContextMenuActions(e?: MouseEvent | GestureEvent): IAction[] {
		...
		if (!this.options.horizontalActivityBarMode) {
			actions.push(...this.model.visibleItems
	...
	isHorizontalActivityBarMode(): boolean {
		return !!this.options.horizontalActivityBarMode;
	}
}
