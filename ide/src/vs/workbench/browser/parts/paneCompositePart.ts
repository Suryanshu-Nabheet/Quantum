...
export enum CompositeBarPosition {
	...
	TITLE,
	BOTTOM,
	HORIZONTAL_ACTIVITY_BAR
}

export const HORIZONTAL_ACTIVITY_BAR_HEADER_CLASS = 'horizontal-activitybar-header';
export const HORIZONTAL_ACTIVITY_BAR_MODE_CLASS = 'horizontal-activitybar-mode';
...
export abstract class AbstractPaneCompositePart extends CompositePart<PaneComposite> implements IPaneCompositePart {
	...
	protected updateCompositeBar(updateCompositeBarOption: boolean = false): void {
		...
		if (wasCompositeBarVisible) {
			...
			if (previousPosition === CompositeBarPosition.TOP || previousPosition === CompositeBarPosition.HORIZONTAL_ACTIVITY_BAR) {
		...
		switch (newPosition) {
			case CompositeBarPosition.TOP:
				...
				newCompositeBarContainer.classList.remove(HORIZONTAL_ACTIVITY_BAR_HEADER_CLASS);
				break;
			case CompositeBarPosition.HORIZONTAL_ACTIVITY_BAR:
				newCompositeBarContainer = this.createHeaderArea();
				newCompositeBarContainer.classList.add(HORIZONTAL_ACTIVITY_BAR_HEADER_CLASS);
				break;
		...
		if (isCompositeBarVisible) {
			...
			if (newPosition === CompositeBarPosition.TOP || newPosition === CompositeBarPosition.HORIZONTAL_ACTIVITY_BAR) {
		...
		this.updateHorizontalActivityBarTitleVisibility(newPosition);
	...
	private updateHorizontalActivityBarTitleVisibility(position: CompositeBarPosition | undefined): void {
		const hideCompositeTitle = position === CompositeBarPosition.HORIZONTAL_ACTIVITY_BAR;
		this.element.classList.toggle(HORIZONTAL_ACTIVITY_BAR_MODE_CLASS, hideCompositeTitle);
		this.setTitleVisibility(!hideCompositeTitle);
	}
