...
MenuRegistry.appendMenuItem(MenuId.AuxiliaryBarTitle, {
	...
	group: 'navigation',
	order: 2,
	when: ContextKeyExpr.equals(`config.${LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION}`, ActivityBarPosition.DEFAULT)
});
...
const agentNativeModeNegated = ContextKeyExpr.notEquals(`config.${ChatConfiguration.AIDisabled}`, true);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			precondition: agentNativeModeNegated,
			menu: [{ id: MenuId.AuxiliaryActivityBarPositionMenu, order: 1, when: agentNativeModeNegated }]
		});
	}
	run(accessor: ServicesAccessor): void {
		accessor.get(IConfigurationService).updateValue(LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION, ActivityBarPosition.DEFAULT);
	}
});
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			precondition: agentNativeModeNegated,
			menu: [{ id: MenuId.AuxiliaryActivityBarPositionMenu, order: 2, when: agentNativeModeNegated }]
		});
	}
	run(accessor: ServicesAccessor): void {
		accessor.get(IConfigurationService).updateValue(LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION, ActivityBarPosition.TOP);
	}
});
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			precondition: agentNativeModeNegated,
			menu: [{ id: MenuId.AuxiliaryActivityBarPositionMenu, order: 3, when: agentNativeModeNegated }]
		});
	}
	run(accessor: ServicesAccessor): void {
		accessor.get(IConfigurationService).updateValue(LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION, ActivityBarPosition.BOTTOM);
	}
});
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			precondition: agentNativeModeNegated,
			menu: [{ id: MenuId.AuxiliaryActivityBarPositionMenu, order: 4, when: agentNativeModeNegated }]
		});
	}
	run(accessor: ServicesAccessor): void {
		accessor.get(IConfigurationService).updateValue(LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION, ActivityBarPosition.HIDDEN);
	}
});
