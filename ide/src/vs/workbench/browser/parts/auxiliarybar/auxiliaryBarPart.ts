...
import { ActivityBarPosition, getAuxiliaryActivityBarPosition, IWorkbenchLayoutService, LayoutSettings, Parts, Position } from '../../../services/layout/browser/layoutService.js';
...
import { isAgentNativeMode } from '../../../contrib/agent/workbench/common/agentMode.js';
...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	constructor(
		...
		this._register(configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION)) {
				this.configuration = this.resolveConfiguration();
				this.onDidChangeActivityBarLocation();
			} else if (e.affectsConfiguration('workbench.secondarySideBar.showLabels')) {
				this.configuration = this.resolveConfiguration();
				this.updateCompositeBar(true);
			} else if (e.affectsConfiguration(LayoutSettings.AUXILIARY_ACTIVITY_BAR_AUTO_HIDE)) {
				this.onDidChangeActivityBarLocation();
			}
	...
	private onDidChangeAutoHideViewContainers(e: { before: number; after: number }): void {
		...
		const autoHide = this.configurationService.getValue<boolean>(LayoutSettings.AUXILIARY_ACTIVITY_BAR_AUTO_HIDE);
	...
	private resolveConfiguration(): IAuxiliaryBarPartConfiguration {
		const position = getAuxiliaryActivityBarPosition(this.configurationService);
	...
	private fillExtraContextMenuActions(actions: IAction[]): void {
		...
		const agentNativeMode = isAgentNativeMode(this.configurationService);
		...
		const activityBarPositionMenu = this.menuService.getMenuActions(MenuId.AuxiliaryActivityBarPositionMenu, this.contextKeyService, { shouldForwardArgs: true, renderShortTitle: true });
