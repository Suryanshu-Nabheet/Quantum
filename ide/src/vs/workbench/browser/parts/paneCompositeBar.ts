...
import { CompositeBar, ICompositeBarItem, ICompositeBarOptions, CompositeDragAndDrop } from './compositeBar.js';
...
import { IHorizontalActivityBarDropdownItem } from './activitybar/horizontalActivityBarActions.js';
...
export class PaneCompositeBar extends Disposable {
	...
	constructor(
		...
		this.dndHandler = new CompositeDragAndDrop(this.viewDescriptorService, this.configurationService, this.location, this.options.orientation,
	...
	private createCompositeBar(cachedItems: ICompositeBarItem[]) {
		const compositeBarOptions: ICompositeBarOptions = {
			...
			...(this.options.horizontalActivityBarMode ? {
				horizontalActivityBarMode: true,
				getHorizontalActivityBarDropdownItems: () => this.getHorizontalActivityBarDropdownItems(),
				getHorizontalActivityBarFooterActions: () => this.getHorizontalActivityBarFooterActions(),
			} : {}),
		};

		return this._register(this.instantiationService.createInstance(CompositeBar, cachedItems, compositeBarOptions));
	}
	...
	private createMoveAction(viewContainer: ViewContainer, newLocation: ViewContainerLocation, defaultLocation: ViewContainerLocation): IAction {
		return toAction({
			...
			label: newLocation === ViewContainerLocation.Panel ? localize('panel', "Panel") : newLocation === ViewContainerLocation.Sidebar ? localize('sidebar', "Primary Side Bar") : localize('auxiliarybar', "Agent"),
			run: () => {
	...
	private getHorizontalActivityBarDropdownItems(): IHorizontalActivityBarDropdownItem[] {
		return this.compositeBar.getVisibleComposites().map(item => {
			const compositeBarActionItem = this.getCompositeActions(item.id).activityAction.compositeBarActionItem;
			const viewContainer = this.viewDescriptorService.getViewContainerById(item.id);
			const icon = viewContainer?.icon && ThemeIcon.isThemeIcon(viewContainer.icon) ? viewContainer.icon : undefined;
			return {
				id: item.id,
				name: item.name ?? compositeBarActionItem.name,
				keybindingId: compositeBarActionItem.keybindingId,
				icon,
				iconClassNames: compositeBarActionItem.classNames,
				pinned: item.pinned,
			};
		});
	}
	...
	private toCompositeBarActionItem(id: string, name: string, icon: URI | ThemeIcon | undefined, keybindingId: string | undefined): ICompositeBarActionItem {
		...
		if (this.options.icon) {
			if (URI.isUri(icon)) {
				...
				const iconClass = this.options.horizontalActivityBarMode
					? `.monaco-workbench .${this.options.partContainerClass} .monaco-action-bar .action-label.${iconId}, .monaco-workbench .horizontal-activitybar-views-panel .horizontal-activitybar-views-item-icon.${iconId}`
					: `.monaco-workbench .${this.options.partContainerClass} .monaco-action-bar .action-label.${iconId}`;
	...
	protected getHorizontalActivityBarFooterActions(): IAction[] {
		return [];
	}
