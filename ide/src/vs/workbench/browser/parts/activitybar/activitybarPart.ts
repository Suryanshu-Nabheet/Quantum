...
import { ActivityBarOrientation, ActivityBarPosition, IWorkbenchLayoutService, LayoutSettings, Parts, Position } from '../../../services/layout/browser/layoutService.js';
...
const activityBarLocationEnabled = ContextKeyExpr.notEquals(`config.${LayoutSettings.ACTIVITY_BAR_ORIENTATION}`, ActivityBarOrientation.HORIZONTAL);
...
export class ActivitybarPart extends Part {
	...
	static readonly horizontalPinnedViewContainersKey = 'workbench.activity.horizontalPinnedViewlets';
	static readonly horizontalPlaceholderViewContainersKey = 'workbench.activity.horizontalPlaceholderViewlets';
	static readonly horizontalViewContainersWorkspaceStateKey = 'workbench.activity.horizontalViewletsWorkspaceState';
...
export class ActivityBarCompositeBar extends PaneCompositeBar {
	...
	private fillContextMenuActions(actions: IAction[], e?: MouseEvent | GestureEvent) {
		// In horizontal activity bar mode, only expose primary sidebar layout actions.
		if (this.configurationService.getValue<string>(LayoutSettings.ACTIVITY_BAR_ORIENTATION) === ActivityBarOrientation.HORIZONTAL) {
			actions.push(...this.getSidebarLayoutMenuActions());
			return;
		}
	...
	getActivityBarContextMenuActions(): IAction[] {
		...
		if (this.configurationService.getValue<string>(LayoutSettings.ACTIVITY_BAR_ORIENTATION) !== ActivityBarOrientation.HORIZONTAL) {
			...
			actions.push(new SubmenuAction('workbench.action.activityBar.position', localize('activity bar position', "Activity Bar Position"), positionActions));
		}
		...
		if (activityBarPosition === ActivityBarPosition.DEFAULT && this.configurationService.getValue<string>(LayoutSettings.ACTIVITY_BAR_ORIENTATION) !== ActivityBarOrientation.HORIZONTAL) {
	...
	protected override getHorizontalActivityBarFooterActions(): IAction[] {
		return [];
	}

	private getSidebarLayoutMenuActions(): IAction[] {
		const actions: IAction[] = [];
	...
	constructor() {
		super({
			...
			menu: [{
				...
				order: 1,
				when: activityBarLocationEnabled
			}, {
				id: MenuId.CommandPalette,
				when: ContextKeyExpr.and(activityBarLocationEnabled, ContextKeyExpr.notEquals(`config.${LayoutSettings.ACTIVITY_BAR_LOCATION}`, ActivityBarPosition.DEFAULT), IsSessionsWindowContext.negate()),
			}]
		});
	}
	...
	constructor() {
		super({
			...
			menu: [{
				...
				id: MenuId.CommandPalette,
				when: ContextKeyExpr.and(activityBarLocationEnabled, ContextKeyExpr.notEquals(`config.${LayoutSettings.ACTIVITY_BAR_LOCATION}`, ActivityBarPosition.TOP), IsSessionsWindowContext.negate()),
			}]
		});
	}
	run(accessor: ServicesAccessor): void {
	...
	constructor() {
		super({
			...
			menu: [{
				...
				id: MenuId.CommandPalette,
				when: ContextKeyExpr.and(activityBarLocationEnabled, ContextKeyExpr.notEquals(`config.${LayoutSettings.ACTIVITY_BAR_LOCATION}`, ActivityBarPosition.BOTTOM), IsSessionsWindowContext.negate()),
			}]
		});
	}
	run(accessor: ServicesAccessor): void {
	...
	constructor() {
		super({
			...
			menu: [{
				...
				id: MenuId.CommandPalette,
				when: ContextKeyExpr.and(activityBarLocationEnabled, ContextKeyExpr.notEquals(`config.${LayoutSettings.ACTIVITY_BAR_LOCATION}`, ActivityBarPosition.HIDDEN), IsSessionsWindowContext.negate()),
			}]
		});
	}
	run(accessor: ServicesAccessor): void {
...
MenuRegistry.appendMenuItem(MenuId.MenubarAppearanceMenu, {
	...
	title: localize('positionPrimaryActivityBar', "Primary Side Bar Activity Bar Position"),
	group: '3_workbench_layout_move',
	order: 2,
	when: ContextKeyExpr.and(IsSessionsWindowContext.negate(), activityBarLocationEnabled)
});

MenuRegistry.appendMenuItem(MenuId.ViewContainerTitleContext, {
	submenu: MenuId.ActivityBarPositionMenu,
	title: localize('positionPrimaryActivityBar', "Primary Side Bar Activity Bar Position"),
	when: ContextKeyExpr.and(ContextKeyExpr.equals('viewContainerLocation', ViewContainerLocationToString(ViewContainerLocation.Sidebar)), activityBarLocationEnabled),
	group: '3_workbench_layout_move',
	order: 1
});
