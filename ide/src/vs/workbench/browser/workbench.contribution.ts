...
import { ActivityBarOrientation, ActivityBarPosition, EditorActionsLocation, EditorTabsMode, LayoutSettings } from '../services/layout/browser/layoutService.js';
...
(function registerConfiguration(): void {
	...
	registry.registerConfiguration({
		...
		'properties': {
			...
			[LayoutSettings.ACTIVITY_BAR_LOCATION]: {
				...
				'default': 'default',
				'markdownDescription': localize({ comment: ['This is the description for a setting'], key: 'activityBarLocation' }, "Controls the location of the Activity Bar relative to the Primary Side Bar. This value is ignored when {0} is set to {1}.", '`#workbench.activityBar.orientation#`', '`horizontal`'),
				'enumDescriptions': [
			...
			[LayoutSettings.ACTIVITY_BAR_ORIENTATION]: {
				'type': 'string',
				'enum': [ActivityBarOrientation.HORIZONTAL, ActivityBarOrientation.VERTICAL],
				'default': ActivityBarOrientation.HORIZONTAL,
				'markdownDescription': localize({ comment: ['This is the description for a setting'], key: 'activityBarOrientation' }, "Controls the layout orientation of the Activity Bar."),
				'enumDescriptions': [
					localize('workbench.activityBar.orientation.horizontal', "Show the Activity Bar as a horizontal row at the top of the Primary Side Bar with a toggle to pin or unpin views."),
					localize('workbench.activityBar.orientation.vertical', "Show the Activity Bar in the classic vertical layout.")
				],
				agentsWindow: { default: ActivityBarOrientation.VERTICAL, readOnly: true },
			},
			[LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION]: {
				'type': 'string',
				'enum': ['default', 'top', 'bottom', 'hidden'],
				'default': 'default',
			...
			[LayoutSettings.ACTIVITY_BAR_AUTO_HIDE]: {
				'type': 'boolean',
				'default': false,
				'markdownDescription': localize({ comment: ['This is the description for a setting'], key: 'activityBarAutoHide' }, "Controls whether the Activity Bar is automatically hidden when there is only one view container to show in the Primary Side Bar when {0} is set to {1} or {2}.", '`#workbench.activityBar.location#`', '`top`', '`bottom`'),
				agentsWindow: { default: false, readOnly: true },
			},
	...
	windowTitleDescription += '\n- ' + [
		...
		localize('appName', "`${appName}`: e.g. Quantum."),
...
Registry.as<IConfigurationMigrationRegistry>(Extensions.ConfigurationMigration)
	.registerConfigurationMigrations([{
		key: LayoutSettings.ACTIVITY_BAR_LOCATION, migrateFn: (value: unknown, valueAccessor) => {
			const results: ConfigurationKeyValuePairs = [];
			let primaryValue = value;
			if (value === 'side') {
				primaryValue = ActivityBarPosition.DEFAULT;
				results.push([LayoutSettings.ACTIVITY_BAR_LOCATION, { value: ActivityBarPosition.DEFAULT }]);
			}
			// Preserve pre-split behavior: mirror primary position to secondary when secondary was never configured.
			if (valueAccessor(LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION) === undefined) {
				results.push([LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION, { value: primaryValue }]);
			}
			return results;
		}
	}]);
