...
export class WalkthroughsService extends Disposable implements IWalkthroughsService {
	...
	private async registerExtensionWalkthroughContributions(extension: IExtensionDescription) {
		...
		if (hadLastFoucs && sectionToOpen && !isQuantumOnboardingEnabled(this.configurationService) && this.configurationService.getValue<string>('workbench.welcomePage.walkthroughs.openOnInstall') && startupEditor !== 'agentSessionsWelcomePage') {
