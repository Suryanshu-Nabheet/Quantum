...
export class GettingStartedPage extends EditorPane {
	...
	private async buildCategoriesSlide(preserveFocus?: boolean) {
		...
		} else if (!this.productService.openToWelcomeMainPage && this.showFeaturedWalkthrough && this.storageService.isNew(StorageScope.APPLICATION) && !isQuantumOnboardingEnabled(this.configurationService)) {
	...
	private async buildAnnouncementList(): Promise<GettingStartedIndexList<AnnouncementEntry>> {
		...
		const announcementList = this.announcementList = new GettingStartedIndexList({
			title: localize('announcements', "Quantum Announcements"),
			klass: 'announcements',
			limit: 5,
