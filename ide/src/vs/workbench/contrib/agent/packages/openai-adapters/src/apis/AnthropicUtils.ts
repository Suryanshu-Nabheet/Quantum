 ...
 * Azure AI Foundry hosts Anthropic models but requires a different credential header.
 *
...
export function getAnthropicHeaders(
  ...
  if (isAzure && apiKey.startsWith("sk-ant-")) {
    console.warn(
      "[Agent] Azure endpoint detected but API key appears to be a standard Anthropic key (sk-ant-*). " +
...
export function openaiToolToAnthropicTool(tool: ChatCompletionTool): Tool {
  if (tool.type === "function" && "function" in tool) {
    return {
      ...
      input_schema: tool.function.parameters as Tool.InputSchema, // unsafe cast, may be differences between openai tool schema and anthropic tool schema,
    };
  } else {
