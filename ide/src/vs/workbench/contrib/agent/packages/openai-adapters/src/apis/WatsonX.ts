...
import { chatCompletion, customFetch, embedding, rerank } from "../util.js";
import {
  BaseLlmApi,
...
export class WatsonXApi implements BaseLlmApi {
  ...
  async completionNonStream(
    ...
    throw new Error("WatsonX does not support non-streaming text completions.");
  }
  ...
  async *fimStream(
    ...
    throw new Error("WatsonX does not support fill-in-the-middle completions.");
  }
  ...
  async embed(
    ...
    const resp = await customFetch(this.config.requestOptions)(
      ...
      ),
      {
        ...
        headers: {
          "Content-Type": "application/json",
          Authorization: `${
            token.expiration === -1 ? "ZenApiKey" : "Bearer"
          } ${token.token}`,
        },
        body: JSON.stringify({
          inputs,
    ...
    if (!resp.ok) {
      throw new Error(`WatsonX embed failed: ${await resp.text()}`);
    }

    const data = (await resp.json()) as { results?: { embedding: number[] }[] };
    const vectors = data.results?.map((result) => result.embedding) ?? [];
    if (!vectors.length) {
      throw new Error("WatsonX generated empty embedding");
    }

    return embedding({ data: vectors, model: body.model });
  }
  ...
  async rerank(body: RerankCreateParams): Promise<CreateRerankResponse> {
    ...
    const resp = await customFetch(this.config.requestOptions)(
      new URL(`${this.apiBase}ml/v1/text/rerank?version=${this.apiVersion}`),
      {
        ...
        body: JSON.stringify({
          inputs: body.documents.map((doc) => ({ text: doc })),
          query: body.query,
          parameters: {
            truncate_input_tokens: 500,
            return_options: {
    ...
    if (!resp.ok) {
      throw new Error(`WatsonX rerank failed: ${await resp.text()}`);
    }
    ...
    return rerank({ data: scores, model: body.model });
  }

  async list(): Promise<OpenAI.Models.Model[]> {
    throw new Error("WatsonX does not support listing models via this adapter.");
  }
}
