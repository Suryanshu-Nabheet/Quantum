...
export class BedrockApi implements BaseLlmApi {
    ...
    for (const toolCallDelta of deltaToolCalls) {
      ...
      if (toolCallDelta.function?.arguments !== undefined) {
        ...
        if (typeof args === "string") {
          ...
          existing.function.arguments = JSON.stringify(args);
        }
      }
    }
  }
  ...
  async chatCompletionNonStream(
    ...
    for await (const chunk of this.chatCompletionStream(
      ...
      if (delta.tool_calls) {
        this.accumulateToolCallDelta(toolCalls, delta.tool_calls);
      }
      if (chunk.usage) {
        usage = chunk.usage;
      }
      ...
      choices: [
        {
          ...
          logprobs: null,
          finish_reason:
            resolvedToolCalls.length > 0 ? "tool_calls" : "stop",
          message: {
            role: "assistant",
  ...
  list(): Promise<Model[]> {
    throw new Error("Bedrock does not support listing models via this adapter.");
  }
}
