...
/**
 * Strips directory prefixes so `/bin/ls` and `ls` are treated the same.
 */
function normalizeBaseCommand(commandToken: string): string {
  const basename = commandToken.split(/[/\\]/).pop() ?? commandToken;
...
function evaluateSingleCommand(
  ...
  const baseCommand = normalizeBaseCommand(commandTokens[0]);
...
function isSafeCommand(baseCommand: string, args: string[]): boolean {
  // Directory navigation (no command substitution in args)
  if (
    (baseCommand === "cd" ||
      baseCommand === "pushd" ||
      baseCommand === "popd" ||
  ...
  // Version / metadata checks for runtimes
  if (
    (baseCommand === "python" ||
      baseCommand === "python3" ||
      baseCommand === "node" ||
  ...
  // Safe git operations (read-only / introspection)
  if (baseCommand === "git") {
    const safeGitOps = [
      "status",
      "log",
    ...
    if (args.length > 0 && safeGitOps.includes(args[0])) {
      ...
      if (
        sub === "config" &&
        args.some((arg) => arg === "--global" || arg === "--system")
      ) {
        return false;
      }
      if (sub === "stash" && args[1] && !["list", "show"].includes(args[1])) {
        return false;
      }
      if (sub === "tag" && args.some((arg) => arg === "-d" || arg === "--delete")) {
        return false;
      }
      return true;
    }
