...
export type QuickEditShowParams = {
  ...
  /**
   * Used when edit is opened for a supplied range instead of the active selection.
   */
  range?: vscode.Range;
};
...
export class QuickEdit {
    ...
    quickPick.onDidAccept(async () => {
      ...
      switch (label) {
        case UserPromptLabels.AcceptAll:
          vscode.commands.executeCommand("agent.acceptDiff", path);
          break;
        case UserPromptLabels.RejectAll:
          vscode.commands.executeCommand("agent.rejectDiff", path);
          break;
        case QuickEditInitialItemLabels.Submit:
          if (quickPick.value) {
            await vscode.commands.executeCommand("agent.rejectDiff", path);
  ...
  private handleQuickPickAccept = ({
    ...
    // If not an initial item, it's a file selection. Allow further prompt editing.
