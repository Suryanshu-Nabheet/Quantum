...
class InlineTipManager {
  ...
  private debouncedSelectionChange(
    ...
    this.debounceTimer = setTimeout(() => {
      ...
      // Use the editor's current selection — the debounced `selection` can be stale
      // if the document changed while waiting.
      this.updateTooltipPosition(editor, editor.selection);
  ...
  private calculateTooltipPosition(
    ...
    const clampLine = (line: number) =>
      Math.max(0, Math.min(line, lineCount - 1));

    const isValidLine = (lineNumber: number) =>
      lineNumber >= 0 && lineNumber < lineCount;

    // Get selection info (clamp — selection can reference deleted lines)
    const startLine = clampLine(selection.start.line);
    const endLine = clampLine(selection.end.line);
    const endLineText = document.lineAt(endLine).text;
    const isFullLineSelection =
      selection.start.character === 0 &&
      (selection.end.line > selection.start.line
        ? selection.end.character === 0
        : selection.end.character === endLineText.length);
