...
export class VsCodeIdeUtils {
  ...
  private async _getThreads(session: vscode.DebugSession) {
    ...
    const allThreads: any[] = threadsResponse.threads ?? [];

    let threads = allThreads.filter((thread: any) =>
      isThreadStopped(thread.id),
    );

    // Some adapters report stopped state before thread ids align; if we know the
    // session is paused, show all threads so @debugger remains usable.
    if (
      threads.length === 0 &&
      allThreads.length > 0 &&
      hasAnyStoppedThreads()
    ...
    threads.sort(
      (a: any, b: any) =>
        normalizeThreadId(a.id) - normalizeThreadId(b.id),
    );
  ...
  async getDebugLocals(threadIndex = 0): Promise<string> {
    ...
    if (!session) {
      return "No active debug session. Start debugging and pause on a breakpoint to use @debugger.";
    }
    ...
    try {
      ...
      const topFrame = traceResponse.stackFrames?.[0];
      if (!topFrame) {
        return "No stack frames available for the selected debug thread.";
      }
      ...
      if (!localsScope) {
        return "No variable scopes available for the selected debug thread.";
      }
      ...
      const variableContext = (variablesResponse.variables ?? [])
        ...
        .reduce(
          (acc: string, variable: any) =>
            `${acc}\nname: ${variable.name}, type: ${variable.type}, value: ${variable.value}`,
          "",
        );

      return variableContext.trim() || "No local variables in the current frame.";
    } catch (error) {
      console.warn("Failed to read debug locals:", error);
      return "Failed to read debug locals. Ensure execution is paused on a breakpoint.";
    }
  }
        ...
        traceResponse.stackFrames
          ...
          .map(async (stackFrame: any) => {
            ...
            return await this.retrieveSource(
              scope.source &&
              Object.keys(scope.source).length > 0
                ? scope
                : stackFrame,
            );
          }),
