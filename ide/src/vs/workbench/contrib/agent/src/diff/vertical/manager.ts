...
export class VerticalDiffManager {
  ...
  // Listener for user doc changes is disabled during Agent edits to the text document
  ...
  async acceptRejectVerticalDiffBlock(
    ...
    // Disable listening to file changes while Agent makes changes
    this.disableDocumentChangeListener();
  ...
  async streamDiffLines(
    ...
    vscode.commands.executeCommand("setContext", "agent.diffVisible", true);
  ...
  async streamEdit({
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by Agent
      // Before adding this back, we need to distinguish between human and Agent
    ...
    try {
      ...
      if (abortController.signal.aborted) {
        void vscode.commands.executeCommand("agent.rejectDiff");
      }
