...
describe("isSecurityConcern", () => {
  ...
  describe("Security directories", () => {
    ...
    it("should detect agent directory as security concern", () => {
      expect(isSecurityConcern(".agent/config.json")).toBe(true);
    });
...
describe("file attach ignores", () => {
  it("keeps media out of the code index ignore list only via MEDIA_ATTACHABLE", () => {
    for (const pattern of MEDIA_ATTACHABLE_FILETYPES) {
      expect(DEFAULT_IGNORES).toContain(pattern);
      expect(DEFAULT_FILE_ATTACH_IGNORES).not.toContain(pattern);
    }
  });

  it("does not blanket-exclude common non-code types from @ Files attach", () => {
    for (const pattern of ["*.csv", "*.pdf", "*.zip", "*.ttf", "*.jsonl"]) {
      expect(DEFAULT_FILE_ATTACH_IGNORES).not.toContain(pattern);
    }
  });
});
