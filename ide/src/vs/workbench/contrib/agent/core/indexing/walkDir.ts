...
import { getGlobalAgentIgArray } from "./agentignore";
...
class DFSWalker {
  ...
  public async *walk(): AsyncGenerator<string> {
    ...
    const defaultAndGlobalIgnores = ignore()
      ...
      .add(getGlobalAgentIgArray());
    ignoreFileTime += Date.now() - section;

    const stack: { walkableEntry: WalkableEntry; ignoreContexts: IgnoreContext[] }[] =
      [
        {
          walkableEntry: {
            name: this.uri,
    ...
    const shouldStop = () =>
      ...
      (this.options.deadlineMs !== undefined &&
        Date.now() - start >= this.options.deadlineMs) ||
      (this.options.maxEntries !== undefined &&
        yieldedEntries >= this.options.maxEntries);
    ...
    while (stack.length > 0) {
      ...
      const cur = stack.pop()!;
      if (visited.has(cur.walkableEntry.uri)) {
        continue;
      }
      visited.add(cur.walkableEntry.uri);

      // Only directories will be added to the stack
      dirs++;
      if (
        this.options.maxDirs !== undefined &&
        dirs > this.options.maxDirs
      ) {
        break;
      }
      ...
      for (const entry of entries) {
        ...
        if (this.entryIsDirectory(entry)) {
          ...
          if (this.options.include !== "files") {
            if (
              this.options.maxDirs !== undefined &&
              yieldedDirs >= this.options.maxDirs
            ) {
              continue;
            }
          ...
          if (
            ...
            yieldedFiles >= this.options.maxFiles
          ) {
            continue;
          }
          yieldedFiles++;
  ...
  private entryIsDirectory(entry: Entry) {
    return (entry[1] & (2 as FileType.Directory)) !== 0;
  }

  private entryIsSymlink(entry: Entry) {
    return (entry[1] & (64 as FileType.SymbolicLink)) !== 0;
  }

  private entryIsFile(entry: Entry) {
    return (entry[1] & (1 as FileType.File)) !== 0;
  }
}
...
export async function walkDirs(
  ...
  for (const dir of workspaceDirs) {
    if (_optionOverrides?.abortSignal?.aborted) {
      break;
    }
    const maxEntries = _optionOverrides?.maxEntries;
    const remaining =
      maxEntries === undefined ? undefined : maxEntries - results.length;
    if (remaining !== undefined && remaining <= 0) {
      break;
    }
    results.push(
      ...(await walkDir(dir, ide, {
        ..._optionOverrides,
...
export async function getIgnoreContext(
  ...
  const dirFiles = currentDirEntries
    .filter(([_, entryType]) => (entryType & (1 as FileType.File)) !== 0)
    .map(([name, _]) => name);

  // Find ignore files and get ignore arrays from their contexts
  // These are done separately so that .agentignore can override .gitignore
  const gitIgnoreFile = dirFiles.find((name) => name === ".gitignore");
  const agentIgnoreFile = dirFiles.find(
  ...
  const getAgentIgnorePatterns = async () => {
    if (agentIgnoreFile) {
      const contents = await ide.readFile(`${currentDir}/.agentignore`);
      return gitIgArrayFromFile(contents);
    }
    return [];
  };
  ...
  const ignoreContext = ignore()
    ...
    .add(defaultAndGlobalIgnores) // default file/folder ignores followed by global .agentignore - this is combined for speed
    .add(ignoreArrays[1]); // local .agentignore
