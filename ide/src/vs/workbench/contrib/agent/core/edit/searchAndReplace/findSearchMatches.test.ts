...
const baz = foo;`;
  ...
  describe("edge cases", () => {
    ...
    it("should handle Unicode characters", () => {
      const content = "const label = 'café 日本語';";
      const search = "café 日本語";
      const matches = findSearchMatches(content, search);

      expect(matches).toHaveLength(1);
      expect(matches[0].strategyName).toBe("exactMatch");
      expect(
        content.slice(matches[0].startIndex, matches[0].endIndex),
      ).toBe(search);
    });
  });
  ...
  describe("jaro winkler fuzzy match strategy", () => {
    it("should match multi-line blocks without collapsing to a single line", () => {
  ...
  return "helo";
      ...
      expect(matches).toHaveLength(1);
      expect(matches[0].strategyName).toBe("jaroWinklerFuzzyMatch");
      expect(content.slice(matches[0].startIndex, matches[0].endIndex)).toBe(
        `function greet() {
  return "hello";
}`,
