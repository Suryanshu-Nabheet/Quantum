...
export function executeFindAndReplace(
  ...
  if (matches.length === 0) {
    throw new AgentError(
      AgentErrorReason.FindAndReplaceOldStringNotFound,
      `Edit at index ${editIndex}: string not found in file: "${oldString}"`,
    );
  }

  if (replaceAll) {
    const hasLooseMatch = matches.some(
      (match) =>
        match.strategyName === "whitespaceIgnoredMatch" ||
        match.strategyName === "jaroWinklerFuzzyMatch",
    );
    if (hasLooseMatch && matches.length > 1) {
      throw new AgentError(
        AgentErrorReason.FindAndReplaceMultipleOccurrences,
        `Edit at index ${editIndex}: replace_all=true matched ${matches.length} loose occurrences for "${oldString}". Provide an exact string or narrower context before replacing all.`,
      );
    }
