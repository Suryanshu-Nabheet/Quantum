...
function findFuzzyMatch(
  ...
  if (searchBlock.length > 5) {
    ...
    for (let i = 0; i <= fileLines.length - searchLines.length; i++) {
      ...
      const candidateBlockRaw = candidateLines.join("\n");
      const candidateBlock = candidateBlockRaw.trim();
      ...
      if (similarity >= threshold && similarity > bestSimilarity) {
        ...
        const blockStartInFile =
          linesBeforeMatch.join("\n").length +
          (linesBeforeMatch.length > 0 ? 1 : 0);
        const trimStartOffset = candidateBlockRaw.indexOf(candidateBlock);
        const startIndex = blockStartInFile + trimStartOffset;
  ...
  // Line-by-line matching only for single-line search; multi-line blocks must stay intact
...
const baseMatchingStrategies: Array<{ strategy: MatchStrategy; name: string }> =
  [
...
const fuzzyMatchingStrategy = {
  strategy: findFuzzyMatch,
  name: "jaroWinklerFuzzyMatch",
};
...
function getMatchingStrategies(
  ...
  return includeFuzzy
    ? [...baseMatchingStrategies, fuzzyMatchingStrategy]
    : baseMatchingStrategies;
}

function runMatchingStrategies(
  fileContent: string,
  searchContent: string,
  includeFuzzy: boolean,
  ...
  for (const { strategy, name } of getMatchingStrategies(includeFuzzy)) {
...
export function findSearchMatch(
  ...
  return runMatchingStrategies(fileContent, searchContent, true);
}
...
export function findSearchMatches(
  ...
  while (remainingContent.length > 0) {
    const match = runMatchingStrategies(
      remainingContent,
      searchContent,
      matches.length === 0,
    );
