...
function toWslPath(windowsOrWslPath: string): string {
  const normalized = windowsOrWslPath.replace(/\\/g, "/");
  const driveMatch = normalized.match(/^([A-Za-z]):\/?(.*)$/);
  if (driveMatch) {
    const rest = driveMatch[2] ? `/${driveMatch[2]}` : "";
    return `/mnt/${driveMatch[1].toLowerCase()}${rest}`;
  }
  return normalized;
}

function getShellCommandForIde(
  command: string,
  options: { useWslFromWindows: boolean; cwd?: string },
): { shell: string; args: string[] } {
  if (options.useWslFromWindows) {
    let wslCommand = command;
    if (options.cwd) {
      const wslCwd = toWslPath(options.cwd);
      wslCommand = `cd ${JSON.stringify(wslCwd)} && ${command}`;
    }
    return {
      shell: "wsl.exe",
      args: ["-e", "bash", "-lc", wslCommand],
    };
  }

  return getShellCommand(command);
}
...
export const runTerminalCommandImpl: ToolImpl = async (args, extras) => {
  ...
  const useWslFromWindows =
    isWindowsHostWithRemote && ideInfo.remoteName === "wsl";

  if (
    ENABLED_FOR_REMOTES.includes(ideInfo.remoteName) &&
    (!isWindowsHostWithRemote || useWslFromWindows)
  ) {
    ...
    if (extras.onPartialOutput) {
      try {
        ...
        return new Promise((resolve, reject) => {
          ...
          const { shell, args } = getShellCommandForIde(command, {
            useWslFromWindows,
            cwd,
          });
          const childProc = childProcess.spawn(shell, args, {
            cwd: useWslFromWindows ? undefined : cwd,
            env: getColorEnv(),
          });
      ...
      if (waitForCompletion) {
        ...
        try {
          ...
          const { shell: nonStreamingShell, args: nonStreamingArgs } =
            getShellCommandForIde(command, { useWslFromWindows, cwd });
  ...
  // Remote / Windows-host-with-remote: run in integrated terminal and capture output.
  await extras.ide.runCommand(command, { terminalName: "Agent" });
    ...
    return [
      {
        name: "Terminal",
        description: "Terminal command output",
        content: "Command is running in the integrated terminal...",
        status: "Command is running in the background...",
      },
    ];
  }

  const pollDelaysMs = [300, 500, 700, 1000, 1500];
  let terminalOutput = "";
  for (const delayMs of pollDelaysMs) {
    await new Promise((resolve) => setTimeout(resolve, delayMs));
    terminalOutput = await extras.ide.getTerminalContents();
  ...
  return [
    {
      ...
      description: "Terminal command output",
      content:
        "Command sent to the integrated terminal. Output could not be captured automatically — check the Agent terminal panel.",
