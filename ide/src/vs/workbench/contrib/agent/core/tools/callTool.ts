...
import { invokeBrowserToolImpl, isBrowserToolName } from "./implementations/invokeBrowserTool";
    ...
    case "mcp:":
      ...
      (response.content as any).forEach((item: any) => {
        if (item.type === "text") {
          ...
          const resourceUri = item.resource?.uri ?? "unknown";
          if (item.resource?.blob) {
            contextItems.push({
              name: extras.tool.displayTitle,
              description: "MCP resource (binary)",
              content: `[Binary MCP resource: ${resourceUri}]`,
              icon: extras.tool.faviconUrl,
            });
          } else if (typeof item.resource?.text === "string") {
            contextItems.push({
              name: extras.tool.displayTitle,
              description: "Tool output",
            ...
            contextItems.push({
              ...
              content: `[MCP resource without text: ${resourceUri}]`,
              icon: extras.tool.faviconUrl,
            });
          }
        } else {
...
export async function callBuiltInTool(
  ...
  if (isBrowserToolName(functionName)) {
    return await invokeBrowserToolImpl(args, extras);
  }
...
export async function callTool(
  ...
  try {
    ...
    let errorMessage = `${e}`;
    let errorReason: AgentErrorReason | undefined;
