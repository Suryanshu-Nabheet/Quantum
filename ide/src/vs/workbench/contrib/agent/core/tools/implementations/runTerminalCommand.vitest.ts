...
describe("runTerminalCommandImpl", () => {
  ...
  const createMockExtras = (
    ...
    const mockGetTerminalContents = vi
      .fn()
      .mockResolvedValue("captured terminal output");

    const mockIde = {
      getIdeInfo: mockGetIdeInfo,
      getWorkspaceDirs: mockGetWorkspaceDirs,
      runCommand: mockRunCommand,
      getTerminalContents: mockGetTerminalContents,
  ...
  it("should handle remote environments", async () => {
    ...
    expect(result[0].status).toMatch(/Command completed|Sent to terminal/);
  });
  ...
  describe("cwd handling", () => {
    ...
    describe("remote environment handling", () => {
      ...
      it("should run via wsl.exe when Windows host connects to WSL", async () => {
...
describe("runTerminalCommandTool.evaluateToolCallPolicy", () => {
  ...
  it("should keep permission gate when user policy requires approval", () => {
  ...
  it("should auto-approve cd commands", () => {
    const basePolicy = "allowedWithoutPermission";
    const args = { command: "cd src" };
