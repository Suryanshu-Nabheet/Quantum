...
import { getTemplateVariables } from "agent-config";
import {
  SSEClientTransport,
  SseError,
...
class MCPConnection {
  ...
  async connectClient(forceRefresh: boolean, externalSignal: AbortSignal) {
    ...
    const unrendered = vars.map((v) => v.replace(/^secrets\./, ""));

    if (unrendered.length > 0) {
      this.errors.push(
        `${this.options.name} MCP Server has unresolved secrets: ${unrendered.join(", ")}.
Set secrets in ~/.agent/.env or your workspace .env file (see config docs in the extension README).`,
      );
    }
