...
export class ProfileLifecycleManager {
  private savedConfigResult: ConfigResult<AgentConfig> | undefined;
  private savedBrowserConfigResult?: ConfigResult<BrowserSerializedAgentConfig>;
  private pendingConfigPromise?: Promise<ConfigResult<AgentConfig>>;
  ...
  async loadConfig(
    ...
    this.pendingConfigPromise = new Promise((resolve) => {
      void (async () => {
        ...
        // Like invalid JSON, file read errors, etc.
        // NOT block-specific loading errors
        try {
          result = await this.profileLoader.doLoadConfig();
        } catch (e) {
          // Capture config loading system failures in local logs
          Logger.error(e, {
  ...
  async getSerializedConfig(
    additionalContextProviders: IContextProvider[],
  ): Promise<ConfigResult<BrowserSerializedAgentConfig>> {
