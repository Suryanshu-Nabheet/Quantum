...
test("GitDiffCache maintains singleton instance per key", () => {
  const getDiffFn1 = vi.fn();
  const getDiffFn2 = vi.fn();

  const instance1 = GitDiffCache.getInstance(getDiffFn1, undefined, "key-a");
  const instance2 = GitDiffCache.getInstance(getDiffFn2, undefined, "key-a");
  const instance3 = GitDiffCache.getInstance(getDiffFn2, undefined, "key-b");

  expect(instance1).toBe(instance2);
  expect(instance1).not.toBe(instance3);
});

test("GitDiffCache invalidateAll clears every instance", async () => {
  const getDiffFn = vi.fn().mockResolvedValue(["diff"]);
  const cache = GitDiffCache.getInstance(getDiffFn, undefined, "test-key");

  await cache.get();
  GitDiffCache.invalidateAll();
  const refreshedCache = GitDiffCache.getInstance(
    getDiffFn,
    undefined,
    "test-key",
  );
  await refreshedCache.get();
