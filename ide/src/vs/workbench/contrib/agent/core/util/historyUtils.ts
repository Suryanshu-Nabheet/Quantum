...
import { getAgentGlobalPath } from "../util/paths.js";
...
export async function shareSession(
  ...
  outputDir = outputDir ?? getAgentGlobalPath();
  ...
  if (outputDir.startsWith("~")) {
    ...
    } else if (workspaceDirectory.includes("://")) {
      // Remote workspaces cannot be resolved to a local path for fs writes.
      outputDir = path.join(getAgentGlobalPath(), "exports");
    } else {
  ...
  const outPath = path.join(outputDir, `${dtString}_session.md`);
