...
class FileSystemIde implements IDE {
  ...
  getBrowserPageContext(_browserId: string) {
    return Promise.resolve(undefined);
  }

  ensureBrowserPageShared(_browserId: string) {
    return Promise.resolve(false);
  }

  invokeBrowserTool(_toolId: string, _parameters: Record<string, unknown>) {
    return Promise.resolve(
      "Browser tools are not available in this environment.",
    );
  }
