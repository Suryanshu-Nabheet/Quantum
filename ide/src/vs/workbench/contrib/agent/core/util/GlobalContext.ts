...
import { MCPServer, ModelConfig, ModelRole } from "agent-config";
...
export interface StoredGuiRule {
  id: string;
  name: string;
  rule: string;
  description?: string;
...
export interface StoredGuiPrompt {
  id: string;
  name: string;
  prompt: string;
  description?: string;
    ...
    if (!fs.existsSync(filepath)) {
      ...
      try {
        ...
        try {
          ...
          // If salvage fails, proceed with empty salvaged object
        }
