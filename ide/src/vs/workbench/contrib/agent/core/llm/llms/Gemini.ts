...
import {
  ...
  convertAgentToolToGeminiFunction,
} from "./gemini-types";
...
class Gemini extends BaseLLM {
  ...
  agentPartToGeminiPart(part: MessagePart): GeminiChatContentPart {
    if (part.type === "text") {
      return {
        text: part.text,
      };
            ...
            const assistantMsg: GeminiChatContent = {
              ...
              parts:
                typeof msg.content === "string"
                  ? [{ text: msg.content }]
                  : msg.content.map(this.agentPartToGeminiPart),
            };
    ...
    if (!isV1API) {
      ...
      if (options.tools?.length) {
        ...
        options.tools.forEach((tool) => {
          try {
            functions.push(convertAgentToolToGeminiFunction(tool));
          } catch (e) {
            console.warn(
