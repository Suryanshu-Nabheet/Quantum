...
/**
 * Checks if a rule is a root-level rule (.agent directory or no file path)
 */
const isRootLevelRule = (rule: RuleWithSource): boolean => {
  return !rule.sourceFile || rule.sourceFile.includes(".agent/"); // sourceFile path is absolute - hence we need to check for it in between
};
...
export const shouldApplyRule = (
  ...
  // Check if this is a root-level rule (in .agent directory or no file path)
