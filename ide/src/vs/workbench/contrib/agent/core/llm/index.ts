...
export abstract class BaseLLM implements ILLM {
  ...
  private async parseError(resp: any): Promise<Error> {
    ...
    if (resp.status === 404 && !resp.url.includes("/v1")) {
      ...
      if (model && (resp.url.match("127.0.0.1:11434") || resp.url.match("localhost:11434"))) {
        ...
        text =
          "This may mean that you are using a provider that expects '/v1' at the end of the apiBase (e.g. LocalAI, vLLM). If you are using Ollama, ensure the model is pulled and Ollama is running.";
      }
  ...
  fetch(url: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    const customFetch = async (input: URL | RequestInfo, init: any) => {
      try {
        ...
        // Capture all fetch errors in local logs for debugging
        Logger.error(e, {
          context: "llm_fetch",
          url: String(input),
  ...
  async *streamFim(
    ...
    try {
      ...
      // Capture FIM (Fill-in-the-Middle) completion failures in local logs
  ...
  async *streamComplete(
    ...
    try {
      ...
      // Capture streaming completion failures in local logs
      Logger.error(e as Error, {
  ...
  async complete(
    ...
    try {
      ...
      // Capture completion failures in local logs
      Logger.error(e as Error, {
  ...
  private _modifyCompletionOptions(
    ...
    // o1 models do not support streaming (OpenAI API limitation).
  ...
  async *streamChat(
    ...
    try {
      ...
      // Capture chat streaming failures in local logs
      Logger.error(e as Error, {
