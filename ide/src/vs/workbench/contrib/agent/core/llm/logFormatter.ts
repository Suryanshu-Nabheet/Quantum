...
export class LLMLogFormatter {
  ...
  private logFragment(
    ...
    let wrapBreakAt = null;
    if (
      wrap &&
      fragment.length - startAt > this.wrapWidth - this.openLineChars
    ) {
      wrapBreakAt = startAt + this.wrapWidth - this.openLineChars;

      // Look for a better line-breaking point at whitespace
      const searchBackwardLimit = Math.max(startAt, wrapBreakAt - 20); // Don't look back too far
      for (let i = wrapBreakAt; i > searchBackwardLimit; i--) {
        if (/\s/.test(fragment.charAt(i))) {
          wrapBreakAt = i + 1; // Break after the whitespace
          break;
        }
      }
      ...
      fragment = fragment.substring(startAt, wrapBreakAt).trimEnd();
      joinAfter = false;
    } else if (startAt > 0) {
