...
describe("LLM", () => {
  ...
  testLLM(
    ...
    {
      skip: !hasApiKey(process.env.ANTHROPIC_API_KEY),
      testToolCall: true,
    },
  );
  testLLM(new OpenAI({ apiKey: process.env.OPENAI_API_KEY, model: "gpt-4o" }), {
    skip: !hasApiKey(process.env.OPENAI_API_KEY),
    testToolCall: true,
  });
  testLLM(
  ...
  testLLM(
    ...
    {
      testFim: true,
      skip: !hasApiKey(process.env.MISTRAL_API_KEY),
      testToolCall: true,
      timeout: 60000,
    },
  );
  testLLM(
    new Azure({
      ...
      apiBase: "https://agent-azure-openai-instance.openai.azure.com",
  ...
  testLLM(
    new Azure({
      ...
      apiBase: "https://agent-foundry-resource.services.ai.azure.com",
      env: { apiType: "azure-foundry", apiVersion: "2024-05-01-preview" },
    }),
    { testFim: false, skip: true, timeout: 20000 }, // Skipped - timing out in CI
  );

  const runsLiveLlmTests =
    hasApiKey(process.env.ANTHROPIC_API_KEY) ||
    hasApiKey(process.env.OPENAI_API_KEY) ||
    hasApiKey(process.env.MISTRAL_API_KEY);

  if (!runsLiveLlmTests) {
    test("Skipping live LLM integration tests (no API keys configured)", () => {});
  }
});
