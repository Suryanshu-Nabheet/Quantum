...
export class MessageIde implements IDE {
  ...
  async getClipboardContent(): Promise<{ text: string; copiedAt: string }> {
    return await this.request("getClipboardContent", undefined);
  }
  ...
  getFileResults(
    ...
    return this.request("getFileResults", {
      pattern,
      maxResults,
  ...
  readFileAsDataUrl(
    ...
    return this.request("readFileAsDataUrl", {
      filepath: fileUri,
      maxBytes,
    });
  }
  ...
  getBrowserPages() {
    return this.request("getBrowserPages", undefined);
  }

  getBrowserPageContext(browserId: string) {
    return this.request("getBrowserPageContext", { browserId });
  }

  ensureBrowserPageShared(browserId: string) {
    return this.request("ensureBrowserPageShared", { browserId });
  }

  invokeBrowserTool(toolId: string, parameters: Record<string, unknown>) {
    return this.request("invokeBrowserTool", { toolId, parameters });
  }

  notifyAgentBrowserSubmit() {
    return this.request("notifyAgentBrowserSubmit", undefined);
  }
}
