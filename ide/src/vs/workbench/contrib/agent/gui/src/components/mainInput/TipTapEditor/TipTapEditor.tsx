...
import { handleImageFile, insertImageDataUrl } from "./utils/imageUtils";
...
function TipTapEditorInner(props: TipTapEditorProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (props.isMainInput) {
        mainEditorContext.setMainEditor(null);
      }
    };
  }, [
    editor,
  ...
  return (
    <InputBoxDiv
      ...
      onKeyUp={handleKeyUp}
      bigger={props.isMainInput && historyLength === 0}
      className={
    ...
    >
      <div
        className={`flex flex-grow flex-col px-2 pb-1 pt-1 ${props.isMainInput && historyLength === 0 ? "min-h-[140px]" : ""}`}
      >
        <EditorContent
          className={`scroll-container flex-grow ${props.isMainInput ? "max-h-[70vh] overflow-y-auto" : "overflow-hidden"}`}
          spellCheck={false}
          editor={editor}
        />
        <InputToolbar
