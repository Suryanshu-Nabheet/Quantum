...
interface AtMentionDropdownProps {
  ...
  editor: Editor;
  enterSubmenu?: (editor: Editor, providerId: string) => void | Promise<void>;
  onClose: () => void;
}
...
const AtMentionDropdown = forwardRef((props: AtMentionDropdownProps, ref) => {
  ...
  function handleItemTooBig(
    ...
    if (fileExceeds) {
      // Trigger warning toast but don't delete text or block addition
      void ideMessenger.ide.showToast(
        "warning",
        `'${item.title}' is very large (${formatFileSize(fileSize)}) and may be truncated.`,
      );
    }
      ...
      items.push(
        {
          ...
          action: () => {
            ...
            dispatch(setDialogMessage(<AddRuleDialog />));
        ...
        },
        {
          ...
          description: "Create a slash-command prompt",
        },
      );
    }
  ...
  const selectItem = (index: number) => {
    ...
    if (item) {
      ...
      } else if (item.type === "code" && item.query) {
        // Code ranges are client-built context items (providerTitle "code"),
        // not a registered @ provider — insert directly.
        props.command({ ...item, itemType: item.type });
      } else {
        props.command({ ...item, itemType: item.type });
      }
  ...
  const showFileIconForItem = (item?: ComboBoxItem) => {
    return Boolean(item && ["file", "code"].includes(item.type));
  };
              ...
              <span className="flex w-full min-w-0 items-center gap-2">
                <div className="flex min-w-0 shrink-0 items-center">
                  ...
                  <div className="flex items-center -mt-2">
                    <TextShimmer>{loadingSubmenuItem.title}</TextShimmer>
                  </div>
                </div>
                {loadingSubmenuItem.description && (
                  <span
                    title={loadingSubmenuItem.description}
                ...
                >
                  <span className="flex w-full min-w-0 items-center gap-2">
                    <div className="flex min-w-0 shrink-0 items-center">
                      ...
                      <span title={item.title} className="whitespace-nowrap">
                        {item.title}
                      </span>
                      {item.type === "contextProvider" &&
                        item.contextProvider?.type === "submenu" && (
                      ...
                      {item.subActions?.map((subAction) => {
                        ...
                        return (
                          <HeaderButtonWithToolTip
                            key={subAction.icon || subAction.label}
                            onClick={(e) => {
