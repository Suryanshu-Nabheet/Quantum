...
export const useFindWidget = (
  ...
  const widget = (
    <div
      className={`fixed top-0 z-50 transition-all ${open ? "" : "-translate-y-full"} bg-vsc-background right-0 flex flex-row items-center gap-1.5 rounded-bl-lg border-0 border-b border-l border-solid border-border pl-[3px] pr-3 sm:gap-2`}
    >
      <Input
        disabled={disabled}
        type="text"
      ...
      <HeaderButtonWithToolTip
        inverted={useRegex}
        text={useRegex ? "Turn off regex search" : "Turn on regex search"}
        onClick={(e) => {
          e.stopPropagation();
          setUseRegex((curr) => !curr);
        }}
        className="h-5 w-6 rounded-full border font-mono text-xs focus:outline-none focus:ring"
      >
        .*
      </HeaderButtonWithToolTip>
      <HeaderButtonWithToolTip
