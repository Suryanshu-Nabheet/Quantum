...
export function McpAppRenderer({
        ...
        contents: [
          {
            uri: params.uri,
            mimeType: "text/plain",
            text: "MCP resource reads from the app UI are not wired in the sidebar. Attach resources with @ mentions instead.",
          },
        ],
      };
    };
  ...
  const handleIframeLoad = useCallback(async () => {
    ...
    try {
      ...
      console.error("[Agent] Failed to connect bridge to MCP App UI:", err);
  ...
  if (error) {
    return (
      <div
        className={`bg-input text-description w-full p-4 text-sm ${
          prefersBorder ? "border-input rounded border" : ""
        }`}
        role="alert"
      >
        <p className="text-foreground mb-1 font-medium">MCP App failed to load</p>
