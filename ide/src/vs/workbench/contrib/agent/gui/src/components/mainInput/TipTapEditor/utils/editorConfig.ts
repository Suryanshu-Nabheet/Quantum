...
import { handleImageFile, insertImageDataUrl } from "./imageUtils";
...
export function getPlaceholderText(
  ...
  return historyLength === 0
    ? "Ask anything, '@' to add context, '/' for workflows"
    : "Ask a follow-up";
}
 ...
 */
function hasValidEditorContent(json: JSONContent): boolean {
  ...
  const hasImage = json.content?.some((c) => c.type === "image");
  ...
  // Content is valid if it has text, special blocks, or an attached image
...
export function createEditorConfig(options: {
  ...
  const { getSubmenuContextItems, refreshSubmenuProviders } =
  ...
  const refreshSubmenuProvidersRef = useUpdatingRef(refreshSubmenuProviders);
  ...
  const enterSubmenu = async (editor: Editor, providerId: string) => {
    ...
    const refreshPromise = refreshSubmenuProvidersRef.current([providerId]);
    refreshSuggestions();
  ...
  const editor: Editor | null = useEditor({
    extensions: [
      ...
      Image.extend({
                ...
                paste(view, event) {
                  ...
                  for (const item of items) {
                    ...
                    void handleImageFile(ideMessenger, file).then((resp) => {
                      ...
                      const [, dataUrl] = resp;
                      if (tipTapEditor) {
                        insertImageDataUrl(tipTapEditor, dataUrl);
                      } else {
        ...
        // Required for chat attachments — we insert compressed data-URL images.
        allowBase64: true,
        HTMLAttributes: {
