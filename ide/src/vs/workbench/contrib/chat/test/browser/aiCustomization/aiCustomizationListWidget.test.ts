...
suite('aiCustomizationListWidget', () => {
	...
	suite('extractExtensionIdFromPath', () => {
		...
		test('extracts extension ID from Code OSS dev path', () => {
			assert.strictEqual(
				extractExtensionIdFromPath('/Users/josh/.quantum-dev/extensions/github.copilot-chat-0.43.2026040602/assets/prompts/skills/troubleshoot/SKILL.md'),
				'github.copilot-chat'
			);
		});
		...
		test('extracts extension ID from User/globalStorage path (Copilot Chat ask agent)', () => {
			assert.strictEqual(
				extractExtensionIdFromPath('/Users/josh/.quantum-dev/User/globalStorage/github.copilot-chat/ask-agent/Ask.agent.md'),
				'github.copilot-chat'
			);
		});
		...
		test('returns undefined for non-extension entries in globalStorage', () => {
			...
			assert.strictEqual(
				extractExtensionIdFromPath('/Users/josh/.quantum-dev/User/globalStorage/state.vscdb'),
				undefined
			);
		});
	});
