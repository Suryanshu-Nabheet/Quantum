...
import { ActivityBarPosition, IWorkbenchLayoutService, LayoutSettings, Position } from '../../../../../services/layout/browser/layoutService.js';
...
export class ChatViewPane extends ViewPane implements IViewWelcomeDelegate {
	...
	private updateViewPaneClasses(fromEvent: boolean): void {
		...
		const activityBarLocationKey = location === ViewContainerLocation.AuxiliaryBar
			? LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION
			: LayoutSettings.ACTIVITY_BAR_LOCATION;
		const activityBarLocationDefault = this.configurationService.getValue<string>(activityBarLocationKey) === ActivityBarPosition.DEFAULT;
	...
	private registerListeners(): void {
		...
		this._register(Event.filter(this.configurationService.onDidChangeConfiguration, e => {
			return e.affectsConfiguration(LayoutSettings.ACTIVITY_BAR_LOCATION) || e.affectsConfiguration(LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION);
