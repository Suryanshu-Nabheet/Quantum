...
class BrowserChatAgentToolsContribution extends Disposable implements IWorkbenchContribution {
	...
	private _syncModelListeners(): void {
		...
		// Add listeners for new views, including inputs whose model resolves later.
		for (const [id, input] of views) {
			if (!this._modelListeners.has(id)) {
				const store = new DisposableStore();
				let observedModel: typeof input.model;
				const addModelListener = () => {
					const model = input.model;
					if (model && model !== observedModel) {
						observedModel = model;
						store.add(model.onDidChangeSharingState(() => this._updateBrowserContext()));
						this._updateBrowserContext();
					}
				};
				addModelListener();
				store.add(input.onDidResolveModel(addModelListener));
