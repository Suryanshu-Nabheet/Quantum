 ...
 * Falls back to the Agent-native Playwright session when no chat context exists.
 */
export const AGENT_NATIVE_PLAYWRIGHT_SESSION_ID = 'agent';

export function getSessionId(invocation: IToolInvocation): string {
	return invocation.context?.sessionResource?.toString() ?? AGENT_NATIVE_PLAYWRIGHT_SESSION_ID;
}
