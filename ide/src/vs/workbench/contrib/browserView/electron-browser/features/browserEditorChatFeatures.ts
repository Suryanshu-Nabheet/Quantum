...
import { AGENT_ATTACH_BROWSER_CONTEXT_COMMAND, AGENT_BROWSER_NOTIFY_SUBMIT_COMMAND, AgentNativeModeContext } from '../../common/agentBridge.js';
...
const CONTEXT_BROWSER_AGENT_ENABLED = ContextKeyExpr.or(
	AgentNativeModeContext,
	ContextKeyExpr.equals(`config.${ChatConfiguration.AIDisabled}`, true),
	ContextKeyExpr.equals('chatIsEnabled', true),
);
...
export class BrowserEditorChatIntegration extends BrowserEditorContribution {
	...
	protected override subscribeToModel(model: IBrowserViewModel, store: DisposableStore): void {
		...
		store.add(model.onDidSelectElement(async data => {
			try {
				await this._attachElementDataToAgent(data, model);
			} catch (error) {
				this.logService.error('BrowserEditor.addElementToAgent: Failed to attach element', error);
			}
		}));
	...
	private _toggleShareWithAgent(): void {
		...
		const shouldShare = model.sharingState !== BrowserViewSharingState.Shared;
		void model.setSharedWithAgent(shouldShare).catch(error => {
			this.logService.error('BrowserEditor.shareWithAgent: Failed to toggle sharing', error);
		});
	}
	...
	private async _attachElementDataToAgent(elementData: IElementData, model: IBrowserViewModel) {
		...
		await this._attachContextToAgent(displayNameShort, displayNameFull, value, model.id);
	...
	private async _attachContextToAgent(name: string, description: string, content: string, browserId: string): Promise<void> {
		await this.commandService.executeCommand(AGENT_ATTACH_BROWSER_CONTEXT_COMMAND, {
			name,
			description,
			content,
			uri: BrowserViewUri.forId(browserId).toString(),
	...
	async addConsoleLogsToChat(): Promise<void> {
		...
		try {
			const logs = (await model.getConsoleLogs()).trim();
			if (!logs) {
				await this.dialogService.info(
					localize('browser.noConsoleLogs', "No console logs to attach."),
					localize('browser.noConsoleLogsDetail', "Run code in the page that calls console.log, console.warn, or console.error, then try again."),
				);
				return;
			}
			...
			await this._attachContextToAgent(
				localize('consoleLogs', 'Console Logs'),
				localize('consoleLogs', 'Console Logs'),
				logs,
				model.id,
			);
...
class AddElementToChatAction extends Action2 {
	...
	constructor() {
		super({
			...
			f1: true,
			precondition: ContextKeyExpr.and(BROWSER_EDITOR_ACTIVE, CONTEXT_BROWSER_HAS_URL, CONTEXT_BROWSER_HAS_ERROR.negate(), CONTEXT_BROWSER_AGENT_ENABLED),
			toggled: CONTEXT_BROWSER_ELEMENT_SELECTION_ACTIVE,
			menu: {
				id: MenuId.BrowserActionsToolbar,
				group: 'actions',
				order: 1,
				when: CONTEXT_BROWSER_AGENT_ENABLED
			},
			keybinding: [{
...
class AddConsoleLogsToChatAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const browserEditor = editorService.activeEditorPane;
		if (!(browserEditor instanceof BrowserEditor)) {
			return;
		}
...
registerAction2(class AgentBrowserNotifySubmitAction extends Action2 {
	constructor() {
		super({
			id: AGENT_BROWSER_NOTIFY_SUBMIT_COMMAND,
			title: localize2('agentBrowser.notifySubmit', "Notify Agent Browser Submit"),
			f1: false,
		});
	}
	...
	run(accessor: ServicesAccessor): void {
		...
		if (activePane.model?.isElementSelectionActive) {
			void activePane.model.toggleElementSelection(false);
		}
	}
});
