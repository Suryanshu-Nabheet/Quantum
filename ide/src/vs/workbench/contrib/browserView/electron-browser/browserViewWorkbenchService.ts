...
import { AgentNativeModeContext } from '../common/agentBridge.js';

/**
 * Native VS Code inspect/focus blue. Used for the element picker so the overlay
 * looks the same as it does in stock themes, where `focusBorder` is blue.
 * Quantum themes intentionally clear `focusBorder`, so we can't derive it.
 */
const DEFAULT_PICKER_HIGHLIGHT = '#0078d4';

function opaqueColorString(color: Color | undefined): string | undefined {
...
export class BrowserViewWorkbenchService extends Disposable implements IBrowserViewWorkbenchService {
	...
	private static readonly _sharingAvailableContext = ContextKeyExpr.or(
		ContextKeyExpr.has(`config.${ChatConfiguration.AIDisabled}`),
		ContextKeyExpr.and(
	...
	private _refreshSharingAvailable(): void {
		const was = this._isSharingAvailable;
		this._isSharingAvailable =
			this.configurationService.getValue<boolean>(ChatConfiguration.AIDisabled) === true ||
			this.contextKeyService.contextMatchesRules(BrowserViewWorkbenchService._sharingAvailableContext);
	...
	private sendTheme(): void {
		...
		// Quantum themes set focusBorder fully transparent by design. Honor a
		// theme's blue focusBorder when it has one (stock themes), otherwise fall
		// back to the native VS Code inspect blue rather than the teal resize
		// accent, so the picker reads as native across themes.
		const pickerHighlight =
			opaqueColorString(theme.getColor(focusBorder))
			?? DEFAULT_PICKER_HIGHLIGHT;
		void this._browserViewService.updateTheme({
			focusBorder: opaqueColorString(theme.getColor(focusBorder)),
			pickerHighlight,
	...
	private sendConfiguration(): void {
		void this._browserViewService.updateConfiguration({
			aiFeaturesDisabled: !this.contextKeyService.contextMatchesRules(
				ContextKeyExpr.or(AgentNativeModeContext, ChatContextKeys.enabled),
			),
		});
	}
}
