...
export const enum LayoutSettings {
	...
	ACTIVITY_BAR_ORIENTATION = 'workbench.activityBar.orientation',
	ACTIVITY_BAR_AUTO_HIDE = 'workbench.activityBar.autoHide',
	ACTIVITY_BAR_COMPACT = 'workbench.activityBar.compact',
	AUXILIARY_ACTIVITY_BAR_LOCATION = 'workbench.auxiliaryActivityBar.location',
	AUXILIARY_ACTIVITY_BAR_AUTO_HIDE = 'workbench.auxiliaryActivityBar.autoHide',
...
export const enum ActivityBarOrientation {
	VERTICAL = 'vertical',
	HORIZONTAL = 'horizontal'
}

export function getPrimaryActivityBarPosition(configurationService: IConfigurationService): ActivityBarPosition {
	return configurationService.getValue<ActivityBarPosition>(LayoutSettings.ACTIVITY_BAR_LOCATION) ?? ActivityBarPosition.DEFAULT;
}

export function getActivityBarOrientation(configurationService: IConfigurationService): ActivityBarOrientation {
	return configurationService.getValue<ActivityBarOrientation>(LayoutSettings.ACTIVITY_BAR_ORIENTATION) ?? ActivityBarOrientation.VERTICAL;
}

export function isHorizontalActivityBarOrientation(configurationService: IConfigurationService): boolean {
	return getActivityBarOrientation(configurationService) === ActivityBarOrientation.HORIZONTAL;
}

export function shouldHideVerticalActivityBar(configurationService: IConfigurationService): boolean {
	const location = getPrimaryActivityBarPosition(configurationService);
	if (location === ActivityBarPosition.HIDDEN) {
		return true;
	}
	if (isHorizontalActivityBarOrientation(configurationService)) {
		return true;
	}
	return isHorizontalActivityBarPosition(location);
}

export function getAuxiliaryActivityBarPosition(configurationService: IConfigurationService): ActivityBarPosition {
	// Quantum reserves the secondary sidebar for Agent only — no auxiliary activity bar.
	if (configurationService.getValue<boolean>(ChatConfiguration.AIDisabled) === true) {
		return ActivityBarPosition.HIDDEN;
	}
	return configurationService.getValue<ActivityBarPosition>(LayoutSettings.AUXILIARY_ACTIVITY_BAR_LOCATION) ?? ActivityBarPosition.DEFAULT;
}

export function isHorizontalActivityBarPosition(position: ActivityBarPosition): boolean {
	return position === ActivityBarPosition.TOP || position === ActivityBarPosition.BOTTOM;
}
...
function isTitleBarEmpty(configurationService: IConfigurationService): boolean {
	...
	// with an activity bar on top/bottom (primary or secondary), or horizontal orientation, we should always show
	const primaryActivityBarPosition = getPrimaryActivityBarPosition(configurationService);
	const auxiliaryActivityBarPosition = getAuxiliaryActivityBarPosition(configurationService);
	if (isHorizontalActivityBarOrientation(configurationService) || isHorizontalActivityBarPosition(primaryActivityBarPosition) || isHorizontalActivityBarPosition(auxiliaryActivityBarPosition)) {
		return false;
	}
