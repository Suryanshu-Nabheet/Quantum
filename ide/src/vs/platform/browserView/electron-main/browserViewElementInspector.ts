...
export class BrowserViewElementInspector extends Disposable {
	...
	private async _registerListeners(): Promise<void> {
		...
		// Register before awaiting CDP so a fast preload-ready does not miss
		// the theme push that paints the picker highlight.
		const onIpcMessage = (event: { senderFrame?: unknown }, channel: string) => {
	...
	async toggleElementSelection(enabled?: boolean): Promise<void> {
		...
		const start = useCDP ? async () => {
			...
			const webContents = this.browser.webContents;
			// Re-push theme so picker CSS vars are present even if preload-ready
			// raced ahead of the first updateTheme call.
			this.setTheme(this._theme);
		...
		try {
			...
			console.error('[ElementPicker] toggleElementSelection failed:', e);
			this._activeSelection.clear();
		}
	}
