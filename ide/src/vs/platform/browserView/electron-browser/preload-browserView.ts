...
class ElementPicker {
	...
	constructor(
		...
		// Full-viewport fixed host so absolute/fixed children are never clipped
		// by a zero-sized box or unusual page overflow rules.
		shadowHost.style.cssText = 'position: fixed; inset: 0; width: auto; height: auto; overflow: visible; z-index: 2147483647; pointer-events: none;';
	...
	private _renderHighlight(target: Element): void {
		...
		// Highlight uses viewport coordinates under a fixed host.
		...
		// Label is sticky-clamped to the viewport.
	...
	private static _buildStyle(): HTMLStyleElement {
		...
		style.textContent = `
			...
			.highlight {
				...
				border: 2px solid var(--vscode-picker-highlight, #4d8bf0);
				background: color-mix(in srgb, var(--vscode-picker-highlight, #4d8bf0) 18%, transparent);
				border-radius: 2px;
				pointer-events: none;
				z-index: 2;
			}
			...
			.label {
				...
				background: var(--vscode-button-background, #4d8bf0);
			...
			.dragbox {
				position: fixed; box-sizing: border-box;
				border: 1px dotted var(--vscode-picker-highlight, #4d8bf0);
				background: color-mix(in srgb, var(--vscode-picker-highlight, #4d8bf0) 10%, transparent);
				z-index: 2;
			}
		`;
		return style;
	...
	private static _applyTheme(host: HTMLElement, theme: IBrowserViewTheme | undefined): void {
		const highlight =
			theme?.pickerHighlight
			|| theme?.focusBorder
			|| null;
		// Only set when we have a real (non-transparent) accent. Leaving the
		// property unset keeps the CSS fallback (#4d8bf0) instead of painting
		// an invisible box from a transparent focusBorder.
		if (highlight && !ElementPicker._isTransparentColor(highlight)) {
			host.style.setProperty('--vscode-picker-highlight', highlight);
		} else {
			host.style.removeProperty('--vscode-picker-highlight');
		}
	...
	private static _isTransparentColor(value: string): boolean {
		const v = value.trim().toLowerCase();
		return (
			v === 'transparent'
			|| v === '#0000'
			|| /^#([0-9a-f]{6})00$/i.test(v)
			|| /^rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*0(?:\.0+)?\s*\)$/i.test(v)
		);
	}
}
