...
export interface IActionListItem<T> {
	...
	/**
	 * Optional CSS classes applied to the row icon (e.g. codicons or extension uri-icon classes).
	 */
...
class ActionItemRenderer<T> implements IListRenderer<IActionListItem<T>, IActionMenuTemplateData> {
	...
	renderElement(element: IActionListItem<T>, _index: number, data: IActionMenuTemplateData): void {
		...
		if (element.iconClassNames?.length) {
			data.icon.className = ['icon', ...element.iconClassNames].join(' ');
			if (element.iconClassNames.includes('uri-icon')) {
				data.icon.style.color = '';
			} else {
				data.icon.style.color = 'var(--vscode-activityBarTop-foreground)';
			}
		} else if (element.group?.icon) {
			data.icon.className = ['icon', ThemeIcon.asClassName(element.group.icon)].join(' ');
