...
|---------|--------------|-----------|
| Sessions list (sidebar) | Enabled Compatible | No gate |
| Chat views (ChatBar) | Enabled Compatible | No gate |
| Changes view (AuxiliaryBar) | Disabled Gated (with mobile equivalent) | `when: !sessionsIsPhoneLayout` on view descriptor; phone uses `MobileChangesView` overlay reachable from the title-bar Changes pill |
| Files view (AuxiliaryBar) | Disabled Gated | `when: !sessionsIsPhoneLayout` on view descriptor |
| Logs view (Panel) | Disabled Gated | `when: !sessionsIsPhoneLayout` on view descriptor |
| Terminal actions | Disabled Gated | `when: !sessionsIsPhoneLayout` on menu item |
| "Open in VS Code" action | Disabled Gated | `when: !sessionsIsPhoneLayout` on menu item |
| Code review toolbar | Disabled Gated | `when: !sessionsIsPhoneLayout` on menu item |
| Customizations toolbar | Disabled Hidden | CSS `display: none` on phone |
| Titlebar | Disabled Hidden | Grid `visible: false` + CSS + MobileTitlebarPart replacement |
...
```
┌──────────────────────────────────┐
│  [☰]  Session Title      [+|User] │  ← MobileTitlebarPart (prepended before grid)
├──────────────────────────────────┤
│                                  │
...
- **MobileTitlebarPart** is a DOM element prepended above the grid. It has a hamburger (☰), session title, and a contextual right slot that swaps between the new session (+) button (when in a chat) and the account indicator User (on the welcome / new session screen).
...
|---|---|---|
| **Titlebar** (3-section toolbar) | **MobileTitlebarPart** (☰ / title / +|User) | Always visible at top |
