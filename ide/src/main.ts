...
const userDataPath = getUserDataPath(args, product.nameShort ?? 'quantum');
...
async function onReady() {
	...
	// macOS Dock caches app icons aggressively for .build/electron apps. Push the
	// branded .icns explicitly so relaunch never keeps a stale oversized tile.
	applyDarwinDockIcon();
...
function applyDarwinDockIcon(): void {
	...
	const iconPath = path.join(process.resourcesPath, `${product.nameShort}.icns`);
	try {
		...
		// Fall back to CFBundleIconFile from Info.plist
	}
}
...
function createDefaultArgvConfigSync(argvConfigPath: string): void {
	try {
		...
		const defaultArgvConfigContent = [
			'// This configuration file allows you to pass permanent command line arguments to Quantum.',
			'// Only a subset of arguments is currently supported to reduce the likelihood of breaking',
			'// the installation.',
			'//',
			'// PLEASE DO NOT CHANGE WITHOUT UNDERSTANDING THE IMPACT',
			'//',
			'// NOTE: Changing this file requires a restart of Quantum.',
			'{',
			'	// Use software rendering instead of hardware accelerated rendering.',
			'	// This can help in cases where you see rendering issues in Quantum.',
