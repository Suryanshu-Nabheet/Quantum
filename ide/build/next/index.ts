...
/**
 * Agent extension host code is built separately (gulp compile-agent / esbuild).
 * Workbench integration under contrib/agent/workbench and shared ids must still
 * transpile with the main workbench bundle.
 */
const AGENT_EXTENSION_ONLY_PATH_PREFIXES = [
	'vs/workbench/contrib/agent/src/',
	'vs/workbench/contrib/agent/core/',
	'vs/workbench/contrib/agent/gui/',
	'vs/workbench/contrib/agent/packages/',
	'vs/workbench/contrib/agent/scripts/',
	'vs/workbench/contrib/agent/node_modules/',
] as const;

const AGENT_EXTENSION_ONLY_GLOB_EXCLUDES = [
	...AGENT_EXTENSION_ONLY_PATH_PREFIXES.map(prefix => `${prefix}**`),
	'vs/workbench/contrib/agent/vitest.config.ts',
];

function isAgentExtensionOnlySource(relativePath: string): boolean {
	if (!relativePath.startsWith('vs/workbench/contrib/agent/')) {
		return false;
	}
	if (relativePath.startsWith('vs/workbench/contrib/agent/workbench/') || relativePath.startsWith('vs/workbench/contrib/agent/shared/')) {
		return false;
	}
	return AGENT_EXTENSION_ONLY_PATH_PREFIXES.some(prefix => relativePath.startsWith(prefix))
		|| relativePath === 'vs/workbench/contrib/agent/vitest.config.ts';
}
...
async function cleanDir(dir: string): Promise<void> {
	...
	// Match gulpfile.ts rimrafOutPreservingAgent: keep agent runtime + packages
	// so parallel `watch-agent` is not wiped when client transpile restarts.
	const preserveNames = dir === OUT_DIR ? ['agent', 'agent-packages'] : [];
	const preserved = preserveNames
		.map(name => {
			const source = path.join(fullPath, name);
			return {
				name,
				source,
				dest: fs.existsSync(source)
					? path.join(REPO_ROOT, `.preserve-${name}-${process.pid}-${Date.now()}`)
					: undefined,
			};
		})
		.filter((entry): entry is { name: string; source: string; dest: string } => !!entry.dest);

	console.log(`[clean] ${dir}`);
	for (const entry of preserved) {
		await fs.promises.rename(entry.source, entry.dest);
	}
	await fs.promises.rm(fullPath, { recursive: true, force: true });
	await fs.promises.mkdir(fullPath, { recursive: true });
	for (const entry of preserved) {
		await fs.promises.rename(entry.dest, path.join(fullPath, entry.name));
	}
}
...
async function copyAllNonTsFiles(outDir: string, excludeTests: boolean): Promise<void> {
	...
	const ignorePatterns = [
		...
		'**/*.ts',
		...AGENT_EXTENSION_ONLY_GLOB_EXCLUDES,
	];
	if (excludeTests) {
...
async function transpile(outDir: string, excludeTests: boolean): Promise<void> {
	...
	// Electron preload scripts must be CommonJS — overwrite any ESM output.
	await compileStandaloneFiles(outDir, false, 'desktop');
}
...
/** Paths under src/ that are rebuilt by watch-agent and must not crash watch-client-transpile. */
...
async function watch(): Promise<void> {
	...
	if (!fs.existsSync(outMain)) {
		...
		console.log(`[watch] Reusing existing ${outDir}/ (skip clean)`);
	}
