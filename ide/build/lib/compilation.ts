...
export function createCompile(src: string, { build, emitError, transpileOnly, preserveEnglish, noEmit }: ICompileTaskOptions) {
	...
	function pipeline(token?: util.ICancellationToken) {
		...
		const isRuntimeJs = (f: File) => f.path.endsWith('.js') && !f.path.includes('fixtures') && !f.isDirectory();
...
export function transpileTask(src: string, out: string, esbuild?: boolean): task.StreamTask {
	...
	const task = () => {
		...
		const srcPipe = gulp.src([`${src}/**`, ...agentExtensionCompileExcludes(src)], { base: `${src}` });
...
export function watchTypeCheckTask(src: string): task.Task {
	return task.define(`watch-typecheck-${path.basename(src)}`, () => {
		...
		generator.execute();
		const watchInput = watch([`${src}/**`, ...agentExtensionCompileExcludes(src)], { base: src, readDelay: 200 });
...
/** Electron preload scripts must be CommonJS; compiled separately after the main transpile. */
const ELECTRON_PRELOAD_FILES = [
...
export async function compileElectronPreloads(out: string): Promise<void> {
	const repoRoot = path.join(import.meta.dirname, '../..');
	await Promise.all(ELECTRON_PRELOAD_FILES.map(async (file) => {
		const entryPath = path.join(repoRoot, 'src', file);
		const outPath = path.join(repoRoot, out, file.replace(/\.ts$/, '.js'));
		await esbuild.build({
...
export const compileElectronPreloadsTask = task.define('compile-electron-preloads', async () => {
	await compileElectronPreloads('out');
	fancyLog(ansiColors.green('Finished compile-electron-preloads'));
});

/** Agent extension sources are built separately; match exclusions in src/tsconfig.json. */
function agentExtensionCompileExcludes(src: string): string[] {
	return [
		`!${src}/vs/workbench/contrib/agent/src/**`,
		`!${src}/vs/workbench/contrib/agent/core/**`,
		`!${src}/vs/workbench/contrib/agent/gui/**`,
		`!${src}/vs/workbench/contrib/agent/packages/**`,
		`!${src}/vs/workbench/contrib/agent/scripts/**`,
		`!${src}/vs/workbench/contrib/agent/node_modules/**`,
		`!${src}/vs/workbench/contrib/agent/vitest.config.ts`,
	];
}
...
export const watchExtensionPointNamesTask = task.define('watch-extension-point-names', () => {
	...
	const task = () => gulp.src(sources)
		.pipe(generateExtensionPointNames())
		.pipe(extensionPointNamesReporter.end(true));

	return watch(sources, { readDelay: 200 })
