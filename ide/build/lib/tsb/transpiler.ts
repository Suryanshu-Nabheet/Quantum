...
export class ESBuildTranspiler implements ITranspiler {
	...
	transpile(file: Vinyl): void {
		...
		const t1 = Date.now();
		// Electron sandbox preload scripts must be CommonJS (not ESM), or
		// Electron fails to load them with "Unexpected token 'export'". The workbench
		// (ESM) and extensions (CJS) are handled by the isExtension check above; standalone
		// preload scripts under vs/base/parts/sandbox/electron-browser/ and
		// vs/platform/browserView/electron-browser/ are not, so compile them as CJS here.
		const normalizedPath = file.path.replace(/\\/g, '/');
		const isPreload = normalizedPath.includes('/sandbox/electron-browser/') || normalizedPath.includes('/browserView/electron-browser/');
		this._jobs.push(esbuild.transform(file.contents, {
			...this._transformOpts,
			sourcefile: file.path,
			...(isPreload ? { format: 'cjs' as const, platform: 'node' as const } : {}),
		}).then(result => {
