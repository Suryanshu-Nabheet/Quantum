...
export async function esbuildExtensions(taskName: string, isWatch: boolean, scripts: { script: string; outputRoot?: string }[]): Promise<void> {
	...
	const tasks = scripts.map(({ script, outputRoot }) => {
		return new Promise<void>((resolve, reject) => {
			...
			// Watch mode must stream stdio — execFile buffers stderr and eventually
			// hits maxBuffer, which kills the entire `npm run watch` pipeline.
			if (isWatch) {
				const proc = cp.spawn(process.argv[0], args, { stdio: ['ignore', 'pipe', 'pipe'] });
				...
				proc.stderr?.on('data', (data: Buffer) => {
					...
					if (text) {
						fancyLog(`${ansiColors.green(taskName)}: ${text}`);
					}
				});
			...
			const proc = cp.execFile(process.argv[0], args, { maxBuffer: 10 * 1024 * 1024 }, (error, _stdout, stderr) => {
				if (error) {
					return reject(error);
				}
