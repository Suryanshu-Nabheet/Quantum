...
import { compileAgentExtension } from './gulpfile.agent.ts';
...
const sourceMappingURLBase = `https://github.com/Suryanshu-Nabheet/quantum/releases/download/${product.quality}-${commit}`;
	...
	const task = () => {
		...
		const productJsonStream = gulp.src(['product.json'], { base: '.' })
			.pipe(jsonEditor((json: Record<string, unknown>) => {
				...
				json.serverDownloadUrlTemplate = `https://github.com/Suryanshu-Nabheet/quantum/releases/download/${version}/quantum-reh-\${os}-\${arch}-${version}.tar.gz`;
				return json;
			}))
			.pipe(es.through(function (file) {
		...
		const agentRuntime = gulp.src('out/agent/**', { base: '.', dot: true });
...
BUILD_TARGETS.forEach(buildTarget => {
	...
	const [vscode, vscodeMin] = ['', 'min'].map(minified => {
		...
		const compileAgentForTargetTask = task.define(`compile-agent${dashed(platform)}${dashed(arch)}`, async () => {
			const previousTarget = process.env.AGENT_BUILD_TARGET;
			process.env.AGENT_BUILD_TARGET = `${platform}-${arch}`;
			try {
				await compileAgentExtension();
			} finally {
				if (previousTarget === undefined) {
					delete process.env.AGENT_BUILD_TARGET;
				} else {
					process.env.AGENT_BUILD_TARGET = previousTarget;
				}
			}
		});
		gulp.task(compileAgentForTargetTask);

		const packageTasks: task.Task[] = [
			compileAgentForTargetTask,
