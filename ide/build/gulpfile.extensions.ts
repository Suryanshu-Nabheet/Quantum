...
const compilations = [
	...
	'extensions/quantum-error-lens/tsconfig.json',
	'extensions/quantum-emulator/tsconfig.json',
	'extensions/quantum-pdfviewer/tsconfig.json',
	'extensions/quantum-project-tree/tsconfig.json',
	'extensions/quantum-timeline/tsconfig.json',
...
const getBaseUrl = (out: string) => `https://github.com/Suryanshu-Nabheet/Quantum/download/${product.quality}-${commit}/extensions-${out.replaceAll('/', '-')}`;
...
const tasks = compilations.map(function (tsconfigFile) {
	...
	// Watch used to clean `out/` then only compile on later src changes.
	// That left every extension broken until a file was touched (and raced
	// with `compile-extensions`, wiping freshly built mains). Always do a
	// full compile after clean, then watch for incremental rebuilds.
	const watchTask = task.define(`watch-extension:${name}`, task.series(cleanTask, async () => {
		...
		return es.merge(watchNonTs, tsgoStream);
	}));
