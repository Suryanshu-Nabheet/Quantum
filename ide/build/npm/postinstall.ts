...
function fixNpmBinPermissions(binDir: string): void {
	...
	for (const entry of fs.readdirSync(binDir)) {
		...
		const targetPath = path.resolve(binDir, fs.readlinkSync(linkPath));
		if (!fs.existsSync(targetPath)) {
			continue;
		}

		try {
			const head = fs.readFileSync(targetPath, { encoding: 'utf8' }).slice(0, 2);
			if (head !== '#!') {
				continue;
			}
...
function fixAllNpmBinPermissions(): void {
	if (process.platform === 'win32') {
		return;
	}
	fixNpmBinPermissions(path.join(root, 'node_modules', '.bin'));
	for (const dir of dirs) {
		fixNpmBinPermissions(path.join(root, dir, 'node_modules', '.bin'));
	}
}
...
async function main() {
	...
	for (const dir of dirs) {
		...
		// Agent packages share file: linked deps (core ↔ gui ↔ extension).
		// Installing them in parallel races npm extract into the same node_modules
		// (ENOTEMPTY on whatwg-url/lib etc). Run them sequentially after natives.
		if (
			dir === 'src/vs/workbench/contrib/agent/core' ||
			dir === 'src/vs/workbench/contrib/agent/gui' ||
			dir === 'src/vs/workbench/contrib/agent'
		) {
			const agentDir = dir;
			nativeTasks.push(() => {
				const env = { ...process.env };
				clearInheritedNpmrcConfig(agentDir, env);
				return npmInstallAsync(agentDir, { env });
			});
			continue;
		}
...
if (process.argv.includes('--fix-bin-permissions-only')) {
	fixAllNpmBinPermissions();
} else {
	main().catch(err => {
		console.error(err);
