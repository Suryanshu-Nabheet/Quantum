...
function packageTask(type: string, platform: string, arch: string, sourceFolderName: string, destinationFolderName: string) {
	...
	const productJsonStream = gulp.src(['product.json'], { base: '.' })
		.pipe(jsonEditor({ commit, date: readISODate(sourceFolderName), version, serverDownloadUrlTemplate: `https://github.com/Suryanshu-Nabheet/Quantum/download/${version}/quantum-reh-\${os}-\${arch}-${version}.tar.gz` }))
			.pipe(es.through(function (file) {
...
['reh', 'reh-web'].forEach(type => {
	...
	const minifyTask = task.define(`minify-vscode-${type}`, task.series(
		...
		optimize.minifyTask(`out-vscode-${type}`, `https://github.com/Suryanshu-Nabheet/Quantum/download/${product.quality}-${commit}/core`)
	));
