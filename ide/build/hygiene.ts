...
export function checkCopilotEnginesVersion(repoRoot: string): string | undefined {
	const copilotPackageJson = path.join(repoRoot, 'extensions/copilot/package.json');
	if (!fs.existsSync(copilotPackageJson)) {
		return undefined;
	}
	const rootPkg = JSON.parse(fs.readFileSync(path.join(repoRoot, 'package.json'), 'utf8'));
	const copilotPkg = JSON.parse(fs.readFileSync(copilotPackageJson, 'utf8'));
...
export function hygiene(some: NodeJS.ReadWriteStream | string[] | undefined, runEslint = true): NodeJS.ReadWriteStream {
	...
	const productJson = es.through(function (file: VinylFile) {
		...
		// Quantum ships Open VSX; block only non-OpenVSX marketplace configs.
		if (product.extensionsGallery) {
			const serviceUrl = String(product.extensionsGallery.serviceUrl ?? '');
			if (!serviceUrl.includes('open-vsx.org')) {
				console.error(`product.json: Contains non-OpenVSX 'extensionsGallery'`);
				errorCount++;
			}
		}
