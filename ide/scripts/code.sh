...
function code() {
	...
	if [[ -z "${VSCODE_SKIP_PRELAUNCH}" ]]; then
		"$ROOT/scripts/node-ts.sh" build/lib/preLaunch.ts
	fi

	# Electron re-download rewrites the .app; always re-apply the branded dock icon after.
	if [[ "$OSTYPE" == "darwin"* ]]; then
		ICNS_SRC="$ROOT/resources/darwin/code.icns"
		ICNS_DST="$ROOT/.build/electron/$NAME.app/Contents/Resources/${EXE_NAME}.icns"
		if [[ -f "$ICNS_SRC" && -d "$(dirname "$ICNS_DST")" ]]; then
			cp -f "$ICNS_SRC" "$ICNS_DST"
			touch "$ROOT/.build/electron/$NAME.app"
		fi
	fi
...
function code-wsl()
{
	...
	# in a wsl shell
	local WIN_EXE="$(node -p "require('./product.json').nameShort + '.exe'")"
	ELECTRON="$ROOT/.build/electron/$WIN_EXE"
	if [ -f "$ELECTRON"  ]; then
		local CWD=$(pwd)
		cd $ROOT
		...
		if [ -n "$WSL_EXT_WLOC" ]; then
			...
			echo "Remote WSL not installed, trying to run Quantum in WSL."
		fi
	fi
}
