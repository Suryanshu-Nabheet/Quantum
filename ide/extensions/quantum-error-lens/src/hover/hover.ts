...
export function createHoverForDiagnostic({
	...
	if (buttonsEnabled) {
		const excludeProblemButton = vscodeUtils.createButtonLinkMarkdown({
			...
			title: 'Exclude problem from Quantum Error Lens by source/code',
		});
		...
		const searchForProblemButton = vscodeUtils.createButtonLinkMarkdown({
			...
			title: 'Open problem in default browser (controlled by `quantumErrorLens.searchForProblemQuery` setting).',
		});
