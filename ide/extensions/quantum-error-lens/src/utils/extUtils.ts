...
export const extUtils = {
	/**
	 * Apply extension settings (`quantumErrorLens.messageTemplate`, `quantumErrorLens.messageMaxChars`, `quantumErrorLens.removeLinebreaks`) to diagnostic message.
	 *
	 ...
	 *
	 * Also, excludes diagnostics according to `quantumErrorLens.excludeSources` & `quantumErrorLens.exclude` settings.
	 *
	 * Also, apply `quantumErrorLens.transmute` setting.
	 *
	...
	shouldExcludeEditor(editor: TextEditor): 'exclude' | 'excludeAndClearDecorations' | 'doNotExclude' {
		...
		if ($config.ignoreDirty && !$state.quantumAutosaveEnabled && editor.document.isDirty) {
			return 'excludeAndClearDecorations';
		}
	...
	/**
	 * Whether or not to align editor message text based on values of `quantumErrorLens.alignMessage` setting.
	 */
	shouldAlign(): boolean {
	...
	shouldShowInlineMessage(): boolean {
		...
		return extensionEnabled && $state.quantumGlobalProblemsEnabled;
	},
