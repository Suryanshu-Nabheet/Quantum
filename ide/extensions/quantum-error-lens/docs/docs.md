...
{
    "key": "ctrl+u",
    "command": "quantumErrorLens.toggleInlineMessage",
    //          ^ Toggles global setting `quantumErrorLens.messageEnabled`
},
```

### If you don't want to go with the hotkey route - it's possible to:

1) Exclude problems (in workspace, by source, by code, by message) [**excludeByMessage**](#quantumErrorLensexcludebymessage), [**excludeBySource**](#quantumErrorLensexcludebysource), [**excludePatterns(Glob)**](#quantumErrorLensexcludepatterns), [**excludeWorkspaces**](#quantumErrorLensexcludeworkspaces)
1) Show fewer decorations (render decoration only on the active line) [**followCursor**](#quantumErrorLensfollowcursor)
1) Delay showing decorations (delay ms / on document save) [**delay**](#quantumErrorLensdelay), [**onSave**](#quantumErrorLensonsave), [**ignoreDirty**](#quantumErrorLensignoredirty)
1) Transform problem message (make it shorter maybe) [**replace**](#quantumErrorLensreplace)
1) Configure other settings or colors to make it less distracting

### 1. Exclude problems:

```js
"quantumErrorLens.excludeByMessage": [
    "Missing semicolon",
    "Newline required at end of file but not found",
    "More than 1 blank line not allowed",
],
// OR
"quantumErrorLens.excludeBySource": [
...
```js
"quantumErrorLens.followCursor": "activeLine",
// OR
"quantumErrorLens.followCursor": "closestProblem",
```

### 3. Delay showing problems:

```js
"quantumErrorLens.delay": 1500,
// OR
"quantumErrorLens.onSave": true,
```
...
### `quantumErrorLens.enabled`

Toggle all decorations and features (background highlighting, inline message, gutter icons, status bar message, code lens).
Extension adds a command to toggle this setting: `quantumErrorLens.toggle` **Quantum Error Lens: Toggle (Enable/Disable) Everything**

### `quantumErrorLens.respectUpstreamEnabled`
...
### `quantumErrorLens.enabledInMergeConflict`

Disable decorations when file has merge conflict symbols `<<<<<<<` or `=======` or `>>>>>>>`.

### `quantumErrorLens.fontFamily`

Change font family of inline message. Not supported natively by VSCode. Non-monospace fonts can usually fit more characters in the same space. May break [quantumErrorLens.alignMessage](#quantumErrorLensalignmessage).
...
### `quantumErrorLens.fontStyleItalic`
...
### `quantumErrorLens.fontSize`

Change font size of inline message. Not supported natively by VSCode. Mostly useful to make message smaller, not bigger. Use negative value to have it relative to the editor's fontSize (e.g. `-3px`). May break [quantumErrorLens.alignMessage](#quantumErrorLensalignmessage).
...
### `quantumErrorLens.padding`

Add space around the inline message. Only visible when [quantumErrorLens.messageBackgroundMode](#quantumErrorLensmessagebackgroundmode) is set to `message`.
...
### `quantumErrorLens.borderRadius`

Round corners for inline message. Only visible when [quantumErrorLens.messageBackgroundMode](#quantumErrorLensmessagebackgroundmode) is set to `message`.
...
### `quantumErrorLens.enabledDiagnosticLevels`
...
### `quantumErrorLens.messageTemplate`
...
- `$severity` - Severity prefix taken from [quantumErrorLens.severityText](#quantumErrorLensseveritytext)
...
```js
"quantumErrorLens.messageTemplate": "[$count] $severity $message $source($code)",
```
...
### `quantumErrorLens.messageMaxChars`
...
### `quantumErrorLens.severityText`

Replaces `$severity` variable in [quantumErrorLens.messageTemplate](#quantumErrorLensmessagetemplate).
...
```js
"quantumErrorLens.messageTemplate": "$severity $message",
"quantumErrorLens.severityText": [
    "Disabled",
    "Note:",
    "ℹ",
...
### `quantumErrorLens.messageEnabled`
...
### `quantumErrorLens.messageBackgroundMode`
...
### `quantumErrorLens.problemRangeDecorationEnabled`
...
"quantumErrorLens.problemRangeDecorationEnabled": true,

"quantumErrorLens.decorations": {
    "errorRange": {
...
### `quantumErrorLens.editorHoverPartsEnabled`
...
### `quantumErrorLens.statusBarIconsEnabled`
...
### `quantumErrorLens.statusBarIconsPriority`

Move status bar icons right/left.

### `quantumErrorLens.statusBarIconsAlignment`

Choose alignment of the status bar icons left/right side of the viewport.

### `quantumErrorLens.statusBarIconsTargetProblems`

Choose what to include in counters for problems (status bar icons).

### `quantumErrorLens.statusBarIconsUseBackground`
...
### `quantumErrorLens.statusBarMessageType`
...
### `quantumErrorLens.statusBarMessagePriority`

Move status bar icons right/left.

### `quantumErrorLens.statusBarMessageAlignment`

Choose alignment of the status bar message left/right side of the viewport.

### `quantumErrorLens.statusBarColorsEnabled`

Uses colors `quantumErrorLens.statusBarErrorForeground`, `quantumErrorLens.statusBarWarningForeground`, `quantumErrorLens.statusBarInfoForeground`, `quantumErrorLens.statusBarHintForeground`.
...
### `quantumErrorLens.statusBarCommand`

Command that executes on click for Status Bar.

### `quantumErrorLens.statusBarMessageTemplate`

Almost the same as [quantumErrorLens.messageTemplate](#quantumErrorLensmessagetemplate) but instead affects status bar message.
...
### `quantumErrorLens.excludePatterns`
...
### `quantumErrorLens.excludeWorkspaces`

Exclude the entire workspace from highlighting problems. Related Command: `errorlens.toggleWorkspace` .

### `quantumErrorLens.disableLineComments`

Used for `quantumErrorLens.disableLine` command that adds a comment disabling linter rule for a line.

To force comment on the same line - add `SAME_LINE` to the message: `"eslint": "// eslint-disable-line $code SAME_LINE"`

### `quantumErrorLens.lintFilePaths`

Specify where to search for linter rule definitions by diagnostic source ([glob](https://code.visualstudio.com/docs/editor/glob-patterns) for local linter files). `node_modules` folder is excluded. Used when running `quantumErrorLens.findLinterRuleDefinition` command.

### `quantumErrorLens.searchForProblemQuery`

Pick query to open in default browser when searching for problem with `quantumErrorLens.searchForProblem` command.

### `quantumErrorLens.selectProblemType`

Which problem to select (closest / active line) when executing `quantumErrorLens.selectProblem` command.
...
### `quantumErrorLens.light`

Override colors when on of the "light" themes is used. Only for colors that are specified in "settings", not in "colors".

### `quantumErrorLens.delay`

Wait this amount (in milliseconds) before showing decorations.

### `quantumErrorLens.delayMode`

- `new` - Old/stale problems should disappear immediately while new problems should respect [quantumErrorLens.delay](#quantumErrorLensdelay)
- `old` - Buggy/Overcomplicated old implementation
- `debounce` - Simply use `debounce()` from `Lodash` library. Old/fixed problems also wait [quantumErrorLens.delay](#quantumErrorLensdelay) ms before being hidden.

### `quantumErrorLens.onSave`

When enabled - updates decorations only on document save (manual, not auto save).

### `quantumErrorLens.onSaveTimeout`

Wait this much (ms) before showing decorations after the document save.

### `quantumErrorLens.onSaveUpdateOnActiveEditorChange`

Update decorations immediately or not when switching focus from one Text Editor to another.

### `quantumErrorLens.enableOnDiffView`
...
### `quantumErrorLens.followCursorMore`

Augments [quantumErrorLens.followCursor](#quantumErrorLensfollowcursor) Adds number of lines to top and bottom when `quantumErrorLens.followCursor` is set to `activeLine`. Adds number of closest problems when `quantumErrorLens.followCursor` is set to `closestProblem`.
...
### `quantumErrorLens.gutterIconsEnabled`
...
### `quantumErrorLens.gutterIconsFollowCursorOverride`

When enabled and [quantumErrorLens.followCursor](#quantumErrorLensfollowcursor) setting is not set to `allLines`, then gutter icons would be rendered for all problems. This setting can overcome that:
...
```js
"quantumErrorLens.gutterIconsEnabled": true,
"quantumErrorLens.followCursor": "activeLine",
"quantumErrorLens.gutterIconsFollowCursorOverride": true,
```
...
"quantumErrorLens.gutterIconsEnabled": true,
"quantumErrorLens.followCursor": "activeLine",
"quantumErrorLens.gutterIconsFollowCursorOverride": false,
```
...
### `quantumErrorLens.gutterIconSize`
...
### `quantumErrorLens.gutterIconSet`
...
### `quantumErrorLens.gutterEmoji`

Control image shown in gutter when [quantumErrorLens.gutterIconSet](#quantumErrorLensguttericonset) is `"emoji"`. Can use other utf-8 symbols like ♞/⚃/⛆/*/▣/◈/... Possible to fit 2 symbols that are not as wide as emoji.
...
### `quantumErrorLens.errorGutterIconPath`
...
```js
"quantumErrorLens.errorGutterIconPath": "C:\\temp\\Stop.png",
"quantumErrorLens.warningGutterIconPath": "C:\\temp\\Warning.png",
"quantumErrorLens.infoGutterIconPath": "C:\\temp\\vscode.png",
"quantumErrorLens.hintGutterIconPath": "C:\\temp\\folder.png",
```
...
### `quantumErrorLens.warningGutterIconPath`

Same as [quantumErrorLens.errorGutterIconPath](#quantumErrorLenserrorguttericonpath)

### `quantumErrorLens.infoGutterIconPath`

Same as [quantumErrorLens.errorGutterIconPath](#quantumErrorLenserrorguttericonpath)

### `quantumErrorLens.hintGutterIconPath`

Same as [quantumErrorLens.errorGutterIconPath](#quantumErrorLenserrorguttericonpath)

### `quantumErrorLens.errorGutterIconColor`
...
```js
"quantumErrorLens.gutterIconSet": "squareRounded",
"quantumErrorLens.errorGutterIconColor": "#6a54e4",
"quantumErrorLens.warningGutterIconColor": "#29d8ff",
"quantumErrorLens.infoGutterIconColor": "#21d439",
"quantumErrorLens.hintGutterIconColor": "#b5a7b0",
```
...
```js
"quantumErrorLens.gutterIconSet": "letter",
"quantumErrorLens.errorGutterIconColor": "#6a54e4",
"quantumErrorLens.warningGutterIconColor": "#29d8ff",
"quantumErrorLens.infoGutterIconColor": "#21d439",
"quantumErrorLens.hintGutterIconColor": "#b5a7b0",
...
### `quantumErrorLens.warningGutterIconColor`

Same as [quantumErrorLens.errorGutterIconPath](#quantumErrorLenserrorguttericoncolor)

### `quantumErrorLens.infoGutterIconColor`

Same as [quantumErrorLens.errorGutterIconPath](#quantumErrorLenserrorguttericoncolor)

### `quantumErrorLens.hintGutterIconColor`

Same as [quantumErrorLens.errorGutterIconPath](#quantumErrorLenserrorguttericoncolor)

### `quantumErrorLens.removeLinebreaks`

Diagnostic message may contain linebreaks, but inline message decorations are ... inline. This replaces linebreaks (multiple in a row too) with the symbol controlled by [quantumErrorLens.replaceLinebreaksSymbol](#quantumErrorLensreplacelinebreakssymbol).

### `quantumErrorLens.replaceLinebreaksSymbol`

When [quantumErrorLens.removeLinebreaks](#quantumErrorLensremovelinebreaks) is enabled => replaced linebreaks `\n` with the new symbol that can be represented inline (`⏎` by default).

### `quantumErrorLens.scrollbarHackEnabled`
...
### `quantumErrorLens.codeLensEnabled`

Show messages as insets between the lines.

<img width="300" src="./img/code_lens_enabled.png">

### `quantumErrorLens.codeLensLength`

Enforce minimum and maximum length of code lens messages.

### `quantumErrorLens.codeLensTemplate`

Almost the same as [quantumErrorLens.messageTemplate](#quantumErrorLensmessagetemplate), but for the Code Lens feature.

### `quantumErrorLens.codeLensOnClick`
...
- `searchForProblem` => Search for problem message in your default browser (runs `quantumErrorLens.searchForProblem`)
...
/* ──── Target only Quantum Error Lens status bar items ─────────────── */
/* "quantumErrorLens.statusBarMessageEnabled": true, */
#usernamehw\.errorlens\.quantumErrorLensMessage {
    font-size: 14px !important;
}

/* "quantumErrorLens.statusBarIconsEnabled": true, */
#usernamehw\.errorlens\.quantumErrorLensError {}
#usernamehw\.errorlens\.quantumErrorLensWarning {}
/* ──────────────────────────────────────────────────────────── */
```
