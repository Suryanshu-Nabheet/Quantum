...
- **For Code OSS (VS Code dev build):** The repo must be built before launching. Run `./scripts/setup.sh` for first-time install/compile/launch, or `./scripts/code.sh` (runs preLaunch automatically), or `VSCODE_SKIP_PRELAUNCH=1 ./scripts/code.sh` / `./scripts/setup.sh --launch-only` if already built.
