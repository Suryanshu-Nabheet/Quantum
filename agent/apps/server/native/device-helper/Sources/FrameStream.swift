...
// `@quantum/contracts` (see HEADER.md).
//
...
final class FrameSocketWriter {
  ...
  private let queue = DispatchQueue(label: "dev.quantum.device-helper.socket")
...
final class FrameStream {
  ...
  private let encodeQueue = DispatchQueue(label: "dev.quantum.device-helper.encode")
