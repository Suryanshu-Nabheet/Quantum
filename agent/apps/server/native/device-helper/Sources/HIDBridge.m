...
typedef union {
  QuantumIndigoTouch touch;
  QuantumIndigoButton button;
...
typedef struct {
  ...
  unsigned int field3;
  QuantumIndigoEvent event;
} QuantumIndigoPayload;

typedef struct {
  QuantumIndigoMachHeader header;
  unsigned int innerSize;
  unsigned char eventType;
  QuantumIndigoPayload payload;
} QuantumIndigoMessage;
...
typedef QuantumIndigoMessage *(*QuantumIndigoButtonFn)(int keyCode, int op, int target);
typedef QuantumIndigoMessage *(*QuantumIndigoKeyboardFn)(uint32_t usage, int op);
typedef QuantumIndigoMessage *(*QuantumIndigoMouseFn)(CGPoint *point0, CGPoint *point1, int target,
                                                    int eventType, BOOL extra);
typedef QuantumIndigoMessage *(*QuantumIndigoArbitraryFn)(int target, uint32_t page, uint32_t usage,
                                                        int op);

BOOL QuantumHardwareButtonFromName(NSString *name, QuantumHardwareButton *outButton) {
  NSDictionary<NSString *, NSNumber *> *map = @{
    @"home": @(QuantumHardwareButtonHome),
    @"lock": @(QuantumHardwareButtonLock),
    @"side": @(QuantumHardwareButtonSide),
    @"siri": @(QuantumHardwareButtonSiri),
    @"volume-up": @(QuantumHardwareButtonVolumeUp),
    @"volume-down": @(QuantumHardwareButtonVolumeDown),
  };
  ...
  if (outButton != NULL) {
    *outButton = (QuantumHardwareButton)found.integerValue;
  }
  return YES;
}

static int QuantumButtonSource(QuantumHardwareButton button) {
  switch (button) {
    case QuantumHardwareButtonHome: return kButtonSourceHome;
    case QuantumHardwareButtonLock: return kButtonSourceLock;
    case QuantumHardwareButtonSide: return kButtonSourceSide;
    case QuantumHardwareButtonSiri: return kButtonSourceSiri;
    case QuantumHardwareButtonVolumeUp:
    case QuantumHardwareButtonVolumeDown: return -1;
  }
...
static uint32_t QuantumConsumerUsage(QuantumHardwareButton button) {
  switch (button) {
    case QuantumHardwareButtonVolumeUp: return kHIDUsageVolumeIncrement;
    case QuantumHardwareButtonVolumeDown: return kHIDUsageVolumeDecrement;
    default: return 0;
  }
}
...
static uint32_t QuantumUsageForCharacter(unichar c, BOOL *outShift) {
  BOOL shift = NO;
  uint32_t usage = 0;
...
@implementation QuantumHIDBridge {
  id _client;
  dispatch_queue_t _sendQueue;
  QuantumIndigoButtonFn _buttonFn;
  QuantumIndigoKeyboardFn _keyboardFn;
  QuantumIndigoMouseFn _mouseFn;
  QuantumIndigoArbitraryFn _arbitraryFn;
...
- (BOOL)attachToDevice:(id)device error:(NSError **)error {
  NSString *developerDir = NSProcessInfo.processInfo.environment[@"QUANTUM_DEVELOPER_DIR"];
  ...
  if (kit == NULL) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.quantum.device-helper.hid"
                                   code:1
  ...
  _arbitraryFn = (QuantumIndigoArbitraryFn)dlsym(kit, "IndigoHIDMessageForHIDArbitrary");
  _buttonFn = (QuantumIndigoButtonFn)dlsym(kit, "IndigoHIDMessageForButton");
  _keyboardFn = (QuantumIndigoKeyboardFn)dlsym(kit, "IndigoHIDMessageForKeyboardArbitrary");
  _mouseFn = (QuantumIndigoMouseFn)dlsym(kit, "IndigoHIDMessageForMouseNSEvent");
  ...
  if (client == nil) {
    if (error) {
      *error = initError ?: [NSError errorWithDomain:@"dev.quantum.device-helper.hid"
                                                code:4
  ...
  _client = client;
  _sendQueue = dispatch_queue_create("dev.quantum.device-helper.hid", DISPATCH_QUEUE_SERIAL);
  return YES;
}

- (void)sendMessage:(QuantumIndigoMessage *)message {
  ...
  ((void (*)(id, SEL, QuantumIndigoMessage *, BOOL, dispatch_queue_t, void (^)(NSError *)))objc_msgSend)(
...
- (void)sendTouchAtX:(double)x y:(double)y down:(BOOL)down {
  ...
  QuantumIndigoMessage *seed = _mouseFn(&point, NULL, kTouchTarget, down ? kButtonOpDown : kButtonOpUp, NO);
  if (seed == NULL) {
    [self noteUndelivered];
    return;
  }
  ...
  size_t stride = sizeof(QuantumIndigoPayload);
  QuantumIndigoMessage *message = calloc(1, sizeof(QuantumIndigoMessage) + stride);
  if (message == NULL) {
    free(seed);
    [self noteUndelivered];
    return;
  ...
  memcpy(&message->payload.event.touch, &seed->payload.event.touch, sizeof(QuantumIndigoTouch));
  ...
  QuantumIndigoPayload *secondPayload = (QuantumIndigoPayload *)second;
...
- (void)sendKeyUsage:(uint32_t)usage down:(BOOL)down {
  ...
  QuantumIndigoMessage *message = _keyboardFn(usage, down ? kButtonOpDown : kButtonOpUp);
  [self sendMessage:message];
}
...
- (NSInteger)typeText:(NSString *)text {
  ...
  for (NSUInteger index = 0; index < length; index++) {
    ...
    BOOL needsShift = NO;
    uint32_t usage = QuantumUsageForCharacter(character, &needsShift);
    if (usage == 0) {
      skipped++;
      continue;
    }
...
- (void)sendButton:(QuantumHardwareButton)button down:(BOOL)down {
  int op = down ? kButtonOpDown : kButtonOpUp;
  uint32_t consumerUsage = QuantumConsumerUsage(button);
  ...
  [self sendMessage:_buttonFn(QuantumButtonSource(button), op, kButtonTargetHardware)];
}

- (void)tapButton:(QuantumHardwareButton)button {
