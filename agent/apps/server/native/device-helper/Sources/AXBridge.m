...
id QuantumEmptyTranslatorResponse(void);
...
static id QuantumMsgSend0(id target, NSString *selectorName) {
...
static NSString *QuantumStringAttribute(id element, NSString *selectorName) {
  id value = QuantumMsgSend0(element, selectorName);
  return [value isKindOfClass:NSString.class] ? value : nil;
}

static BOOL QuantumBoolAttribute(id element, NSString *selectorName) {
  SEL selector = NSSelectorFromString(selectorName);
...
static id QuantumJSONValue(id value) {
  if (value == nil) {
    return NSNull.null;
  }
...
@implementation QuantumAXBridge {
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _callbackQueue = dispatch_queue_create("dev.quantum.device-helper.ax", DISPATCH_QUEUE_CONCURRENT);
    _requestTimeout = 5.0;
  }
  return self;
}
...
- (id)accessibilityTranslationDelegateBridgeCallbackWithToken:(NSString *)token {
  ...
  id (^callback)(id) = ^id(id request) {
    if (device == nil) {
      return QuantumEmptyTranslatorResponse();
    }
    ...
    return response ?: QuantumEmptyTranslatorResponse();
  };
...
- (nullable NSDictionary *)frontmostTreeWithMaxDepth:(NSInteger)maxDepth error:(NSError **)error {
  if (_translator == nil) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.quantum.device-helper.ax"
                                   code:1
...
- (NSDictionary *)serializeElement:(id)element depth:(NSInteger)depth maxDepth:(NSInteger)maxDepth {
  ...
  NSString *role = QuantumStringAttribute(element, @"accessibilityRole");
  ...
  NSString *label = QuantumStringAttribute(element, @"accessibilityLabel");
  if (label.length > 0) {
    node[@"label"] = label;
  }
  id value = QuantumJSONValue(QuantumMsgSend0(element, @"accessibilityValue"));
  if (value != NSNull.null) {
    node[@"value"] = value;
  }
  NSString *identifier = QuantumStringAttribute(element, @"accessibilityIdentifier");
  if (identifier.length > 0) {
    node[@"identifier"] = identifier;
  }
  NSString *title = QuantumStringAttribute(element, @"accessibilityTitle");
  if (title.length > 0) {
    node[@"title"] = title;
  }
  ...
  NSString *subrole = QuantumStringAttribute(element, @"accessibilitySubrole");
  ...
  node[@"enabled"] = @(QuantumBoolAttribute(element, @"isAccessibilityEnabled"));
  ...
  id rawChildren = QuantumMsgSend0(element, @"accessibilityChildren");
