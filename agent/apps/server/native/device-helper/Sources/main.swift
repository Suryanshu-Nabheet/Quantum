// quantum-device-helper — the native side of Quantum's Device Pane.
//
...
final class HelperSession {
  ...
  private(set) var hid: QuantumHIDBridge?
  private(set) var accessibility: QuantumAXBridge?
  ...
  func attach(udid: String) throws -> [String: Any] {
    ...
    let hidBridge = QuantumHIDBridge()
  ...
  func requireHID() throws -> QuantumHIDBridge {
  ...
  func requireAccessibility() throws -> QuantumAXBridge {
...
///
/// Every injection path in QuantumHIDBridge returns silently when it has no
...
private func withHIDDelivery(_ hid: QuantumHIDBridge, _ body: () -> Void) throws {
...
func handle(method: String, params: Params, session: HelperSession) throws -> Any {
  ...
  case "button":
    ...
    var button = QuantumHardwareButton.home
    guard QuantumHardwareButtonFromName(name, &button) else {
      throw RPCError(
        .invalidParams,
...
let requestQueue = DispatchQueue(label: "dev.quantum.device-helper.rpc")
