/**
 * Device pane end-to-end verification against a live Quantum server.
 *
...
import {
  ...
  decodeDeviceFrame,
} from "@quantum/shared/deviceFrame";
import {
  DEVICE_WS_METHODS,
...
/**
 * Reboot the simulator behind Quantum's back, the way `simctl` from a terminal,
...
async function screenshotViaSimctl(udid: string): Promise<Buffer> {
  const path = join(tmpdir(), `quantum-device-e2e-${randomUUID()}.png`);
  try {
...
describeE2e("device pane end-to-end", () => {
  ...
  it("boots the target simulator", async () => {
    ...
    expect(booted?.bootSource).toBe("quantum");
  }, 180_000);
  ...
  it("keeps input working after the simulator is rebooted outside Quantum", async () => {
    if (!target) throw new Error("no target device");
