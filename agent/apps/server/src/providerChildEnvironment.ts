...
// Purpose: Builds provider child environments without Quantum control-plane authority.
...
const isTestHarnessKey = (key: string, env: NodeJS.ProcessEnv): boolean =>
  Boolean(env.VITEST) && (key.startsWith("QUANTUM_FAKE_") || key.startsWith("QUANTUM_ACP_"));

export function buildProviderChildEnvironment(input: {
  readonly provider: ProviderChildKind;
  readonly baseEnv?: NodeJS.ProcessEnv;
  readonly inheritedQuantumKeys?: ReadonlyArray<string>;
  ...
  const allowedQuantumKeys = new Set(input.inheritedQuantumKeys ?? []);
