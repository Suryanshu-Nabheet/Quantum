 ...
 * directory so both land in the same place under a custom QUANTUM_HOME.
 *
...
function defaultBootOwnershipPath(): string {
  const baseDir = process.env.QUANTUM_HOME?.trim() || path.join(homedir(), ".quantum");
          ...
          if (reclaimed.length > 0) {
            console.info(
              `[device] shut down ${reclaimed.length} simulator(s) left booted by a previous ` +
                `Quantum run: ${reclaimed.join(", ")}`,
            );
          }
        });
      }

      // App quit shuts down every simulator Quantum booted and leaves the
