 ...
 */
import type { DeviceUiNode, DeviceUiPoint } from "@quantum/contracts";
    ...
    if (hasPoint) {
      throw new DeviceUiTargetError(
        "A tap takes either label (with optional role) or x and y, not both. " +
          "Pass label alone to let Quantum resolve the element's own tap point.",
      );
    }
  ...
  if (!hasPoint) {
    throw new DeviceUiTargetError(
      "A tap needs either label (with optional role) or both x and y. " +
        "Prefer label: Quantum then resolves the element's own tap point from the accessibility tree.",
    );
  }
