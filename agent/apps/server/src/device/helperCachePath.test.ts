...
describe("helper cache path agreement", () => {
  it("builds into the directory the shared key names", async () => {
    ...
    const backend = new IosSimulatorBackend({
      platform: "darwin",
      helperSourceDir: "/tmp/quantum-helper-src",
      helperCacheRoot: "/tmp/quantum-helper-cache",
    ...
    expect(build?.args[1]).toBe(join("/tmp/quantum-helper-cache", expectedKey));
  });
  ...
  it("names the binary the build script produces", () => {
    expect(DEVICE_HELPER_BINARY_NAME).toBe("quantum-device-helper");
  });
});
