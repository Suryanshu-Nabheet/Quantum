...
} from "@quantum/shared/deviceHelperCache";
...
export class IosSimulatorBackend implements DeviceBackend {
  ...
  async availability(): Promise<DeviceAvailability> {
    ...
    steps.push({
      ...
      label: "Build the Quantum device helper",
      done: helperBuilt,
  ...
  async screenshot(
    ...
    const directory = await mkdtemp(path.join(tmpdir(), "quantum-device-"));
   ...
   * Quantum, SimulatorKit dropped the connection) the injection silently
  ...
  private async attachedHelper(
    ...
    try {
      ...
      // A device can also be rebooted outside Quantum (Simulator.app, or simctl
