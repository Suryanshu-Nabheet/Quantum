 ...
 * - The Quantum boot cap (`DEVICE_QUANTUM_BOOT_LIMIT`). Boot past the cap is
...
/** How long a Quantum-booted device stays up with no thread attached. */
...
export interface DeviceManagerOptions {
  ...
  /** Remembers Quantum's boots across a crash; defaults to remembering nothing. */
...
export class DeviceManager {
  ...
  private readonly quantumBooted = new Set<string>();
  ...
  constructor(options: DeviceManagerOptions) {
    ...
    this.bootLimit = options.bootLimit ?? DEVICE_QUANTUM_BOOT_LIMIT;
  ...
  private async recordBootOwnership(): Promise<void> {
    await this.bootOwnership.write([...this.quantumBooted]).catch(() => undefined);
  }
  ...
  async quantumBootedDevices(): Promise<readonly DeviceDescriptor[]> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    this.reconcileQuantumBooted(devices);
    return devices
      .filter((device) => this.quantumBooted.has(device.udid))
   ...
   *
   * The set is Quantum's own bookkeeping, but the simulators are not Quantum's to
   ...
   */
  private reconcileQuantumBooted(devices: readonly DeviceDescriptor[]): void {
    const running = new Set(
      devices
        .filter((device) => device.state === "booted" || device.state === "booting")
        .map((device) => device.udid),
    );
    for (const udid of this.quantumBooted) {
      if (running.has(udid)) continue;
      this.quantumBooted.delete(udid);
  ...
  async boot(udid: string): Promise<DeviceBootResult> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    // Devices that stopped without Quantum doing it still held their slots, so
    ...
    // Quantum from accumulating simulators, not to limit what the user watches.
    if (known?.state === "booted") {
      return { kind: "booted", device: this.describe(known) };
    }
    if (this.quantumBooted.size >= this.bootLimit) {
      return {
        kind: "boot-limit-reached",
        limit: this.bootLimit,
        quantumBooted: await this.quantumBootedDevices(),
      };
    }
    ...
    // exists to stop Quantum accumulating multi-gigabyte simulators would be
    // exceeded by however many requests arrived inside that window.
    this.quantumBooted.add(udid);
    ...
    return { kind: "booted", device: { ...device, bootSource: "quantum" } };
  }
  ...
  /**
   * App quit: shut down everything Quantum booted, leave the user's devices
  ...
  async dispose(): Promise<void> {
    ...
    const booted = Array.from(this.quantumBooted);
  ...
  private describe(device: DeviceDescriptor): DeviceDescriptor {
    const bootSource = this.quantumBooted.has(device.udid) ? "quantum" : device.bootSource;
   ...
   * the device down or start the idle countdown if Quantum booted it.
   *
   * A switch shuts down immediately rather than waiting out the idle window.
   * The cap is three Quantum-booted devices, and each one costs a couple of GB
  ...
  private async releaseDevice(
    ...
    if (!this.quantumBooted.has(udid)) return;
    if (reason === "switched") {
      this.clearIdleTimer(udid);
  ...
  private async shutdownIfStillIdle(udid: string): Promise<void> {
    ...
    if (this.isAttachedAnywhere(udid) || !this.quantumBooted.has(udid)) return;
    await this.shutdown(udid).catch(() => undefined);
  }
