...
function makeBackend() {
  ...
  const backend = new IosSimulatorBackend({
    platform: "darwin",
    helperCacheRoot: "/tmp/quantum-device-test-cache",
  ...
  Object.defineProperty(backend, "compileHelperIfNeeded", {
    value: async () => "/tmp/quantum-device-test-cache/quantum-device-helper",
  });
...
describe("simulator reboot", () => {
  it("re-attaches after a Quantum-driven shutdown and reboot", async () => {
  ...
  it("recovers when the device was rebooted outside Quantum", async () => {
    const { backend, helper } = makeBackend();
    await backend.tap(DEVICE, 10, 10);
