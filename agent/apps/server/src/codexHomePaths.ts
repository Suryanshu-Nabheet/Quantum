...
export const QUANTUM_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
...
export function resolveQuantumCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.QUANTUM_HOME?.trim();
  const overlayRoot = runtimeHome || path.join(path.dirname(sourceHomePath), ".quantum", "runtime");
  return path.join(overlayRoot, QUANTUM_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. Quantum keeps its generated config isolated from the user's
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  const overlay = resolveQuantumCodexHomeOverlayPath(env, source);
