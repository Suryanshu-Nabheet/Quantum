...
/**
 * A tracker identity that a *released* Quantum build wrote for a migration whose
 ...
 * does the migration Quantum registers at `historicalId` *today* still need to
 ...
 * once the imported tracker's high-water mark reaches Quantum's latest ID,
 * every Quantum migration is skipped silently and startup crashes on missing
 ...
 *
 * Instead, compare the recorded (id, name) pairs against Quantum's lineage and
 ...
 *
 * Divergences caused by Quantum renumbering one of its *own* released
...
export const reconcileMigrationLineage = Effect.gen(function* () {
  ...
  // A released Quantum build may have recorded a migration under an ID it no
  ...
  if (aliasRepairs.length > 0) {
    yield* Effect.logInfo(
      "Migration tracker records a renumbered Quantum migration; repairing tracker metadata in place",
    ).pipe(
      Effect.annotateLogs({
  ...
  if (diverged === undefined) {
    ...
    // a newer Quantum build. Continuing would expose it to stale writable
  ...
  yield* Effect.logWarning(
    "Migration tracker diverges from the Quantum lineage (legacy import); re-running migrations from the divergence point",
