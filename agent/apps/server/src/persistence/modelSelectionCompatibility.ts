...
import { MODEL_OPTIONS_BY_PROVIDER } from "@quantum/contracts";
...
// Imported instance ids may be runtime names rather than Quantum provider literals.
...
export function normalizePersistedModelSelection(input: unknown): unknown {
  ...
  // Newer Quantum writes provider-less selections as { instanceId, model } and
  // option rows as [{ id, value }]; Quantum stores canonical provider/options objects.
