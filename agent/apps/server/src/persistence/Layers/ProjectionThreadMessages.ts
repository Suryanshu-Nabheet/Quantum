...
import { MessageId } from "@quantum/contracts";
...
import {
  ...
  AppendStreamingThreadMessageTextInput,
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const appendStreamingThreadMessageTextRow = SqlSchema.findOneOption({
    Request: AppendStreamingThreadMessageTextInput,
    Result: Schema.Struct({ messageId: MessageId }),
    execute: (event) =>
      sql<{ readonly messageId: string }>`
        UPDATE projection_thread_messages
        SET
          turn_id = COALESCE(turn_id, ${event.turnId ?? null}),
          role = ${event.role},
          text = text || ${event.text},
          dispatch_mode = COALESCE(${event.dispatchMode ?? null}, dispatch_mode),
          dispatch_origin = COALESCE(${event.dispatchOrigin ?? null}, dispatch_origin),
          is_streaming = 1,
          source = COALESCE(${event.source ?? null}, source),
          sequence = COALESCE(sequence, ${event.sequence ?? null}),
          updated_at = COALESCE(${event.updatedAt ?? null}, CURRENT_TIMESTAMP)
        WHERE thread_id = ${event.threadId} AND message_id = ${event.messageId}
  ...
  const appendStreamingText: ProjectionThreadMessageRepositoryShape["appendStreamingText"] = (
    ...
    appendStreamingThreadMessageTextRow(input).pipe(
      Effect.mapError(
