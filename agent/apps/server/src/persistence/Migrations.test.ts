...
layer("reconcileMigrationLineage", (it) => {
  ...
  // mark is at or beyond Quantum's latest migration ID. The migrator's max-ID
  // gate then skips every Quantum migration — including the #032 self-heal —
  // and startup crashes on the missing env_mode column.
  it.effect("re-runs skipped migrations when an imported tracker outruns Quantum's latest ID", () =>
    Effect.gen(function* () {
      ...
      // Record a foreign lineage from 17 through past Quantum's latest ID.
      const latestQuantumId = Math.max(...migrationEntries.map(([id]) => id));
      for (let id = 17; id <= latestQuantumId + 3; id++) {
        yield* sql`
      ...
      // The tracker now mirrors the Quantum lineage exactly; foreign rows are gone.
  ...
  it.effect("refuses writable migration startup for a newer Quantum schema", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        INSERT INTO effect_sql_migrations (migration_id, name)
        VALUES (${futureId}, 'FutureQuantumMigration')
      `;
