...
import type { ProviderRuntimeEvent } from "@quantum/contracts";
...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  afterEach(() => {
    if (previousQuantumHome === undefined) delete process.env.QUANTUM_HOME;
    else process.env.QUANTUM_HOME = previousQuantumHome;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.QUANTUM_HOME = "/quantum-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/quantum-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.quantum/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.quantum/runtime/codex-home-overlay";
