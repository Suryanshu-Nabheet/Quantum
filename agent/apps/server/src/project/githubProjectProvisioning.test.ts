...
import { CommandId, ProjectId, type GitHubProjectProvisionInput } from "@quantum/contracts";
  ...
  it("uses authenticated GitHub CLI cloning without forcing SSH or HTTPS", async () => {
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        const parent = yield* fileSystem.makeTempDirectoryScoped({ prefix: "quantum-provision-" });
    ...
    expect(result.ghCalls).toEqual([
      [
        ...
        "--no-upstream",
        "openai/codex",
        expect.stringContaining(".quantum-clone-"),
        "--",
        "--progress",
      ],
    ]);
