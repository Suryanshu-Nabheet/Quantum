...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Quantum refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/quantum\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/quantum/", "refs/historical/");
