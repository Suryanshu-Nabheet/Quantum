...
// Purpose: Builds the exact environment used when Quantum launches Codex subprocesses.
...
import { readActiveCodexProviderEnvKey } from "@quantum/shared/codexConfig";
import {
  readEnvironmentFromLoginShell,
  resolveLoginShell,
  type ShellEnvironmentReader,
} from "@quantum/shared/shell";

import { resolveBaseCodexHomePath, resolveQuantumCodexHomeOverlayPath } from "./codexHomePaths.ts";
import { buildProviderChildEnvironment } from "./providerChildEnvironment.ts";

const CODEX_PROCESS_SHELL_ENV_NAMES = ["PATH", "SSH_AUTH_SOCK"] as const;
const CODEX_OVERLAY_SHARED_STATE_FILES = new Set(["auth.json"]);
const QUANTUM_CONFIG_SUPPRESSIONS_FILE = "quantum-config-suppressions-v1.json";
const QUANTUM_MANAGED_MCP_TABLE_HEADER = "[mcp_servers.quantum]";
export const QUANTUM_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS = [
...
export async function readQuantumConfigSuppressions(
  markerPath: string,
...
async function writeQuantumConfigSuppressions(
  markerPath: string,
  sectionHeaders: readonly string[],
): Promise<void> {
...
export const QUANTUM_MANAGED_CODEX_CONFIG_BEGIN = "# >>> quantum managed config >>>";
export const QUANTUM_MANAGED_CODEX_CONFIG_END = "# <<< quantum managed config <<<";

export function extractManagedCodexConfigSection(config: string): string | undefined {
  const begin = config.indexOf(QUANTUM_MANAGED_CODEX_CONFIG_BEGIN);
  if (begin === -1) {
    return undefined;
  }
  const contentStart = begin + QUANTUM_MANAGED_CODEX_CONFIG_BEGIN.length;
  const end = config.indexOf(QUANTUM_MANAGED_CODEX_CONFIG_END, contentStart);
  if (end === -1) {
    return undefined;
  }
...
function appendManagedCodexConfigSection(config: string, section: string): string {
  ...
  const managedMcpTableName = normalizeTomlTableHeaderName(QUANTUM_MANAGED_MCP_TABLE_HEADER);
  const tables: string[] = [];
  ...
  for (const table of splitTomlTables(section.trim())) {
    ...
    if (normalizeTomlTableHeaderName(header) === managedMcpTableName) {
      // The session-scoped gateway entry is authoritative inside Quantum's
      // overlay. The user's source config remains untouched.
      overlayConfig = removeTomlTableNamespace(overlayConfig, QUANTUM_MANAGED_MCP_TABLE_HEADER);
      tables.push(table);
      continue;
    }
...
async function prepareQuantumCodexHomeOverlayUnlocked(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
  readonly appendConfigToml?: string;
}): Promise<string | undefined> {
  const sourceHomePath = resolveBaseCodexHomePath(input.env, input.homePath);
  const overlayHomePath = resolveQuantumCodexHomeOverlayPath(input.env, sourceHomePath);
  ...
  const suppressionMarkerPath = path.join(overlayHomePath, QUANTUM_CONFIG_SUPPRESSIONS_FILE);
  const suppressedSections = [
    ...new Set([
      ...
      ...(await readQuantumConfigSuppressions(suppressionMarkerPath)),
    ]),
  ...
  await writeQuantumConfigSuppressions(suppressionMarkerPath, suppressedSections);

  return overlayHomePath;
}

async function prepareQuantumCodexHomeOverlay(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
  readonly appendConfigToml?: string;
}): Promise<string | undefined> {
  ...
  return serializeCodexOverlayPreparation(overlayHomePath, () =>
    prepareQuantumCodexHomeOverlayUnlocked(input),
  );
}
...
export async function buildCodexProcessEnv(
  ...
  const overlayHomePath = await prepareQuantumCodexHomeOverlay({
    env: baseEnv,
