...
async function fetchLocalPageTitleResponse(
  ...
  const response = await globalThis.fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "QuantumLocalServerMonitor/1.0",
    },
  });
...
function toServerProcess(
  ...
  const server: ServerLocalServerProcess = {
    ...
    addresses,
    isStoppable,
    ...(isStoppable ? {} : { stopDisabledReason: "Quantum cannot signal this process." }),
  };
...
export async function stopLocalServer(
  ...
  if (!target.isStoppable) {
    return {
      pid: input.pid,
      stopped: false,
      message: target.stopDisabledReason ?? "Quantum cannot stop this process.",
    };
  }
