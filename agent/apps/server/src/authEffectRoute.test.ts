...
import { AuthSessionId } from "@quantum/contracts";
import {
...
function makeServerAuth(sideEffects: { count: number }): ServerAuthShape {
  ...
  const descriptor = {
    ...
    sessionCookieName: "quantum_session",
  };
  ...
  return {
    ...
    authenticateHttpRequest: (request) => {
      ...
      const cookie = request.cookies.quantum_session === "cookie-token";
      if (!bearer && !cookie) {
    ...
    issueStartupPairingUrl: () =>
      Effect.succeed("https://quantum.example.test/pair#token=PAIRINGTOKEN"),
  ...
  return {
    ...
    headers: {
      ...
      ...(input.credential === "bearer"
        ...
        : { Cookie: "quantum_session=cookie-token" }),
...
describe("authEffectRouteLayer", () => {
  ...
  it("rejects every cookie-authenticated mutation without a trusted origin", async () => {
    ...
    const config = {
      host: "0.0.0.0",
      publicUrl: new URL("https://quantum.example.test/"),
  ...
  it("logs out either role and clears the exact cookie with secure public-mode attributes", async () => {
    ...
    await withAuthEffectServer(config, makeServerAuth(sideEffects), async (serverOrigin) => {
      const response = await fetch(
        ...
        mutationRequest({
          origin: "https://quantum.example.test",
          credential: "cookie",
        }),
      );
      ...
      expect(cookie).toContain("quantum_session=");
...
describe("binaryUploadEffectRouteLayer", () => {
  it("allows credentialed Canary attachment upload preflights", async () => {
    const config = {
      host: "127.0.0.1",
      attachmentsDir: fs.mkdtempSync(path.join(os.tmpdir(), "quantum-upload-cors-")),
    } as ServerConfigShape;
    try {
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          expect(response.headers.get("access-control-allow-origin")).toBe("quantum-canary://app");
  ...
  it("rejects ambient cookie uploads without an origin and accepts explicit bearer auth", async () => {
    const attachmentsDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantum-upload-route-"));
    const config = {
      host: "0.0.0.0",
      publicUrl: new URL("https://quantum.example.test/"),
      attachmentsDir,
      ...
      await withAuthEffectServer(
        ...
        async (serverOrigin) => {
          ...
          const cookieResponse = await fetch(url, {
            method: "POST",
            headers: { Cookie: "quantum_session=cookie-token" },
