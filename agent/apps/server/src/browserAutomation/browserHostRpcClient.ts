...
import type { BrowserToolName, ProviderKind, ThreadId } from "@quantum/contracts";
...
function assertCompatibleHostInfo(value: unknown, expectedSessionId: string): void {
  ...
  if (
    protocolVersion !== 1 ||
    (info?.type !== undefined && info.type !== "quantum-browser-host") ||
...
export function resolveBrowserHostPipePath(env: NodeJS.ProcessEnv = process.env): string | null {
  const configured =
    env.QUANTUM_BROWSER_HOST_PIPE_PATH?.trim() || env.QUANTUM_BROWSER_USE_PIPE_PATH?.trim();
...
export function resolveBrowserHostCapability(env: NodeJS.ProcessEnv = process.env): string | null {
  const direct = env.QUANTUM_BROWSER_HOST_CAPABILITY?.trim();
  if (direct && Buffer.byteLength(direct, "utf8") >= 32) return direct;

  const rawFd = env.QUANTUM_BROWSER_HOST_CAPABILITY_FD?.trim();
