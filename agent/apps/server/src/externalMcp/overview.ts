import type { ExternalMcpCapability } from "@quantum/contracts";
...
export function buildExternalMcpOverviewNextSteps(
  ...
  return [
    ...
    ...(capabilities.has("tasks:create") ? ["Create work with quantum_create_task."] : []),
    ...(capabilities.has("tasks:wait")
      ? ["Follow permitted work with quantum_wait_for_task."]
      : []),
    ...(capabilities.has("tasks:read")
      ? ["Read permitted task results with quantum_read_task."]
      : []),
  ];
}
