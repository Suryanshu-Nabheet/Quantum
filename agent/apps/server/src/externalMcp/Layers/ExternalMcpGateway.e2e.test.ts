...
import { MessageId, ProjectId, TurnId } from "@quantum/contracts";
...
describe("external MCP gateway stdio flow", () => {
  it("pairs, filters tools, creates one safe task, waits, reads, and audits without prompt leakage", async () => {
    const baseDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantum-external-e2e-"));
      ...
      Effect.gen(function* () {
        ...
        stdin.write(
          `${JSON.stringify({
            ...
            id: 4,
            method: "tools/call",
            params: { name: "quantum_read_task", arguments: { threadId } },
          })}\n`,
        );
        stdin.end();
        ...
        const overview = yield* gateway.handlePost({
          ...
          body: {
            ...
            id: "overview",
            method: "tools/call",
            params: { name: "quantum_overview", arguments: {} },
          },
        });
        ...
        expect(overviewPayload.nextSteps).toEqual([
          "Call quantum_capabilities with a projectId to list the exact provider/model targets available to this integration.",
        ]);
        ...
        const successfulDespiteAuditFailure = yield* gateway.handlePost({
          ...
          body: {
            ...
            method: "tools/call",
            params: { name: "quantum_list_allowed_projects", arguments: {} },
          },
        });
