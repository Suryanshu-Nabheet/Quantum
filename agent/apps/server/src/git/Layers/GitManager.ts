...
} from "@quantum/shared/git";
import { parseGitHubRepositoryNameWithOwnerFromRemoteUrl } from "@quantum/shared/githubRepository";
import { summarizeUnifiedPatchTotals } from "@quantum/shared/unifiedPatchStats";
import { resolveWorktreeHandoffIntent } from "@quantum/shared/worktreeHandoff";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `quantum/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.gen(function* () {
  ...
  const runPrStep = (
    ...
    Effect.gen(function* () {
      ...
      const bodyFile = path.join(tempDir, `quantum-pr-body-${process.pid}-${randomUUID()}.md`);
      yield* fileSystem
  ...
  const handoffThread: GitManagerShape["handoffThread"] = Effect.fnUntraced(function* (input) {
    ...
    if (input.targetMode === "local") {
      ...
      const preservedLocalStash = yield* stashWorkingTree(
        input.cwd,
        `quantum preserve local handoff ${randomUUID()}`,
      );
      const sourceStash = yield* stashWorkingTree(
        input.worktreePath,
        `quantum handoff to local ${randomUUID()}`,
      );
    ...
    const sourceStash = yield* stashWorkingTree(
      input.cwd,
      `quantum handoff to worktree ${randomUUID()}`,
    );
