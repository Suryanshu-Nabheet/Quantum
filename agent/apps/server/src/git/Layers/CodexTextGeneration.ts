...
import { sanitizeGeneratedThreadTitle } from "@quantum/shared/chatThreads";
import { resolveCodexHome } from "@quantum/shared/codexConfig";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@quantum/shared/git";
...
const makeCodexTextGeneration = Effect.gen(function* () {
  ...
  const writeTempFile = (
    ...
    const filePath = path.join(tempDir, `quantum-${prefix}-${process.pid}-${randomUUID()}.tmp`);
  ...
  const prepareIsolatedCodexHome = (
    ...
    Effect.gen(function* () {
      ...
      const isolatedHomePath = path.join(
        tempDir,
        `quantum-codex-home-${process.pid}-${randomUUID()}`,
      );
