...
import { MAX_CHAT_THREAD_TITLE_WORDS } from "@quantum/shared/chatThreads";
...
export function buildThreadRecapPrompt(input: {
  ...
  return {
    prompt: [
      "You are writing a compact live recap for Quantum's chat side panel.",
...
export function buildAutomationIntentPrompt(input: {
  ...
  return {
    prompt: [
      "You extract structured Quantum automation creation intents.",
      ...
      "- Quantum automations run a saved prompt on a schedule.",
...
export function buildAutomationCompletionEvaluationPrompt(input: {
  ...
  return {
    prompt: [
      "You evaluate whether a completed Quantum heartbeat automation should stop.",
