...
describe("linkOrCopyCodexOverlayEntry", () => {
  it("copies auth.json when symlink creation is unavailable", async () => {
    ...
    await linkOrCopyCodexOverlayEntry(
      {
        ...
        targetPath: "C:\\Users\\test\\.quantum\\codex-home-overlay\\auth.json",
        type: "file",
      },
      { symlink, copyFile },
    );
    ...
    expect(symlink).toHaveBeenCalledWith(
      ...
      "C:\\Users\\test\\.quantum\\codex-home-overlay\\auth.json",
      "file",
    );
      ...
      linkOrCopyCodexOverlayEntry(
        {
          ...
          targetPath: "C:\\Users\\test\\.quantum\\codex-home-overlay\\sessions",
          type: "dir",
        },
...
describe("buildCodexProcessEnv", () => {
  it("replaces a user-defined Quantum MCP table only inside the session overlay", async () => {
    const sourceHome = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-source-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-runtime-"));
    const sourceConfig = [
      'model = "gpt-5.5"',
      "",
      "[mcp_servers.quantum]",
      ...
      "",
      "[mcp_servers.quantum.headers]",
      'Authorization = "stale-inline-secret"',
      "",
      "[mcp_servers.quantum.env]",
      'STALE_GATEWAY_TOKEN = "stale-inline-secret"',
      "",
      "[mcp_servers.quantum-other]",
      'url = "http://127.0.0.1:2111/quantum-other"',
      "",
    ...
    const managedConfig = [
      ...
      'bearer_token_env_var = "QUANTUM_AGENT_GATEWAY_TOKEN"',
      "",
      "[shell_environment_policy]",
      'exclude = ["QUANTUM_AGENT_GATEWAY_TOKEN"]',
    ].join("\n");
    ...
    try {
      ...
      if (!overlayHome) {
        throw new Error("Expected a Quantum Codex home overlay.");
      }
      const overlayConfig = readFileSync(path.join(overlayHome, "config.toml"), "utf8");

      expect(overlayConfig.match(/^\[mcp_servers\.quantum\]$/gm)).toHaveLength(1);
      expect(overlayConfig).toContain('url = "http://127.0.0.1:3773/mcp"');
      expect(overlayConfig).toContain('bearer_token_env_var = "QUANTUM_AGENT_GATEWAY_TOKEN"');
      ...
      expect(overlayConfig).not.toContain("[mcp_servers.quantum.headers]");
      expect(overlayConfig).not.toContain("[mcp_servers.quantum.env]");
      ...
      expect(overlayConfig).toContain('exclude = ["QUANTUM_AGENT_GATEWAY_TOKEN", "USER_SECRET"]');
