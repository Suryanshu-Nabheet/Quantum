...
describe("remote access policy", () => {
  ...
  it("requires authentication when an HTTPS proxy publishes a loopback bind", () => {
    ...
    ).toContain("without QUANTUM_AUTH_TOKEN");
  });

  it("rejects invalid public URLs in the shared embedded-server policy", () => {
    for (const publicUrl of [
      new URL("http://quantum.example.test/"),
      new URL("https://quantum.example.test/app"),
    ]) {
      expect(
        remoteAccessPolicyError({
          ...remoteBase,
  ...
  it("accepts only credential-free HTTPS root origins", () => {
    expect(normalizeHttpsPublicOrigin(new URL("https://quantum.example.test/"))?.origin).toBe(
      "https://quantum.example.test",
    );
    for (const value of [
      "http://quantum.example.test/",
      "https://user:pass@quantum.example.test/",
      "https://quantum.example.test/app",
      "https://quantum.example.test/?query=1",
      "https://quantum.example.test/#fragment",
    ]) {
