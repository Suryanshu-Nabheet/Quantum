...
import { workspaceRootsEqual } from "@quantum/shared/threadWorkspace";
import {
  isThreadDetailEventFor,
  THREAD_DETAIL_EVENT_TYPES,
} from "@quantum/shared/threadDetailEvents";
...
import { discoverSkillsCatalog, quantumSkillsDir } from "./provider/skillsCatalog";
...
class WsRequestAdmissionMiddleware extends RpcMiddleware.Service<WsRequestAdmissionMiddleware>()(
  "quantum/WsRequestAdmissionMiddleware",
              ...
              Effect.gen(function* () {
                ...
                const registerCheckout = Effect.gen(function* () {
                  yield* Queue.offer(queue, {
                    ...
                    kind: "phase",
                    phase: "registering",
                    message: "Adding project to Quantum",
                  });
          ...
          rpcEffect(
            Effect.tryPromise(() =>
              discoverSkillsCatalog({
                cwd: input.cwd ?? null,
                homeDir: config.homeDir,
                quantumBaseDir: config.baseDir,
                includeDuplicateOrigins: true,
              }),
            ).pipe(
              Effect.map((skills) => ({
                skills,
                quantumSkillsDir: quantumSkillsDir(config.baseDir),
              })),
            ),
    ...
    Effect.gen(function* () {
      ...
      yield* router.add(
        ...
        Effect.gen(function* () {
          ...
          // The desktop app fetches cross-origin (quantum://app); reflect only
