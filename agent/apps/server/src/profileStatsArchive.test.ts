...
import { MessageId, ThreadId, TurnId } from "@quantum/contracts";
...
const testLayer = Layer.mergeAll(ProfileStatsQueryLive, ProfileStatsArchiveLive).pipe(
  ...
  Layer.provide(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "quantum-profile-stats-archive-test-",
    }),
  ),
      ...
      Effect.gen(function* () {
        ...
        yield* sql`
          INSERT INTO external_mcp_integrations (
            ...
            'integration-purge', 'Purge integration', 'quantum.external-mcp', 'other',
