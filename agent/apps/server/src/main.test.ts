...
import { CliConfig, makeServerStartupLogData, quantumCli, type CliConfigShape } from "./main";
...
let defaultQuantumHome = "";
const tempHomes = new Set<string>();

function makeTempHome(prefix = "quantum-main-test-"): string {
...
const testLayer = Layer.mergeAll(
  Layer.succeed(CliConfig, {
    cwd: "/tmp/quantum-test-workspace",
    fixPath: Effect.void,
...
const runCli = (args: ReadonlyArray<string>, env: Record<string, string> = {}) => {
  const program = Command.runWith(quantumCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            QUANTUM_HOME: defaultQuantumHome,
            QUANTUM_NO_BROWSER: "true",
            ...env,
          },
        }),
      ),
...
beforeEach(() => {
  vi.clearAllMocks();
  defaultQuantumHome = makeTempHome();
  resolvedConfig = null;
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("quantum-main-flag-");
  ...
  it.effect("passes the root --home-dir flag to MCP subcommands", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("quantum-main-mcp-flag-");
  ...
  it.effect("creates fresh local state directories with private permissions", () =>
    Effect.gen(function* () {
      ...
      const homeDir = makeTempHome("quantum-main-private-fresh-");
  ...
  it.effect("repairs permissions for an upgraded local state directory", () =>
    Effect.gen(function* () {
      if (process.platform === "win32") return;
      const homeDir = makeTempHome("quantum-main-private-upgrade-");
      const stateDir = path.join(homeDir, "userdata");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      const envHome = makeTempHome("quantum-main-env-");

      yield* runCli([], {
        QUANTUM_MODE: "desktop",
        QUANTUM_PORT: "4999",
        QUANTUM_HOST: "127.0.0.1",
        QUANTUM_HOME: envHome,
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        QUANTUM_NO_BROWSER: "true",
        QUANTUM_AUTH_TOKEN: "env-token",
        QUANTUM_DESKTOP_SHUTDOWN_TOKEN: "shutdown-token",
      });
  ...
  it.effect("consumes desktop shutdown authority before generic child launches", () =>
    Effect.gen(function* () {
      const canonicalKey = "QUANTUM_DESKTOP_SHUTDOWN_TOKEN";
      const mixedCaseKey = "qUaNtUm_dEsKtOp_ShUtDoWn_ToKeN";
      ...
      try {
        ...
        resolvedConfig = null;
        yield* runCli([], { QUANTUM_DESKTOP_SHUTDOWN_TOKEN: injectedToken });
  ...
  it.effect("omits both server authority secrets from startup log data", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        QUANTUM_AUTH_TOKEN: "browser-secret",
        QUANTUM_DESKTOP_SHUTDOWN_TOKEN: "shutdown-secret",
      });
  ...
  it.effect("prefers --mode over QUANTUM_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over QUANTUM_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        QUANTUM_NO_BROWSER: "false",
      });
    ...
    Effect.gen(function* () {
      yield* runCli(
        ...
        ],
        {
          QUANTUM_MODE: "desktop",
          QUANTUM_NO_BROWSER: "true",
          QUANTUM_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
          QUANTUM_LOG_PROVIDER_EVENTS: "true",
          QUANTUM_LOG_WS_EVENTS: "true",
        },
      );
  ...
  it.effect("honors insecure remote opt-in from the environment when the CLI flag is absent", () =>
    Effect.gen(function* () {
      yield* runCli(["--host", "0.0.0.0", "--auth-token", "remote-secret"], {
        QUANTUM_ALLOW_INSECURE_REMOTE: "true",
        QUANTUM_NO_BROWSER: "true",
      });
  ...
  it.effect("uses the HTTPS public origin for remote startup pairing", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.publicUrl?.origin, "https://quantum.example.test");
      assert.equal(openBrowser.mock.calls.length, 1);
      assert.match(
        openBrowser.mock.calls[0]?.[0] ?? "",
        /^https:\/\/quantum\.example\.test\/pair#token=/,
      );
    }),
  );
  ...
  it.effect("supports the HTTPS public origin through environment configuration", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        QUANTUM_HOST: "192.168.1.50",
        QUANTUM_AUTH_TOKEN: "remote-secret",
        QUANTUM_PUBLIC_URL: "https://quantum.example.test",
      });
  ...
  it.effect("rejects non-root or non-HTTPS public URLs", () =>
    Effect.gen(function* () {
      for (const publicUrl of ["http://quantum.example.test", "https://quantum.example.test/app"]) {
        const error = yield* Effect.flip(
  ...
  it.effect("refuses non-loopback exposure without authentication", () =>
    Effect.gen(function* () {
      ...
      assert.match(String(error), /Refusing to bind Quantum to non-loopback host 0\.0\.0\.0/);
    }),
  );
  ...
  it.effect("supports CLI and env for bootstrap/provider-log/websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        QUANTUM_MODE: "desktop",
        QUANTUM_LOG_PROVIDER_EVENTS: "true",
        QUANTUM_LOG_WS_EVENTS: "false",
        QUANTUM_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        QUANTUM_NO_BROWSER: "true",
      });
  ...
  it.effect("rejects invalid boolean environment values instead of treating them as absent", () =>
    Effect.gen(function* () {
      const error = yield* Effect.flip(
        runCli([], {
          QUANTUM_LOG_PROVIDER_EVENTS: "sometimes",
        }),
      );
