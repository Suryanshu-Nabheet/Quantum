...
export function remoteAccessPolicyError(
  ...
  if (config.publicUrl && !normalizeHttpsPublicOrigin(config.publicUrl)) {
    return "QUANTUM_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://quantum.example.com).";
  }
  const isPubliclyExposed = isRemoteBind || Boolean(config.publicUrl);
  if (!isPubliclyExposed) return null;
  if (!config.authToken?.trim()) {
    return config.publicUrl
      ? "Refusing to publish Quantum through QUANTUM_PUBLIC_URL/--public-url without QUANTUM_AUTH_TOKEN/--auth-token."
      : `Refusing to bind Quantum to non-loopback host ${config.host ?? "<unspecified>"} without QUANTUM_AUTH_TOKEN/--auth-token.`;
  }
  if (config.devUrl) {
  ...
  if (isRemoteBind && !config.publicUrl && !config.allowInsecureRemote) {
    return "Refusing plaintext remote access. Configure an HTTPS reverse-proxy origin with QUANTUM_PUBLIC_URL/--public-url, or explicitly accept unencrypted LAN traffic with QUANTUM_ALLOW_INSECURE_REMOTE/--allow-insecure-remote.";
  }
  return null;
}
...
export function resolveDefaultChatWorkspaceRoot(input: {
  ...
  return pathApi.join(homeDir, "Documents", "Quantum");
}
...
export const resolveStaticDir = Effect.fn(function* () {
  ...
  const snapshotDir = process.env.QUANTUM_STATIC_DIR?.trim();
  if (snapshotDir) {
