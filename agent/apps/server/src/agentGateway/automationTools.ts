...
} from "@quantum/shared/automationMode";
...
export function makeAgentGatewayAutomationTools(
  ...
  const createAutomation: ToolEntry = {
    ...
    definition: {
      ...
      description: `Create a heartbeat, standalone, or dedicated Quantum automation. ${AUTOMATION_AUTHORING_GUIDANCE} Existing calls remain compatible: omitting mode/schedule creates a heartbeat on your thread using everyMinutes (default 5). Prefer suggested:true unless the user explicitly requested creation.`,
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Create a Quantum automation", ...WRITE_TOOL_ANNOTATIONS },
    },
  ...
  const listAutomations: ToolEntry = {
    ...
    definition: {
      name: "quantum_list_automations",
      description:
        "List Quantum automations (id, name, mode, schedule, target thread, enabled, next run).",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "List Quantum automations", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args) =>
      Effect.gen(function* () {
  ...
  const viewAutomation: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "View a complete automation definition, recent runs, next run, and persistent-memory excerpt. Call this immediately before quantum_update_automation and resend every unchanged mutable field.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "View a Quantum automation", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
  ...
  const updateAutomation: ToolEntry = {
    ...
    definition: {
      ...
      annotations: { title: "Replace a Quantum automation", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const automationId = readStringArg(args, "automationId", { required: true })!;
  ...
  const cancelAutomation: ToolEntry = {
    ...
    definition: {
      ...
      description:
        'Stop a Quantum automation. mode "disable" (default) pauses it and keeps history; "delete" archives it. An automation-dispatched run may always stop its own automation, whatever its mode. Prefer a completionPolicy stop clause for conditions known when the automation is created.',
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Stop a Quantum automation", ...WRITE_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
        const automationId = readStringArg(args, "automationId", { required: true })!;
  ...
  const reportResult: ToolEntry = {
    ...
    requiresActiveTurn: true,
    definition: {
      name: "quantum_report_automation_result",
      description:
