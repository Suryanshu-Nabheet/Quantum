/**
 * Stdio-to-HTTP proxy script for the Quantum agent gateway.
 *
...
const STDIO_PROXY_SCRIPT = `// Quantum agent gateway stdio<->HTTP MCP proxy (generated file, do not edit).
const url = process.env.QUANTUM_AGENT_GATEWAY_URL;
let token = process.env.QUANTUM_AGENT_GATEWAY_TOKEN;
let bootstrapToken = process.env.QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
...
function localInactiveResponse(message) {
  ...
  // CLI launched outside Quantum therefore sees a valid empty MCP server
  // instead of a noisy failed integration. Quantum-managed processes receive
  ...
  return [
    {
      jsonrpc: "2.0",
      id,
      error: { code: -32601, message: "Quantum is not active for this Antigravity session." },
    },
  ];
}
...
async function resolveToken() {
  ...
  if (!tokenResolution) {
    ...
    tokenResolution = (async () => {
      ...
      if (!response.ok) {
        throw new Error("Quantum gateway bootstrap failed with HTTP " + response.status);
      }
      const payload = await response.json();
      if (!isRecord(payload) || typeof payload.bearerToken !== "string") {
        throw new Error("Quantum gateway bootstrap returned an invalid response");
      }
      token = payload.bearerToken;
      bootstrapToken = undefined;
      delete process.env.QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
      return token;
    })().finally(() => {
    ...
    return [
      {
        jsonrpc: "2.0",
        id,
        error: { code: -32603, message: "Quantum gateway request failed: " + String(error) },
      },
    ];
  }
}
