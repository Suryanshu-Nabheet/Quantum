...
import {
  ...
  takeQuantumHarnessPolicyForSession,
} from "./harnessPolicy.ts";

describe("Quantum harness policy", () => {
  it("identifies Quantum and explains exact batch coordination when MCP is available", () => {
    ...
    assert.include(policy, "exact thread-scoped Electron page Quantum surfaces to the user");
    ...
    assert.include(policy, "do not create Quantum threads");
  ...
  it("never advertises gateway mutation to providers without scoped MCP", () => {
    const policy = renderQuantumHarnessPolicy({ gatewayControlAvailable: false });
    assert.include(policy, "Quantum MCP control is unavailable");
    assert.notInclude(policy, "one exact quantum_create_threads plan");
  });

  it("delivers a private host-context block once per provider session", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    assert.include(
      takeQuantumHarnessPolicyForSession(state, { gatewayControlAvailable: true }) ?? "",
      "<quantum_host_context>",
    );
    assert.isNull(takeQuantumHarnessPolicyForSession(state, { gatewayControlAvailable: true }));
  });
  ...
  it("delivers once on fresh/load/fork sessions for every scoped MCP provider", () => {
    for (const provider of [
      ...
      for (const lifecycle of ["fresh", "load", "fork"] as const) {
        ...
        assert.include(first, QUANTUM_HARNESS_POLICY_MARKER, `${provider}/${lifecycle}`);
        assert.include(first, "Use the quantum_* tools", `${provider}/${lifecycle}`);
        assert.isNull(
          takeQuantumHarnessPolicyForProviderSession(state, {
            provider,
            scopedGatewayConnectionAvailable: true,
          }),
          `${provider}/${lifecycle}`,
  ...
  it("keeps OpenCode, Kilo, and Pi identity-only until scoped setup succeeds", () => {
    for (const provider of ["opencode", "kilo", "pi"] as const) {
      ...
      assert.include(text, QUANTUM_HARNESS_POLICY_MARKER, provider);
      assert.include(text, "Quantum MCP control is unavailable", provider);
      assert.notInclude(text, "one exact quantum_create_threads plan", provider);
    }
  });
  ...
  it("teaches the device tools well enough for a plain prompt to work", () => {
    ...
    // a window the user is not watching and leaves the Quantum pane empty. A
