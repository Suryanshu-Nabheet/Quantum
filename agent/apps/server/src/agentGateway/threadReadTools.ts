...
import { QUANTUM_HARNESS_POLICY_VERSION } from "./harnessPolicy.ts";
...
export function makeThreadReadTools(input: ThreadReadToolsInput): ReadonlyArray<ToolEntry> {
  ...
  const contextTool: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "Inspect the current Quantum harness identity, caller thread/turn, and authorized coordination capabilities.",
    ...
    handler: (_args, context) =>
      Effect.gen(function* () {
        ...
        return mcpToolResultJson({
          harness: { name: "Quantum", policyVersion: QUANTUM_HARNESS_POLICY_VERSION },
          caller: {
            threadId: caller.id,
            turnId,
  ...
  const capabilitiesTool: ToolEntry = {
    ...
    definition: {
      ...
      description: `List canonical Quantum provider/model targets, exact provider option keys, examples, and gateway limits used to validate thread creation. ${AGENT_GATEWAY_TARGET_OPTIONS_DESCRIPTION}`,
      inputSchema: { type: "object", properties: {}, additionalProperties: false },
      annotations: {
    ...
    handler: (_args, context) =>
      Effect.gen(function* () {
        ...
        return mcpToolResultJson({
          ...
          providers,
          limits: {
            maxThreadsPerOperation: QUANTUM_GATEWAY_MAX_THREADS_PER_OPERATION,
            maxWaitMs: 60_000,
  ...
  const listProjects: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "List Quantum projects (id, title, workspace root). System-managed containers (the Chats and Studio surfaces) are not projects and are excluded. Use before creating a thread in another project.",
      inputSchema: { type: "object", properties: {}, additionalProperties: false },
      annotations: { title: "List Quantum projects", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: () =>
  ...
  const listThreads: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "Discover Quantum threads by project, hierarchy, provider, model, status, title, creation source, or update window. Archived threads are hidden unless includeArchived is true.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "List Quantum threads", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
  ...
  const readThread: ToolEntry = {
    ...
    definition: {
      ...
      description:
        "Read one Quantum thread's status and recent messages (newest last, truncated). Pass the returned nextCursor as cursor to page older messages.",
      inputSchema: {
        type: "object",
        properties: {
      ...
      annotations: { title: "Read a Quantum thread", ...READ_ONLY_TOOL_ANNOTATIONS },
    },
    handler: (args, context) =>
      Effect.gen(function* () {
  ...
  const waitForThreads: ToolEntry = {
    ...
    definition: {
      ...
      annotations: {
        title: "Wait for Quantum threads",
        readOnlyHint: true,
        destructiveHint: false,
        idempotentHint: true,
        openWorldHint: false,
                ...
                return Effect.succeed(
                  pinned.map((pin) => {
                    ...
                    return {
                      ...
                      readThread: {
                        tool: "quantum_read_thread" as const,
