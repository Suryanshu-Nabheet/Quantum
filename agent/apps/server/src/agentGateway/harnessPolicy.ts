import type { ProviderKind } from "@quantum/contracts";
...
export const QUANTUM_HARNESS_POLICY_VERSION = "2026-08-03.8";
export const QUANTUM_HARNESS_POLICY_MARKER = `[Quantum harness policy ${QUANTUM_HARNESS_POLICY_VERSION}]`;

export interface QuantumHarnessCapabilities {
 ...
 * Quantum resources.
 */
export function renderQuantumHarnessPolicy(capabilities: QuantumHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        "Use the quantum_* tools for Quantum threads, projects, automations, and coordination.",
        "Use the browser_* tools autonomously whenever the user refers in any language to Quantum's integrated, embedded, visible, or in-app browser. They are the canonical and complete control surface for that browser: do not load or use a generic Browser, Chrome, Computer Use, OS-automation, Node REPL, Playwright, or other browser-control skill/tool instead. They control the exact thread-scoped Electron page Quantum surfaces to the user, including its live DOM, cookies, and session. The page may continue in the background while the user views another chat; browser actions must never change the user's active chat. When no assigned tab exists, start with browser_open rather than browser_navigate. Take a fresh semantic browser_snapshot before element actions and after navigation or human interaction, requesting an image only when semantics are insufficient.",
        ...
        "For thread discovery and diagnosis, use quantum_list_threads, quantum_read_thread, quantum_read_thread_activity, quantum_read_thread_events, quantum_read_thread_runtime_events, and quantum_diagnose_thread before inspecting Quantum's SQLite files or process logs. Fall back to host storage only when a tool's coverage metadata says the required evidence is unavailable.",
        "Provider-native subagent or Task tools are implementation details: they do not create Quantum threads and must not substitute for an explicit request to create Quantum threads.",
        "For a plural thread request, submit one exact quantum_create_threads plan. The array length is the exact requested count.",
        "If quantum_create_threads rejects the plan during validation or preflight before returning an operationId, correct that same plan and retry it with the same requestId. This is safe because no durable operation, thread, or worktree was created.",
        "Use quantum_capabilities to select canonical provider, model, and option values. Never guess a model slug or silently substitute a provider or model.",
        "Provider option keys are not interchangeable: Codex uses options.reasoningEffort and Claude Agent uses options.effort. Follow quantum_capabilities.targetConstruction for every provider instead of inspecting Quantum source code.",
        "When results are requested, call quantum_wait_for_threads for the created thread ids, wait for every requested result, then synthesize all outcomes.",
        "After quantum_create_threads returns an operationId, retries must keep the same requestId and exact plan. Report terminal operation failures as outcomes; do not create replacement threads unless the user gives a new instruction.",
        "Quantum automations support heartbeat, standalone, and dedicated modes plus interval, once, daily, weekdays, weekly, and cron schedules. Existing everyMinutes heartbeat calls remain supported. Use fastInterval: true only when the user explicitly accepts a sub-minute bounded loop.",
        ...
        'Mode does not restrict stop conditions. completionPolicy {"type":"ai-evaluated","stopWhen":"..."} works in both modes and disables the automation when the clause matches a successful run; prefer it over encoding the stop condition in the prompt. maxIterations remains the backstop, and an automation-dispatched run may always call quantum_cancel_automation on its own automation.',
        AUTOMATION_AUTHORING_GUIDANCE,
        "Prefer quantum_create_automation with suggested: true when the user has not explicitly asked to create an automation. Suggested automations remain disabled until the user accepts their proposal card.",
        "Before quantum_update_automation, call quantum_view_automation and resend the complete mutable configuration, including unchanged fields. Updates are full replacement and partial payloads are rejected.",
        ...
        'During an automation-dispatched turn, persist durable context with quantum_update_automation_memory {"memory": "..."} before finishing; memory is full replacement, DB-backed, and capped at 32 KiB.',
        'Every automation-dispatched turn must finish by calling quantum_report_automation_result. Use decision "silent" only for a successful run with nothing requiring user attention; otherwise use "notify" with a concise title and summary. Failures remain visible regardless of this decision or the automation notification policy. Never call this tool for a manual follow-up turn.',
      ]
    : [
        "Quantum MCP control is unavailable in this provider session. Do not claim that Quantum threads, projects, or automations were created or changed.",
        "Provider-native subagent or Task tools do not create Quantum threads. If the user explicitly requests Quantum resource management, explain that this session cannot perform it.",
      ];
...
export const QUANTUM_GATEWAY_HARNESS_POLICY = renderQuantumHarnessPolicy({
  gatewayControlAvailable: true,
});

export const QUANTUM_IDENTITY_ONLY_HARNESS_POLICY = renderQuantumHarnessPolicy({
  gatewayControlAvailable: false,
});

export interface QuantumHarnessPolicyDeliveryState {
  harnessPolicyDelivered?: boolean | undefined;
}

const PROVIDERS_WITH_THREAD_SCOPED_QUANTUM_MCP = new Set<ProviderKind>([
  "codex",
  "claudeAgent",
  "antigravity",
  "cursor",
...
export function providerHasQuantumGatewayControl(input: {
  readonly provider: ProviderKind;
  readonly scopedGatewayConnectionAvailable: boolean;
}): boolean {
  return (
    input.scopedGatewayConnectionAvailable &&
    PROVIDERS_WITH_THREAD_SCOPED_QUANTUM_MCP.has(input.provider)
  );
}

/** Return the private host-context block exactly once for one provider session. */
export function takeQuantumHarnessPolicyForSession(
  state: QuantumHarnessPolicyDeliveryState,
  capabilities: QuantumHarnessCapabilities,
): string | null {
  if (state.harnessPolicyDelivered === true) return null;
  state.harnessPolicyDelivered = true;
  return [
    "<quantum_host_context>",
    renderQuantumHarnessPolicy(capabilities),
    "</quantum_host_context>",
  ].join("\n");
}
 ...
 */
export function takeQuantumHarnessPolicyForProviderSession(
  state: QuantumHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  return takeQuantumHarnessPolicyForSession(state, {
    gatewayControlAvailable: providerHasQuantumGatewayControl(input),
  });
}

export function takeQuantumHarnessPolicyTextPartForProviderSession(
  state: QuantumHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  const text = takeQuantumHarnessPolicyForProviderSession(state, input);
