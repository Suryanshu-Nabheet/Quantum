...
import {
  ...
  type ProviderKind,
  type QuantumCreateThreadsInput,
  type QuantumCreateThreadsResult,
} from "@quantum/contracts";
import { buildPromptThreadTitleFallback } from "@quantum/shared/chatThreads";
import { WORKTREE_BRANCH_PREFIX } from "@quantum/shared/git";
import { parseGitHubRepositoryNameWithOwnerFromPullRequestUrl } from "@quantum/shared/githubRepository";
...
export const makeCreateThreadsHandler = Effect.fn(function* (
  ...
  const awaitCreationReplay = (
    ...
    Effect.gen(function* () {
      ...
      return yield* Effect.fail(
        new GatewayToolError(
          "operation_failed",
          "The original thread-creation operation is still in progress. Retry only with the same request id; Quantum will not create replacement threads.",
  ...
  const appendThreadCreationRecap = (input: {
    ...
    readonly result: QuantumCreateThreadsResult;
  }) => {
    ...
    return orchestrationEngine
      .dispatch({
        ...
        activity: {
          ...
          tone: "info",
          kind: "quantum.threads.created",
          summary: `Created ${input.result.createdCount} Quantum ${threadLabel}`,
          payload: {
            source: "quantum_mcp",
            operationId: input.result.operationId,
  ...
  const run = (input: typeof QuantumCreateThreadsInput.Type, context: GatewayCreationContext) => {
    return Effect.gen(function* () {
      ...
      if (deprecatedBranchName) {
        return yield* Effect.fail(
          new ToolInputError(
            '"branchName" is no longer supported for managed worktrees. Quantum creates a managed temporary branch and renames it after the first prompt; create additional branches inside the new thread if needed.',
          ),
        );
      }
        ...
        Effect.gen(function* () {
          ...
          if (environment === "worktree") {
            ...
            if (existsSync(plannedWorktreePath)) {
              return yield* Effect.fail(
                new ToolInputError(
                  `Worktree path "${plannedWorktreePath}" already exists. Quantum will not reuse or remove a pre-existing path.`,
                ),
              );
            }
          }
          return {
            ...
            // The 8-hex-digit token keeps it a temporary quantum/* branch.
            newBranch:
        ...
        Effect.gen(function* () {
                          ...
                          Effect.flatMap(() =>
                            worktree.branch === null
                              ...
                              : // The branch is this operation's own deterministic
                                // quantum/* name and its worktree was just force-removed.
          ...
          if (compensationErrors.length > 0) {
            ...
            return new GatewayToolError(
              "operation_failed",
              "Quantum could not dispatch the exact creation plan and cleanup is still pending. The durable operation remains compensating and will never create replacements.",
          ...
          if (statusFailure !== null) {
            ...
            return new GatewayToolError(
              "operation_failed",
              "Quantum compensated the created resources but could not persist a terminal operation status. The operation remains compensating and will never create replacements.",
              { operationId, ...failure, compensationPending: true },
            );
          }
          return new GatewayToolError(
            "operation_failed",
            "Quantum could not dispatch the exact creation plan. Created operation-owned resources were compensated; no replacements were created.",
            { operationId, ...failure },
          );
        });
      ...
      const outcome = yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
                ...
                Effect.gen(function* () {
                  ...
                  yield* orchestrationEngine
                    .dispatch({
                      ...
                      worktreePath,
                      creationSource:
                        context.kind === "external-client" ? "external_mcp" : "quantum_mcp",
                      ...(context.kind === "provider-session"
                        ? {
          ...
          const result = {
            ...
            threads: results,
          } satisfies QuantumCreateThreadsResult;
