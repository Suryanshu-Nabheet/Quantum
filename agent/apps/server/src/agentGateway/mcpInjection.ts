/**
 * Provider-facing config builders for the Quantum agent gateway.
 *
 ...
 *
 * - Codex: `[mcp_servers.quantum]` TOML block (streamable HTTP +
...
export const QUANTUM_MCP_SERVER_NAME = "quantum";
export const QUANTUM_AGENT_GATEWAY_TOKEN_ENV = "QUANTUM_AGENT_GATEWAY_TOKEN";
export const QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV = "QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN";
export const QUANTUM_AGENT_GATEWAY_URL_ENV = "QUANTUM_AGENT_GATEWAY_URL";
 ...
 * the block references an env var that Quantum sets per app-server process.
 *
...
export function buildCodexMcpConfigToml(endpointUrl: string): string {
  return [
    `[mcp_servers.${QUANTUM_MCP_SERVER_NAME}]`,
    `url = ${JSON.stringify(endpointUrl)}`,
    `bearer_token_env_var = ${JSON.stringify(QUANTUM_AGENT_GATEWAY_TOKEN_ENV)}`,
    "",
    "[shell_environment_policy]",
    `exclude = [${JSON.stringify(QUANTUM_AGENT_GATEWAY_TOKEN_ENV)}]`,
  ].join("\n");
}
 ...
 * provider process dedicated to the owning Quantum thread or an exclusive
...
async function postAgentGatewayJsonRpc(input: {
  ...
  if (!response.ok) {
    throw new Error(`Quantum MCP request failed with HTTP ${String(response.status)}.`);
  }
  const payload: unknown = await response.json();
  if (!isRecord(payload) || payload.jsonrpc !== "2.0") {
    throw new Error("Quantum MCP returned an invalid JSON-RPC response.");
  }
  if ("error" in payload) {
    const failure = isRecord(payload.error) ? payload.error : null;
    throw new Error(failure?.message ? String(failure.message) : "Quantum MCP request failed.");
  }
  if (payload.id !== id || !("result" in payload)) {
    throw new Error("Quantum MCP returned a mismatched JSON-RPC response.");
  }
...
export async function listAgentGatewayMcpTools(input: {
  ...
  if (!isRecord(result) || !Array.isArray(result.tools)) {
    throw new Error("Quantum MCP tools/list returned an invalid tool catalog.");
  }
  return result.tools.map((value) => {
    if (
      ...
      throw new Error("Quantum MCP tools/list returned an invalid tool descriptor.");
    }
    return {
      name: value.name,
...
/**
 * Build the secret-free MCP fragment installed with Quantum's Antigravity
  ...
  return {
    mcpServers: {
      [QUANTUM_MCP_SERVER_NAME]: {
        ...
        env: {
          [QUANTUM_AGENT_GATEWAY_URL_ENV]: `$${QUANTUM_AGENT_GATEWAY_URL_ENV}`,
          [QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: `$${QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV}`,
 ...
 */
export function buildAcpQuantumMcpServers(input: {
  readonly connection: AgentGatewayMcpConnection;
  ...
  return [
    {
      ...
      env: [
        { name: QUANTUM_AGENT_GATEWAY_URL_ENV, value: input.connection.url },
        { name: QUANTUM_AGENT_GATEWAY_TOKEN_ENV, value: input.connection.bearerToken },
      ],
    },
  ];
}
