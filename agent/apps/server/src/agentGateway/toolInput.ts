...
export const MODEL_SELECTION_INPUT_SCHEMA = {
  ...
  properties: {
    ...
    model: {
      type: "string",
      description: "Exact model slug from quantum_capabilities providers[].models[].slug.",
    },
    options: {
      type: "object",
      description: AGENT_GATEWAY_TARGET_OPTIONS_DESCRIPTION,
...
export function decodeCreateThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(QuantumCreateThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Quantum creation plan: ${errorText(error)}`);
  }
}

export function decodeWaitForThreadsInput(value: unknown) {
  try {
    return Schema.decodeUnknownSync(QuantumWaitForThreadsInput)(value);
  } catch (error) {
    throw new ToolInputError(`Invalid Quantum wait request: ${errorText(error)}`);
  }
}
