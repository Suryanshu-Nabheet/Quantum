...
describe("agent gateway MCP injection", () => {
  ...
  it("builds a codex config block that references the token env var, not the token", () => {
    ...
    assert.include(block, "[mcp_servers.quantum]");
    assert.include(block, `url = "${connection.url}"`);
    assert.include(block, `bearer_token_env_var = "${QUANTUM_AGENT_GATEWAY_TOKEN_ENV}"`);
  ...
  it("appends the codex section once and keeps existing config intact", () => {
    ...
    assert.include(appended, "[mcp_servers.quantum]");

    const reappended = appendCodexConfigSection(appended, section);
    assert.equal(reappended.split("[mcp_servers.quantum]").length, 2);
  });
  ...
  it("merges the token exclusion into a user-defined shell environment policy", () => {
    ...
    const merged = mergeShellEnvPolicyExclude(withExclude, QUANTUM_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${QUANTUM_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`);

    // Idempotent: the var is not added twice.
    assert.equal(mergeShellEnvPolicyExclude(merged, QUANTUM_AGENT_GATEWAY_TOKEN_ENV), merged);

    // A policy table without an exclude key gains one.
    const withoutExclude = ["[shell_environment_policy]", 'inherit = "core"'].join("\n");
    const gained = mergeShellEnvPolicyExclude(withoutExclude, QUANTUM_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(gained, `exclude = ["${QUANTUM_AGENT_GATEWAY_TOKEN_ENV}"]`);
    assert.include(gained, 'inherit = "core"');

    // No policy table: unchanged (the managed section appends its own).
    assert.equal(
      mergeShellEnvPolicyExclude('[model]\nname = "gpt-5.5"', QUANTUM_AGENT_GATEWAY_TOKEN_ENV),
      '[model]\nname = "gpt-5.5"',
    );
  });
  ...
  it("ignores commented and unrelated token references when merging shell exclusions", () => {
    const commentedExample = [
      "[shell_environment_policy]",
      `# exclude = ["${QUANTUM_AGENT_GATEWAY_TOKEN_ENV}"]`,
      'exclude = ["AWS_*"]',
    ].join("\n");
    ...
    assert.include(
      ...
      `exclude = ["${QUANTUM_AGENT_GATEWAY_TOKEN_ENV}", "AWS_*"]`,
    );

    const unrelatedString = [
      "[shell_environment_policy]",
      `note = "keep ${QUANTUM_AGENT_GATEWAY_TOKEN_ENV} private"`,
      'exclude = ["AWS_*"]',
    ].join("\n");
  ...
  it("recognizes only active exact entries in multiline shell exclusion arrays", () => {
    ...
    assert.equal(mergeShellEnvPolicyExclude(existing, QUANTUM_AGENT_GATEWAY_TOKEN_ENV), existing);
    ...
    const merged = mergeShellEnvPolicyExclude(tokenOnlyInComment, QUANTUM_AGENT_GATEWAY_TOKEN_ENV);
    assert.include(merged, `exclude = ["${QUANTUM_AGENT_GATEWAY_TOKEN_ENV}",`);
  });

  it("detects real TOML table headers, ignoring comments and strings", () => {
    assert.isTrue(
      configHasTomlTableHeader('[mcp_servers.quantum]\nurl = "x"', "[mcp_servers.quantum]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [mcp_servers.quantum]  # managed", "[mcp_servers.quantum]"),
    );
    assert.isTrue(
      configHasTomlTableHeader("  [ mcp_servers.quantum ]  # managed", "[mcp_servers.quantum]"),
    );
    assert.isTrue(configHasTomlTableHeader("  [ mcp_servers . quantum ]", "[mcp_servers.quantum]"));
    assert.isTrue(configHasTomlTableHeader('[mcp_servers."quantum"]', "[mcp_servers.quantum]"));
    assert.isTrue(configHasTomlTableHeader("['mcp_servers'.'quantum']", "[mcp_servers.quantum]"));
    assert.isTrue(
      configHasTomlTableHeader('[mcp_servers."quant\\u0075m"]', "[mcp_servers.quantum]"),
    );
    assert.isTrue(
      configHasTomlTableHeader('["shell_environment_policy"]', "[shell_environment_policy]"),
    );
    assert.isFalse(configHasTomlTableHeader('["mcp_servers.quantum"]', "[mcp_servers.quantum]"));
    assert.isFalse(configHasTomlTableHeader('[mcp_servers."quant\\qum"]', "[mcp_servers.quantum]"));
    // A commented-out example block must not count as the table being present.
    assert.isFalse(configHasTomlTableHeader("# [mcp_servers.quantum]", "[mcp_servers.quantum]"));
    assert.isFalse(
      configHasTomlTableHeader('note = "see [mcp_servers.quantum] docs"', "[mcp_servers.quantum]"),
    );
  });
                ...
                tools: [
                  {
                    ...
                    description: "List Quantum threads.",
  ...
  it("uses the ACP http transport when the agent advertises support", () => {
    const servers = buildAcpQuantumMcpServers({
      connection,
    ...
    assert.deepEqual(servers, [
      {
        ...
        args: stdioProxy.args,
        env: [
          { name: "QUANTUM_AGENT_GATEWAY_URL", value: connection.url },
          { name: QUANTUM_AGENT_GATEWAY_TOKEN_ENV, value: connection.bearerToken },
        ],
      },
    ]);
  });
