...
import { prewarmChatGptVoiceTranscriptionConnection } from "@quantum/shared/chatGptVoiceTranscription";
import { getModelSelectionBooleanOptionValue, normalizeModelSlug } from "@quantum/shared/model";
import { decodeSubagentReceiverThreadIds } from "@quantum/shared/subagents";
import { prepareWindowsSafeProcess } from "@quantum/shared/windowsProcess";
...
import { QUANTUM_GATEWAY_HARNESS_POLICY } from "./agentGateway/harnessPolicy.ts";
import {
...
Prefer Quantum's built-in browser for browser work. It may continue in the background without changing the user's active chat. In code mode, call its MCP methods directly inside \`functions.exec\` with the exact \`tools.mcp__quantum__browser_*\` prefix (for example, \`await tools.mcp__quantum__browser_open({ url })\` and \`await tools.mcp__quantum__browser_snapshot({})\`). The available suffixes are ${BROWSER_TOOL_NAMES.map((name) => `\`${name.slice("browser_".length)}\``).join(", ")}.
...
// Maps Quantum's simple runtime toggle to Codex thread-level permission overrides.
...
// Quantum re-sends turn-level Codex permission overrides, so keep "always allow"
...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  private readonly quantumSkillsDir: string | undefined;
  ...
  constructor(
    ...
    options?: {
      readonly quantumSkillsDir?: string;
    ...
    this.quantumSkillsDir = options?.quantumSkillsDir;
  ...
  // The Quantum MCP server rides on the shared overlay config (no secrets),
  ...
  private async buildSessionProcessEnv(
    ...
    if (gatewayBearerToken) {
      env[QUANTUM_AGENT_GATEWAY_TOKEN_ENV] = gatewayBearerToken;
    }
    return env;
  }

  // Registers `~/.quantum/skills` as a codex skill root so portable skills are
  ...
  private async registerQuantumSkillsRoot(context: CodexSessionContext): Promise<void> {
    if (!this.quantumSkillsDir) {
      return;
    }
    try {
      await this.sendRequest(context, "skills/extraRoots/set", {
        extraRoots: [this.quantumSkillsDir],
      });
    } catch (error) {
      // Older codex builds (< extra-roots support) keep working; Quantum-only
  ...
  async startSession(input: CodexAppServerStartSessionInput): Promise<ProviderSession> {
    ...
    try {
      ...
      await this.registerQuantumSkillsRoot(context);
  ...
  private async handleServerRequest(
    ...
    if (isUserInputRequest) {
      ...
      const detail = "Codex asked a question Quantum could not render, so it was declined.";
