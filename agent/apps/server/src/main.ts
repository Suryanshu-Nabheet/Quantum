...
const DESKTOP_SHUTDOWN_TOKEN_ENV_KEY = "QUANTUM_DESKTOP_SHUTDOWN_TOKEN";
...
interface CliInput {
  ...
  readonly quantumHome: Option.Option<string>;
...
const CliEnvConfig = Config.all({
  mode: Config.string("QUANTUM_MODE").pipe(
    Config.option,
    Config.map(
  ...
  port: Config.port("QUANTUM_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("QUANTUM_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  quantumHome: Config.string("QUANTUM_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  publicUrl: Config.url("QUANTUM_PUBLIC_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  allowInsecureRemote: optionalBooleanEnvironmentConfig("QUANTUM_ALLOW_INSECURE_REMOTE"),
  noBrowser: optionalBooleanEnvironmentConfig("QUANTUM_NO_BROWSER"),
  authToken: Config.string("QUANTUM_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  desktopShutdownToken: Config.string("QUANTUM_DESKTOP_SHUTDOWN_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: optionalBooleanEnvironmentConfig(
    "QUANTUM_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
  ),
  logProviderEvents: optionalBooleanEnvironmentConfig("QUANTUM_LOG_PROVIDER_EVENTS"),
  logWebSocketEvents: optionalBooleanEnvironmentConfig("QUANTUM_LOG_WS_EVENTS"),
});
    ...
    Effect.gen(function* () {
      ...
      if (configuredPublicUrl && publicUrl === undefined) {
        return yield* new StartupError({
          message:
            "QUANTUM_PUBLIC_URL/--public-url must be an HTTPS root origin without credentials, path, query, or fragment (for example https://quantum.example.com).",
        });
      }
      ...
      const configuredHome = Option.getOrUndefined(input.quantumHome) ?? env.quantumHome;
      ...
      yield* Effect.try({
        try: () => preparePrivateServerPaths(derivedPaths),
          ...
          new StartupError({ message: "Failed to secure Quantum's local state directory", cause }),
      });
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("Quantum running", makeServerStartupLogData(config));
      ...
      if (config.allowInsecureRemote && !config.publicUrl) {
        yield* Effect.logWarning(
          "INSECURE REMOTE ACCESS ENABLED: credentials and session traffic are unencrypted",
          {
            ...
            hint: "Use only on a trusted LAN. Configure QUANTUM_PUBLIC_URL behind HTTPS for protected remote access.",
          },
        );
      }
...
const quantumHomeFlag = Flag.string("home-dir").pipe(
  Flag.withDescription("Base directory for all Quantum data (equivalent to QUANTUM_HOME)."),
  Flag.optional,
);
...
const publicUrlFlag = Flag.string("public-url").pipe(
  ...
  Flag.withDescription(
    "HTTPS public root origin provided by a TLS-terminating reverse proxy (equivalent to QUANTUM_PUBLIC_URL).",
  ),
  Flag.optional,
);
const allowInsecureRemoteFlag = optionalBooleanFlag("allow-insecure-remote", {
  description:
    "Explicitly allow unencrypted authenticated remote access on a trusted LAN (equivalent to QUANTUM_ALLOW_INSECURE_REMOTE).",
});
...
const logProviderEventsFlag = optionalBooleanFlag("log-provider-events", {
  description:
    "Emit native/canonical provider NDJSON logs for debugging (equivalent to QUANTUM_LOG_PROVIDER_EVENTS).",
});
const logWebSocketEventsFlag = optionalBooleanFlag("log-websocket-events", {
  description:
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to QUANTUM_LOG_WS_EVENTS).",
...
// Base `quantumm` command defined before the MCP subcommands so they can yield
// its parsed input (notably `--home-dir` / `quantumHome`) via Effect's command
...
const baseServerCommand = Command.make("quantumm", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  quantumHome: quantumHomeFlag,
  devUrl: devUrlFlag,
...
}).pipe(Command.withDescription("Run the Quantum server."));
...
const mcpServeCommand = Command.make(
  ...
  ({ integration }) =>
    Effect.gen(function* () {
      ...
      const baseDir = resolveExternalMcpBaseDir(Option.getOrUndefined(parent.quantumHome));
  ...
  Command.withDescription(
    "Serve the paired Quantum external MCP integration over stdio for Codex, Claude, and other MCP clients.",
  ),
);
...
const mcpPairCommand = Command.make(
  "pair",
  {
    code: Flag.string("code").pipe(
      Flag.withDescription("Short-lived pairing code issued by Quantum Settings."),
    ),
  },
  ({ code }) =>
    Effect.gen(function* () {
      ...
      process.stdout.write(
        `Paired Quantum external MCP integration "${paired.paired.name}".\nCredential stored privately at ${paired.storePath}.\nConfigure the MCP client command as: ${externalMcpShellCommand(externalMcpLauncher(["mcp", "serve", "--integration", paired.paired.integrationId, "--home-dir", baseDir]))}\n`,
      );
      if (process.platform === "win32") {
        process.stdout.write(
          "Windows note: Quantum stores this credential under your user profile, but Windows does not expose POSIX 0600 permission checks. Protect the profile and its Quantum data directory.\n",
        );
      }
    }),
).pipe(Command.withDescription("Pair this CLI with a user-approved Quantum MCP integration."));

const mcpCommand = Command.make("mcp").pipe(
  Command.withDescription("Manage Quantum's loopback external MCP bridge."),
...
export const quantumCli = serverCommand;
