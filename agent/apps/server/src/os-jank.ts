...
import { createCachedLoginShellPathReader } from "@quantum/shared/loginShellEnvironment";
import {
  expandHomePath as expandHomePathSync,
  resolveQuantumHomeDirectory,
} from "@quantum/shared/quantumHome";
...
export function fixPath(
  ...
  try {
    ...
    // Cached by default: the probe result is persisted under the Quantum home and reused
...
export const resolveBaseDir = (raw: string | undefined): Effect.Effect<string> =>
  Effect.succeed(resolveQuantumHomeDirectory({ configuredHome: raw }));
