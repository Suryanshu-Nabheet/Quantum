...
describe("trustedOrigins", () => {
  ...
  it("trusts only the HTTPS public origin for browser traffic in proxy-backed remote mode", () => {
    ...
    expect(
      isTrustedAppOrigin({
        ...
        requestOrigin: "http://quantum.example.test",
        config: remoteConfig,
      }),
    ).toBe(true);
    expect(
  ...
  it("normalizes desktop origins with trailing slashes", () => {
    expect(normalizeCorsOrigin("quantum://app/")).toBe("quantum://app");
    expect(normalizeCorsOrigin("quantum-canary://app/")).toBe("quantum-canary://app");
  });
