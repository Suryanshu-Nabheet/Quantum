...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "quantum-terminal-"));
  ...
  it("keeps a running terminal alive when open reattaches with different cwd or env", async () => {
    ...
    const snapshot = await manager.open(
      openInput({ cwd: logsDir, env: { QUANTUM_TERMINAL_TEST: "changed" } }),
    );
    ...
    if (process.platform === "win32") {
      ...
      expect(
        ptyAdapter.spawnInputs.some((input) =>
          [process.env.SHELL, "/bin/zsh", "/bin/bash", "/bin/sh", "zsh", "bash", "sh"]
            .filter(Boolean)
            .includes(input.shell),
        ),
      ).toBe(true);
    }
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.QUANTUM_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          QUANTUM_PROJECT_ROOT: "/repo",
          QUANTUM_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.QUANTUM_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.QUANTUM_WORKTREE_PATH).toBe("/repo/worktree-a");
