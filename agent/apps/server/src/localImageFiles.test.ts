...
describe("resolveAllowedLocalPreviewFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("quantum-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("quantum-codex-home-");
  ...
  it("allows images written to the QUANTUM_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Quantum overlay
    // directory (see resolveQuantumCodexHomeOverlayPath). Generated images therefore
    // live under <QUANTUM_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    const quantumHome = path.join(fakeRoot, "quantum", "runtime");
    ...
    process.env.QUANTUM_HOME = quantumHome;
    try {
      const result = await resolveAllowedLocalPreviewFile({
        requestedPath: imagePath,
        cwd: null,
  ...
  it("allows PDFs inside the current workspace", async () => {
    const workspace = makeTempDir("quantum-pdf-workspace-");
    writeFileSync(path.join(workspace, ".git"), "gitdir: .git");
  ...
  it("allows PDFs inside a per-thread scratch workspace without a cwd", async () => {
    ...
    // <tmpdir>/quantum-codex-workspaces/<threadId>; files agents create there
    // are workspace-equivalent, so documents must be servable from that root.
    const scratchRoot = path.join(os.tmpdir(), "quantum-codex-workspaces");
  ...
  it("rejects PDFs outside the workspace even under the temp-dir image roots", async () => {
    ...
    const tempDir = makeTempDir("quantum-pdf-outside-");
  ...
  it("still allows images under the temp-dir roots without a workspace", async () => {
    const tempDir = makeTempDir("quantum-image-tmp-root-");
