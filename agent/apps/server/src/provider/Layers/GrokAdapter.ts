...
import { decodeOutboundJson, decodeOutboundText, outboundHttp } from "@quantum/shared/outboundHttp";
import {
  Cause,
  DateTime,
  Deferred,
...
import { buildAcpQuantumMcpServers } from "../../agentGateway/mcpInjection.ts";
import {
  type QuantumHarnessPolicyDeliveryState,
  takeQuantumHarnessPolicyTextPartForProviderSession,
...
export const takeGrokQuantumHarnessPolicyTextPart = (
  state: QuantumHarnessPolicyDeliveryState,
  scopedGatewayConnectionAvailable: boolean,
) =>
  takeQuantumHarnessPolicyTextPartForProviderSession(state, {
    provider: PROVIDER,
...
const GROK_ACP_DEBUG_ENV = "QUANTUM_GROK_ACP_DEBUG";
const QUANTUM_GROK_ACP_DEBUG_ENV = "QUANTUM_GROK_ACP_DEBUG";
...
// override with QUANTUM_GROK_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const GROK_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "QUANTUM_GROK_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
const GROK_ACTIVE_TOOL_IDLE_TIMEOUT_MS = 60 * 60_000;
...
const GROK_PLAN_MODE_PROMPT_PREFIX = [
  "Quantum requested Grok's native plan mode.",
...
const GROK_PLAN_GUARD_HOOK_CALLBACK_ID = "quantum-plan-guard";
...
export function buildGrokPromptMeta(interactionMode: ProviderInteractionMode): {
  ...
  // the provider state when Quantum sends the desired mode again.
...
export function resolveGrokPlanHookResponse(
  ...
  return {
    decision: "deny",
    systemMessage: `Quantum Plan mode blocks the mutating or unknown Grok tool "${toolName || "unknown"}".`,
  };
}
...
function isGrokAcpDebugEnabled(): boolean {
  return (
    ...
    process.env[QUANTUM_GROK_ACP_DEBUG_ENV] === "1" ||
...
export function makeGrokAdapter(
  ...
  return Effect.gen(function* () {
    ...
    // present, every session gets the quantum_* MCP tools.
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const acp = yield* makeGrokAcpRuntime({
            ...
            clientInfo: { name: "Quantum", version: "0.0.0" },
                ...
                Effect.gen(function* () {
                  ...
                  if (planMarkdown !== undefined) {
                    ...
                    if (
                      ...
                      ctx.lastPlanFingerprint = planMarkdown;
                      // The extension response must reach Grok before Quantum cancels the
    ...
    const startGrokTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeGrokQuantumHarnessPolicyTextPart(
          ctx,
              ...
              Effect.gen(function* () {
                ...
                if (terminalPlanMarkdown !== undefined) {
                  ...
                  yield* offerRuntimeEvent(ctx.lifecycleGeneration, {
                    ...
                    raw: {
                      source: "acp.jsonrpc",
                      method: "quantum.grok.terminal-plan-response",
                      payload: result,
                    },
                  });
                }
        ...
        yield* forkAcpTurnIdleWatchdog({
          ...
          currentIdleTimeoutMs: () =>
            ctx.activeToolCallIds.size > 0
              ? GROK_ACTIVE_TOOL_IDLE_TIMEOUT_MS
              : GROK_TURN_IDLE_TIMEOUT_MS,
          ...
          isToolActive: () => ctx.activeToolCallIds.size > 0,
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Grok ACP version does not advertise session/fork; Quantum will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Grok session has a turn in flight; Quantum will rebuild the fork from its retained transcript.",
          });
        }
        const forked = activeSource
          ...
          : yield* Effect.gen(function* () {
              ...
              const runtime = yield* makeGrokAcpRuntime({
                ...
                clientInfo: { name: "Quantum Fork", version: "0.0.0" },
                sessionMeta: GROK_SESSION_META,
              });
