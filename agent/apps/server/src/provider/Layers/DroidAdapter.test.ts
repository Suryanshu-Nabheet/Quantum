...
import {
  ...
  takeDroidQuantumHarnessPolicyTextPart,
} from "./DroidAdapter.ts";

describe("Droid Quantum harness policy", () => {
  it("delivers private scoped host context once", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    expect(takeDroidQuantumHarnessPolicyTextPart(state, true)?.text).toContain(
      QUANTUM_HARNESS_POLICY_MARKER,
    );
    expect(takeDroidQuantumHarnessPolicyTextPart(state, true)).toBeNull();
  });
});
