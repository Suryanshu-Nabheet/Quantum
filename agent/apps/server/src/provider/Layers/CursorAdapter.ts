...
export const takeCursorQuantumHarnessPolicyTextPart = (
...
// forever. Generous by design; override with QUANTUM_CURSOR_TURN_IDLE_TIMEOUT_MS.
const CURSOR_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "QUANTUM_CURSOR_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
const CURSOR_ACTIVE_TOOL_IDLE_TIMEOUT_MS = 60 * 60_000;
...
const CURSOR_PLAN_MODE_PROMPT_PREFIX = [
  "Quantum Cursor plan mode is active.",
...
export function describeCursorErrorCause(cause: unknown): string {
  ...
  return Effect.gen(function* () {
    ...
    const sendTurn: CursorAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeCursorQuantumHarnessPolicyTextPart(
          ctx,
        ...
        yield* forkAcpTurnIdleWatchdog({
          ...
          currentIdleTimeoutMs: () =>
            ctx.activeToolCallIds.size > 0
              ? CURSOR_ACTIVE_TOOL_IDLE_TIMEOUT_MS
              : CURSOR_TURN_IDLE_TIMEOUT_MS,
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Cursor ACP version does not advertise session/fork; Quantum will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Cursor session has a turn in flight; Quantum will rebuild the fork from its retained transcript.",
          });
        }
