...
interface OpenCodeSessionContext {
  ...
  /** Permission request ids resolved by Quantum policy and never surfaced to the UI. */
...
const installOpenCodeGatewayMcp = Effect.fn("installOpenCodeGatewayMcp")(function* (input: {
  ...
  const result = yield* runOpenCodeSdk("mcp.add", () =>
    input.client.mcp.add({
      ...
      name: QUANTUM_MCP_SERVER_NAME,
      config: buildOpenCodeMcpServer(input.connection),
    }),
  );
  const status = result.data?.[QUANTUM_MCP_SERVER_NAME];
  ...
  return yield* new OpenCodeRuntimeError({
    ...
    detail:
      status?.status === "failed"
        ? `${input.displayName} Quantum MCP connection failed: ${status.error}`
        : `${input.displayName} Quantum MCP connection did not become ready.`,
  });
});
...
function isMatchingHarnessPolicyDelivery(
  ...
  return (
    ...
    delivery.policyVersion === QUANTUM_HARNESS_POLICY_VERSION &&
    ...
    ...(input.harnessPolicyDelivered
      ? {
          harnessPolicyDelivery: {
            ...
            policyVersion: QUANTUM_HARNESS_POLICY_VERSION,
  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
        ...
        if (!context.activeTurnToolCallIdleWatchdogStarted) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (context.activeTurnSawFinalAssistant) {
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "quantum.opencode.deferred-idle-completion",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "quantum.opencode.deferred-idle-local-part",
                    event: raw,
                  },
              ...
              if (
                ...
                (yield* recoverOpenCodeTurnFromMessageId(context, {
                  ...
                  raw: {
                    source: "quantum.opencode.deferred-idle-completion-retry",
                    event: raw,
                  },
                }))
              ) {
              ...
              if (
                ...
                yield* completeOpenCodeTurn(context, {
                  turnId,
                  raw: {
                    source: "quantum.opencode.deferred-idle-local-part-retry",
                    event: raw,
                  },
                  totalCostUsd: context.latestTurnCostUsd,
                });
            ...
            const completed = yield* completeOpenCodeTurn(context, {
              turnId,
              raw: {
                source: "quantum.opencode.idle-after-tool-calls",
                event: raw,
              },
      ...
      const recoverOpenCodeTurnFromMessages = Effect.fn("recoverOpenCodeTurnFromMessages")(
        function* (
          ...
          return yield* recoverOpenCodeTurnFromAssistantMessage(context, {
            ...
            assistantEntry,
            raw: {
              source: "quantum.opencode.prompt.recovery",
        ...
        yield* Effect.gen(function* () {
          ...
          Effect.flatMap(() =>
            Effect.gen(function* () {
              ...
              const completed = yield* completeOpenCodeTurn(context, {
                turnId: input.turnId,
                raw: { source: "quantum.opencode.prompt.watchdog" },
                errorMessage: message,
              });
              if (!completed) return;
              yield* emit(context, {
        ...
        switch (event.type) {
          ...
          case "permission.replied": {
            if (context.policyResolvedPermissionIds.has(event.properties.requestID)) {
              // Quantum policy resolved this request; nothing was surfaced to the UI.
              break;
            }
          ...
          // agent loop. Mirror them into Quantum's canonical transcript stream.
                  ...
                  Effect.gen(function* () {
                      ...
                      gatewayControlAvailable = yield* installOpenCodeGatewayMcp({
                        ...
                        Effect.catchCause((cause) =>
                          Effect.sync(() => agentGatewaySessionLease?.release()).pipe(
                            Effect.andThen(
                              Effect.logWarning(
                                `${adapterConfig.displayName} could not install thread-scoped Quantum MCP control`,
                                Cause.squash(cause),
                              ),
                            ),
                            Effect.as(false),
                    ...
                    const createSessionId = resumedSessionId
                      ? // A resumed provider may still be executing an interrupted Plan turn.
                        // Install the read-only ruleset until Quantum dispatches a new turn with a
                        ...
                        // without ever emitting a permission request for Quantum to reject.
      ...
      const sendTurnImpl = Effect.fn("sendTurn")(function* (
        ...
        const harnessPolicy = takeQuantumHarnessPolicyForProviderSession(
          {
        ...
        // Always pin Quantum's interaction mode to OpenCode's primary agent.
        // Otherwise a user config with default agent=plan (or a stale options.agent=plan
        // after leaving Quantum plan mode) can trap default turns in plan mode.
        ...
        if (provider === "kilo") {
          ...
          // server rather than by one long-lived HTTP request. Keep Quantum's event
        ...
        Effect.gen(function* () {
          ...
          if (sourceContext?.activeTurnId !== undefined) {
            return yield* new ProviderAdapterValidationError({
              ...
              issue: `The source ${adapterConfig.displayName} session has a turn in flight; Quantum will rebuild the fork from its retained transcript.`,
            });
          }
