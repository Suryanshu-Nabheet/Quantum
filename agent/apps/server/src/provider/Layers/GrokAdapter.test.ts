...
import { TurnId } from "@quantum/contracts";
...
import {
  ...
  takeGrokQuantumHarnessPolicyTextPart,
} from "./GrokAdapter.ts";

describe("Grok Quantum harness policy", () => {
  it("delivers private scoped host context once", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    expect(takeGrokQuantumHarnessPolicyTextPart(state, true)?.text).toContain(
      QUANTUM_HARNESS_POLICY_MARKER,
    );
    expect(takeGrokQuantumHarnessPolicyTextPart(state, true)).toBeNull();
  });
});
...
describe("Grok native plan approval", () => {
  it("adds Plan instructions without sending a pager-only slash command as model text", () => {
    ...
    ).toMatch(/^Quantum requested Grok's native plan mode\./u);
  });
  ...
  it("backs native Plan mode with a fail-closed pre-tool hook", () => {
    expect(
      resolveGrokPlanHookResponse("plan", {
        hookCallbackId: "quantum-plan-guard",
  ...
  it("keeps native plan mode gated after Quantum captures the plan", () => {
    expect(makeGrokExitPlanModeCapturedResponse()).toEqual({
      outcome: "cancelled",
      feedback:
        "Quantum captured this plan for user review. Do not revise or implement it now. End this turn and wait for the user's next message.",
    });
  });
...
describe("Grok native user questions", () => {
  ...
  it("maps Quantum answers to Grok's question-text keyed response", () => {
