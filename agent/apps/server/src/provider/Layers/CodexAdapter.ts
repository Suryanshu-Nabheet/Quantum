...
import { quantumSkillsDir } from "../skillsCatalog.ts";
...
// Generous by design; override with QUANTUM_CODEX_TURN_IDLE_TIMEOUT_MS.
const CODEX_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "QUANTUM_CODEX_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 900_000,
});
const CODEX_ACTIVE_TOOL_IDLE_TIMEOUT_MS = 60 * 60_000;
  ...
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        return (
          ...
          new CodexAppServerManager(services, {
            quantumSkillsDir: quantumSkillsDir(serverConfig.baseDir),
      ...
      if (entry) {
        ...
        for (const runtimeEvent of runtimeEvents) {
          if (runtimeEvent.type === "item.started" && runtimeEvent.itemId) {
            const itemType = (runtimeEvent.payload as { itemType?: string } | undefined)?.itemType;
            if (
              ...
              entry.activeToolCallIds.add(runtimeEvent.itemId);
            }
          } else if (runtimeEvent.type === "item.completed" && runtimeEvent.itemId) {
            entry.activeToolCallIds.delete(runtimeEvent.itemId);
          }
        }
      }
      for (const runtimeEvent of runtimeEvents) {
    ...
    const evaluateTurnWatchdogs = (): void => {
      ...
      for (const [threadId, entry] of turnWatchdogs) {
        ...
        const currentIdleTimeoutMs =
          entry.activeToolCallIds.size > 0
            ? CODEX_ACTIVE_TOOL_IDLE_TIMEOUT_MS
        ...
        const decision = evaluateAcpTurnIdleTick({
          ...
          isToolActive: entry.activeToolCallIds.size > 0,
          idleMs,
