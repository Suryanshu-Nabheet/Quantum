...
import {
  ...
  QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV,
  QUANTUM_AGENT_GATEWAY_URL_ENV,
} from "../../agentGateway/mcpInjection.ts";
import {
  type QuantumHarnessPolicyDeliveryState,
  takeQuantumHarnessPolicyForProviderSession,
...
/**
 * Hook output when capture is inactive (the session is not Quantum-managed).
...
export function buildAntigravityCaptureCommand(
  ...
  if (platform === "win32") {
    return `if not defined QUANTUM_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo ${fallback}) else (set "ELECTRON_RUN_AS_NODE=1" && ${invocation})`;
  }
  return `if [ -z "\${QUANTUM_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '${fallback}'; else ELECTRON_RUN_AS_NODE=1 ${invocation}; fi`;
}
...
process.stdin.on("end", () => {
  const target = process.env.QUANTUM_ANTIGRAVITY_EVENTS;
  if (!target) {
  ...
  if (event === "pre-tool") {
    const decision = process.env.QUANTUM_ANTIGRAVITY_HOOK_DECISION === "allow" ? "allow" : "ask";
    ...
    `${JSON.stringify(
      {
        ...
        name: "quantum-capture",
        description: "Streams Antigravity CLI lifecycle events to Quantum when requested.",
      },
      null,
      2,
    )}\n`,
...
export function buildAntigravityTurnProcessEnvironment(input: {
  ...
  const gatewayKeys = hasGatewayBootstrap
    ? [QUANTUM_AGENT_GATEWAY_URL_ENV, QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]
    : [];
  const gatewayEnvironment = hasGatewayBootstrap
    ? {
        [QUANTUM_AGENT_GATEWAY_URL_ENV]: input.gatewayConnection!.url,
        [QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN_ENV]: input.gatewayBootstrapToken!,
      }
    : {};
  return buildProviderChildEnvironment({
    provider: PROVIDER,
    ...(input.baseEnv === undefined ? {} : { baseEnv: input.baseEnv }),
    inheritedQuantumKeys: [
      "QUANTUM_ANTIGRAVITY_EVENTS",
      "QUANTUM_ANTIGRAVITY_HOOK_DECISION",
      ...gatewayKeys,
    ],
    overrides: {
      QUANTUM_ANTIGRAVITY_EVENTS: input.eventFile,
...
export function buildAntigravityTurnPrompt(
  ...
  const harnessPolicy = takeQuantumHarnessPolicyForProviderSession(state, {
    provider: PROVIDER,
...
export function parseAntigravityCliModelLabel(
  ...
  if (!stripped || /^(?:fetching|loading)\b/i.test(stripped)) return null;

  // Newer `agy models` rows are `slug<TAB>Display Name (Effort)` or `slug<SPACES>Display Name (Effort)`.
  // Older builds printed only the display label. Prefer the display column when present so
  // Quantum never treats `slug\tName` or `slug  Name` as a single model id at dispatch.
  const columnMatch = stripped.match(/^[a-z0-9_.-]+(?:[\t ]{2,}|\t)(.+)$/i);
  const labelColumn = columnMatch?.[1]?.trim() ?? stripped.replace(/^(?:[*•-]\s+)+/u, "");
...
export function parseAntigravityModelLines(output: string): ProviderListModelsResult["models"] {
  ...
  return [...groups.entries()].map(([model, discoveredEfforts]) => {
    ...
    const defaultEffort =
      DEFAULT_EFFORT_BY_MODEL[model] ?? (efforts.includes("medium") ? "medium" : efforts[0]);
    return {
      slug: model,
      name: model,
      ...(efforts.length > 0
        ...
        yield* Effect.tryPromise({
          try: () =>
            ...
            new ProviderAdapterRequestError({
              ...
              detail: messageFromCause(cause, "Failed to install the Quantum capture hook."),
              cause,
            }),
        });
    ...
    const sendTurn: AntigravityAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const runDir = yield* Effect.tryPromise({
          try: () => fs.mkdtemp(path.join(os.tmpdir(), "quantum-antigravity-")),
          catch: (cause) =>
            new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "turn/prepare",
        ...
        if (gatewaySessionLease && !gatewayBootstrapToken) {
          ...
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "turn/prepare",
            detail: "The Quantum gateway credential is no longer active for this provider turn.",
          });
        }
