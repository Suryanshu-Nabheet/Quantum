...
describe("Claude Quantum harness policy", () => {
  it("advertises scoped MCP additively when credentials are available", () => {
    const text = buildEmbeddedClaudeSystemPromptAppend(true);
    assert.include(text, QUANTUM_HARNESS_POLICY_MARKER);
    assert.include(text, "Use the quantum_* tools");
    assert.notInclude(text, "Quantum MCP control is unavailable");
  });

  it("stays truthful when scoped MCP credentials are absent", () => {
    const text = buildEmbeddedClaudeSystemPromptAppend(false);
    assert.include(text, QUANTUM_HARNESS_POLICY_MARKER);
    assert.include(text, "Quantum MCP control is unavailable");
  });
});
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("injects the canonical Quantum browser MCP into an Opus 4.8 session", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        systemPrompt.append ?? "",
        "exact thread-scoped Electron page Quantum surfaces to the user",
      );
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
...
`;
  ...
  it.effect("sets plan permission mode on sendTurn when interactionMode is plan", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(promptText ?? "", "Quantum plan mode is active.");
