...
export const takeDroidQuantumHarnessPolicyTextPart = (
...
const DROID_ACP_DEBUG_ENV = "QUANTUM_DROID_ACP_DEBUG";
...
// override with QUANTUM_DROID_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const DROID_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "QUANTUM_DROID_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const DROID_PLAN_MODE_PROMPT_PREFIX = [
  "Quantum Droid plan mode is active.",
...
interface DroidSessionContext {
  ...
  /** Turns cancelled by Quantum only because their Plan proposal was captured. */
  ...
  return Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          if (resumeSessionId !== undefined && started.sessionSetupMethod === "new") {
            return yield* new ProviderAdapterRequestError({
              ...
              detail:
                "Droid could not resume the requested native session. Quantum refused the fresh fallback to avoid silently losing conversation context.",
            });
          }
    ...
    const startDroidTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeDroidQuantumHarnessPolicyTextPart(
          ctx,
        ...
        yield* forkAcpTurnIdleWatchdog({
          ...
          currentIdleTimeoutMs: () =>
            ctx.activeNestedTaskToolCallIds.size > 0 || ctx.activeToolCallIds.size > 0
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Droid ACP version does not advertise session/fork; Quantum will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Droid session has a turn in flight; Quantum will rebuild the fork from its retained transcript.",
          });
        }
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Quantum Model Discovery",
          });
          yield* runtime.start();
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Quantum Command Discovery",
          });
          yield* runtime.start();
