...
import { ThreadId } from "@quantum/contracts";
...
describe("Antigravity CLI integration helpers", () => {
  it("rotates the gateway lease per print turn and rejects a retained prior bootstrap", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "quantum-antigravity-turn-lease-"));
    ...
    try {
      await Effect.runPromise(
        Effect.gen(function* () {
          ...
          const bootstrapA = spawnedEnvironments[0]?.QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
          ...
          const bootstrapB = spawnedEnvironments[1]?.QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
  ...
  it("installs the generated Quantum MCP plugin alongside the capture hooks", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "quantum-antigravity-home-test-"));
    const stdioProxy = {
      command: "/Applications/Quantum.app/Contents/MacOS/Quantum",
      ...
      expect(
        ...
        mcpServers: {
          quantum: {
            ...
            env: {
              QUANTUM_AGENT_GATEWAY_URL: "$QUANTUM_AGENT_GATEWAY_URL",
              QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "$QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN",
  ...
  it("gives an Antigravity turn only its thread-scoped gateway credential", () => {
    const env = buildAntigravityTurnProcessEnvironment({
      ...
      baseEnv: {
        ...
        QUANTUM_AGENT_GATEWAY_URL: "http://127.0.0.1:9999/stale",
        QUANTUM_AGENT_GATEWAY_TOKEN: "stale-token",
        QUANTUM_AUTH_TOKEN: "host-control-plane-token",
        QUANTUM_BROWSER_HOST_PIPE_PATH: "/tmp/desktop.sock",
        QUANTUM_BROWSER_USE_PIPE_PATH: "/tmp/legacy.sock",
        QUANTUM_BROWSER_HOST_CAPABILITY: "desktop-capability",
        QUANTUM_BROWSER_HOST_CAPABILITY_FD: "3",
    ...
    expect(env).toEqual({
      ...
      HOME: "/home/test",
      GEMINI_API_KEY: "gemini-key",
      QUANTUM_AGENT_GATEWAY_URL: "http://127.0.0.1:3773/mcp",
      QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "thread-a-bootstrap",
      QUANTUM_ANTIGRAVITY_EVENTS: "/tmp/thread-a-hooks.ndjson",
      QUANTUM_ANTIGRAVITY_HOOK_DECISION: "allow",
    });
  });
  ...
  it("advertises canonical browser tools only while the session owns a gateway lease", () => {
    ...
    expect(identityOnlyPrompt).toContain("Quantum MCP control is unavailable");
    ...
    const envWithoutLease = buildAntigravityTurnProcessEnvironment({
      ...
      baseEnv: {
        QUANTUM_AGENT_GATEWAY_URL: "http://127.0.0.1:9999/stale",
        QUANTUM_AGENT_GATEWAY_TOKEN: "stale-token",
        QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "stale-bootstrap",
      },
    });
    expect(envWithoutLease.QUANTUM_AGENT_GATEWAY_URL).toBeUndefined();
    expect(envWithoutLease.QUANTUM_AGENT_GATEWAY_TOKEN).toBeUndefined();
    expect(envWithoutLease.QUANTUM_AGENT_GATEWAY_BOOTSTRAP_TOKEN).toBeUndefined();
  });
  ...
  it("keeps the globally installed hook neutral outside Quantum sessions", () => {
    const command = buildAntigravityCaptureCommand(
      "__quantum_gui_must_not_launch__",
  ...
  it("answers pre-tool with a decision from the capture script when capture is inactive", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "quantum-antigravity-hook-test-"));
    ...
    try {
      ...
      const result = spawnSync(process.execPath, [scriptPath, "pre-tool"], {
        env: { ...process.env, QUANTUM_ANTIGRAVITY_EVENTS: "" },
  ...
  it("runs the capture script for Quantum-managed sessions", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "quantum-antigravity-hook-test-"));
    const scriptPath = path.join(directory, "capture.cjs");
    ...
    try {
      ...
      const result = runCaptureCommand(command, payload, {
        QUANTUM_ANTIGRAVITY_EVENTS: eventPath,
        QUANTUM_ANTIGRAVITY_HOOK_DECISION: "allow",
      });
  ...
  it("runs packaged Electron as Node only for Quantum-managed sessions", () => {
    expect(
      buildAntigravityCaptureCommand(
        "/Applications/Quantum.app/Contents/MacOS/Quantum",
        "/tmp/quantum-capture/capture.cjs",
        "pre-tool",
        "darwin",
      ),
    ).toBe(
      `if [ -z "\${QUANTUM_ANTIGRAVITY_EVENTS:-}" ]; then cat >/dev/null 2>&1 || :; printf '%s\\n' '{"decision":"ask"}'; else ELECTRON_RUN_AS_NODE=1 '/Applications/Quantum.app/Contents/MacOS/Quantum' '/tmp/quantum-capture/capture.cjs' 'pre-tool'; fi`,
    );
    expect(
      buildAntigravityCaptureCommand(
        String.raw`C:\Program Files\Quantum\Quantum.exe`,
        String.raw`C:\Users\test\.gemini\capture.cjs`,
        "pre-tool",
        "win32",
      ),
    ).toBe(
      String.raw`if not defined QUANTUM_ANTIGRAVITY_EVENTS (more >nul 2>nul & echo {"decision":"ask"}) else (set "ELECTRON_RUN_AS_NODE=1" && "C:\Program Files\Quantum\Quantum.exe" "C:\Users\test\.gemini\capture.cjs" "pre-tool")`,
    );
  });
  ...
  it("advances file offsets only past complete JSONL records", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "quantum-antigravity-test-"));
  ...
  it("streams hook tool names and terminal states without arguments", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "quantum-antigravity-tool-events-"));
    ...
    const spawnProcess = ((
      ...
      eventFile = options.env?.QUANTUM_ANTIGRAVITY_EVENTS;
  ...
  it("answers stop hooks with a neutral allow-exit payload", async () => {
    const directory = await fs.mkdtemp(path.join(os.tmpdir(), "quantum-antigravity-stop-hook-"));
    const scriptPath = path.join(directory, "capture.cjs");
    const eventPath = path.join(directory, "events.ndjson");
    try {
      await fs.writeFile(scriptPath, hookScriptSource(), { mode: 0o700 });
      const result = spawnSync(process.execPath, [scriptPath, "stop"], {
        env: { ...process.env, QUANTUM_ANTIGRAVITY_EVENTS: eventPath },
...
describe("Antigravity turn settle on cancel (#465)", () => {
  ...
  it("unlocks Cancel without letting a late close settle the follow-up", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "quantum-antigravity-interrupt-hung-"));
