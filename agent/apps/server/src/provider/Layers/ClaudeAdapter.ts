...
import {
  ...
  resolveApiModelId,
  trimOrNull,
} from "@quantum/shared/model";
import { buildClaudeSubagentPrompt } from "@quantum/shared/agentMentions";
...
import { renderQuantumHarnessPolicy } from "../../agentGateway/harnessPolicy.ts";
...
// from these hints (see extractSubagentIdentityHints in @quantum/shared/subagents).
...
export const buildEmbeddedClaudeSystemPromptAppend = (gatewayControlAvailable: boolean) =>
  [
    "You are running inside Quantum, a coding app that embeds the Claude Agent SDK.",
    ...
    renderQuantumHarnessPolicy({ gatewayControlAvailable }),
  ].join("\n");
        ...
        if (Exit.isFailure(exit)) {
          if (hasPendingUserInterrupt(context) || isClaudeInterruptedCause(exit.cause)) {
            ...
            if (isClaudeMissingResumeConversationCause(exit.cause)) {
              ...
              // starts a fresh Claude session and bootstraps Quantum's retained
    ...
    const startSessionUnlocked: ClaudeAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const settings = {
          ...
          // Quantum's safer 200k budget explicit unless the thread opts into 1M.
      ...
      Effect.gen(function* () {
        ...
        if (liveSource?.turnState !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            issue:
              "The source Claude session has a turn in flight; Quantum will rebuild the fork from its retained transcript.",
          });
        }
