...
//          that makes Quantum catalog skills usable on every provider.
...
import type { ProviderKind, ProviderSkillReference } from "@quantum/contracts";
...
export function shouldInlineSkillForProvider(provider: ProviderKind, skillPath: string): boolean {
  ...
  switch (provider) {
    ...
    case "codex":
      ...
      // folders plus `~/.quantum/skills`, which Quantum registers at session start
    ...
    case "cursor":
      ...
      // only Quantum-owned paths need inlining.
      return segments.has(".quantum");
    case "claudeAgent":
