 ...
 * so callers fall back to Quantum's retained-transcript fork, and bounds the
  ...
  return Effect.gen(function* () {
    ...
    if (!(yield* input.runtime.supportsSessionRecovery)) {
      return yield* new ProviderAdapterValidationError({
        provider: input.provider,
        operation: "forkThread",
        issue: `This ${input.provider} ACP version advertises session/fork but cannot reopen the forked session; Quantum will rebuild the fork from its retained transcript.`,
      });
    }
