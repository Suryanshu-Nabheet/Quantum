// FILE: debugMode.ts
// Purpose: Applies Quantum's provider-independent evidence-first Debug instructions.
// Layer: Provider prompt policy

import type { ProviderInteractionMode } from "@quantum/contracts";

export const PROVIDER_DEBUG_MODE_PROMPT_PREFIX = `<quantum_debug_mode>
You are operating in Quantum Debug mode. Diagnose the reported defect using this evidence-first loop: observe -> reproduce -> investigate -> fix -> verify.
...
- Do not imply Quantum can observe external actions. If browser state, terminal output, logs, or another required signal is inaccessible, ask the user for that evidence.
