...
import type { ProviderSkillDescriptor } from "@quantum/contracts";
...
beforeEach(() => {
  clearSkillsCatalogCacheForTests();
  root = mkdtempSync(path.join(os.tmpdir(), "quantum-skills-catalog-"));
  homeDir = path.join(root, "home");
  quantumBaseDir = path.join(homeDir, ".quantum");
});
...
describe("discoverSkillsCatalog", () => {
  it("creates the Quantum skills folder on first discovery", async () => {
    await discoverSkillsCatalog({ homeDir, quantumBaseDir });
    await expect(access(path.join(quantumBaseDir, "skills"))).resolves.toBeUndefined();
  });

  it("aggregates skills from quantum and provider home folders with origin scopes", async () => {
    await writeSkill(path.join(quantumBaseDir, "skills", "portable"), "portable", "Quantum skill");
    ...
    const skills = await discoverSkillsCatalog({ homeDir, quantumBaseDir });
    const byName = new Map(skills.map((skill) => [skill.name, skill]));

    expect(byName.get("portable")?.scope).toBe("quantum");
  ...
  it("includes user and matching project Claude plugins but excludes other projects", async () => {
    ...
    const skills = await discoverSkillsCatalog({ cwd, homeDir, quantumBaseDir });
  ...
  it("can include duplicate skill names from different origins for settings", async () => {
    ...
    const defaultCatalog = await discoverSkillsCatalog({ homeDir, quantumBaseDir });
  ...
  it("prefers the provider-native copy and falls back to Quantum for that provider", async () => {
    await writeSkill(path.join(quantumBaseDir, "skills", "shared"), "shared", "Quantum copy");
    await writeSkill(path.join(homeDir, ".codex", "skills", "shared"), "shared", "Codex copy");
    await writeSkill(
      path.join(quantumBaseDir, "skills", "only-quantum"),
      "only-quantum",
      "Fallback",
    );

    const codexView = await discoverSkillsCatalog({ homeDir, quantumBaseDir, provider: "codex" });
    ...
    expect(codexView.some((skill) => skill.name === "only-quantum")).toBe(true);

    // A provider without its own copy resolves the Quantum fallback.
    ...
    expect(claudeShared?.scope).toBe("quantum");
  });

  it("uses documented provider alias roots before Quantum fallbacks", async () => {
    await writeSkill(path.join(quantumBaseDir, "skills", "shared"), "shared", "Quantum copy");
...
`,
  ...
  it("serves cached results within the TTL and rescans on forceReload", async () => {
    await writeSkill(path.join(quantumBaseDir, "skills", "first"), "first", "First skill");

    const initial = await discoverSkillsCatalog({ homeDir, quantumBaseDir });
    expect(initial.map((skill) => skill.name)).toEqual(["first"]);

    // A skill added after the first scan is invisible to the cached entry...
    await writeSkill(path.join(quantumBaseDir, "skills", "second"), "second", "Second skill");
    const cached = await discoverSkillsCatalog({ homeDir, quantumBaseDir });
    expect(cached.map((skill) => skill.name)).toEqual(["first"]);

    // ...but forceReload bypasses the cache and refreshes it.
    const reloaded = await discoverSkillsCatalog({ homeDir, quantumBaseDir, forceReload: true });
    expect(reloaded.map((skill) => skill.name).sort()).toEqual(["first", "second"]);
  });

  it("includes project-level .quantum skills when a cwd is provided", async () => {
    const cwd = path.join(root, "repo", "packages", "web");
    await mkdir(cwd, { recursive: true });
    await writeSkill(
      path.join(root, "repo", ".quantum", "skills", "repo-skill"),
      "repo-skill",
      "Project skill",
    );
  ...
  it("keeps home origins when the cwd lives under the home dir", async () => {
    ...
    await writeSkill(path.join(quantumBaseDir, "skills", "portable"), "portable", "Quantum");
    ...
    expect(skills.find((skill) => skill.name === "portable")?.scope).toBe("quantum");
  });

  it("dedupes same-named skills within a root deterministically", async () => {
    await writeSkill(path.join(quantumBaseDir, "skills", "zeta"), "twin", "Copy in zeta");
    await writeSkill(path.join(quantumBaseDir, "skills", "alpha"), "twin", "Copy in alpha");
...
describe("mergeSkillsIntoCatalog", () => {
  ...
  it("keeps provider-native entries and appends catalog-only entries", () => {
    const merged = mergeSkillsIntoCatalog({
      ...
      catalog: [descriptor("Shared", "quantum"), descriptor("extra", "quantum")],
    });
