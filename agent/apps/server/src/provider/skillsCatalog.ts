...
//          walking) plus the unified cross-provider skills catalog backing Quantum
//          portable skills. Aggregates `~/.quantum/skills` with every provider-native
...
//          mergeSkillsIntoCatalog, filterDisabledSkills, ensureQuantumSkillsDir

import * as fs from "node:fs/promises";
import * as nodePath from "node:path";

import type { ProviderKind, ProviderSkillDescriptor } from "@quantum/contracts";
...
export interface SkillsCatalogDiscoveryInput {
  ...
  /** Quantum base dir (usually `~/.quantum`); skills live in `{base}/skills`. */
  readonly quantumBaseDir: string;
...
export interface SkillsCatalogRootInput extends SkillsCatalogDiscoveryInput {
  /** Native provider scans can opt out; the catalog itself always includes Quantum. */
  readonly includeQuantumRoot?: boolean;
}
...
const ensuredQuantumSkillsDirs = new Set<string>();

export function clearSkillsCatalogCacheForTests(): void {
  skillsCatalogCache.clear();
  skillsCatalogInflight.clear();
  ensuredQuantumSkillsDirs.clear();
}

export function quantumSkillsDir(quantumBaseDir: string): string {
  return nodePath.join(quantumBaseDir, "skills");
}

// Creates the portable skills folder on first use so users have a drop-in target.
export async function ensureQuantumSkillsDir(quantumBaseDir: string): Promise<string> {
  const dir = quantumSkillsDir(quantumBaseDir);
  if (ensuredQuantumSkillsDirs.has(dir)) {
    return dir;
  }
  try {
    await fs.mkdir(dir, { recursive: true });
    ensuredQuantumSkillsDirs.add(dir);
  } catch {
...
const SKILL_ORIGIN_ROOTS = {
  quantum: {
    homeRoots: (input) => [quantumSkillsDir(input.quantumBaseDir)],
    projectRootNames: [".quantum"],
  },
  codex: {
    // Keep Quantum's existing Codex-local root. Official Codex discovery uses
...
// Native copies first so an agent keeps using its own skill, then Quantum as the
...
function orderedOriginsForProvider(
  provider: ProviderKind | null | undefined,
  includeQuantumRoot = true,
  includeRemainingOrigins = true,
): SkillsHomeOrigin[] {
  const preferred = preferredOriginsForProvider(provider);
  const ordered = [...preferred];
  if (includeQuantumRoot && !ordered.includes("quantum")) {
    ordered.push("quantum");
  }
  if (!includeRemainingOrigins) {
    return ordered.filter((origin) => includeQuantumRoot || origin !== "quantum");
  }
  for (const origin of HOME_ORIGIN_ORDER) {
    if (!includeQuantumRoot && origin === "quantum") {
      continue;
    }
...
export function skillsCatalogRoots(input: SkillsCatalogRootInput): SkillRoot[] {
  return rootsForOrderedOrigins(
    input,
    orderedOriginsForProvider(input.provider, input.includeQuantumRoot !== false),
  );
}
...
export async function discoverSkillsCatalog(
  ...
  const scan = (async () => {
    await ensureQuantumSkillsDir(input.quantumBaseDir);
    const roots = [
