 ...
 * Homebrew-prefix install checked by Quantum while nvm's node makes npm install
...
export function resolvePackageManagedProviderMaintenance(
  ...
  for (const commandPath of commandPaths) {
    ...
    if (
      ...
      (definition.nativeUpdate?.strategy === "always" &&
        !definition.nativeUpdate.excludedInstallSources?.includes("unknown"))
    ) {
...
export function createProviderVersionAdvisory(input: {
  ...
  const isInstalled = Boolean(input.currentVersion);

  return {
    status: advisory.status,
    currentVersion: input.currentVersion,
    latestVersion,
    // Knowable when installed and either a registry can be queried, or when a latest version was already resolved.
    latestVersionKnowable:
      isInstalled && (capabilities.latestVersionSource !== null || latestVersion !== null),
    updateCommand: isInstalled ? (capabilities.update?.command ?? null) : null,
    canUpdate: isInstalled && capabilities.update !== null,
    checkedAt: input.checkedAt ?? null,
    message: isInstalled ? advisory.message : null,
  };
}
