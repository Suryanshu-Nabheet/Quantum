...
import type { ChatAttachment } from "@quantum/contracts";
import { resolveWindowsComSpec } from "@quantum/shared/windowsProcess";
...
describe("toOpenCodeFileParts", () => {
  it("materializes image attachments as SDK file parts", () => {
    ...
    expect(
      toOpenCodeFileParts({
        ...
        resolveAttachmentPath: () => "/tmp/quantum-attachments/screenshot.png",
      }),
    ).toEqual([
      {
        type: "file",
        ...
        url: "file:///tmp/quantum-attachments/screenshot.png",
      },
    ]);
  });
  ...
  it("leaves generic files for prompt-path projection", () => {
    ...
    expect(
      toOpenCodeFileParts({
        attachments: [attachment],
        resolveAttachmentPath: () => "/tmp/quantum-attachments/notes.docx",
      }),
    ).toEqual([]);
  });
});
...
describe("buildOpenCodeServerProcessEnv", () => {
  ...
  it("strips inherited Quantum authority from managed server processes", () => {
    const env = buildOpenCodeServerProcessEnv({
      baseEnv: {
        OPENAI_API_KEY: "provider-key",
        QUANTUM_AUTH_TOKEN: "server-secret",
        QUANTUM_BROWSER_USE_PIPE_PATH: "/tmp/browser.sock",
      },
    });

    expect(env.OPENAI_API_KEY).toBe("provider-key");
    expect(env.QUANTUM_AUTH_TOKEN).toBeUndefined();
        ...
        Effect.gen(function* () {
          ...
          const connectionFiber = yield* runtime
            .connectToOpenCodeServer({
              ...
              poolIsolationKey: "quantum-kilo-thread",
            })
        ...
        Effect.gen(function* () {
          ...
          const first = yield* runtime
            .connectToOpenCodeServer({
              binaryPath: "opencode",
              cwd: "/repo",
              poolIsolationKey: "quantum-thread-a",
            })
            .pipe(Effect.provideService(Scope.Scope, firstScope));
          const second = yield* runtime
            .connectToOpenCodeServer({
              binaryPath: "opencode",
              cwd: "/repo",
              poolIsolationKey: "quantum-thread-b",
            })
            .pipe(Effect.provideService(Scope.Scope, secondScope));
