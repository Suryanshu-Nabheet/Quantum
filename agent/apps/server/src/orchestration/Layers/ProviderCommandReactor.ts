...
import { isTemporaryWorktreeBranch, WORKTREE_BRANCH_PREFIX } from "@quantum/shared/git";
import { claudeSelectionRequiresRestart } from "@quantum/shared/model";
...
} from "@quantum/shared/providerDeliveryBlock";
import { buildStalePendingRequestFailureDetail } from "@quantum/shared/threadSummary";
import { resolveThreadWorkspaceState } from "@quantum/shared/threadEnvironment";
...
import { providerStartOptionsFromServerSettings } from "@quantum/shared/serverSettings";
...
function providerPromptOverflowIssue(goalPromptOverheadChars: number): string {
  return goalPromptOverheadChars > 0
    ...
    : "The latest message is too long to include Quantum Debug mode instructions. Shorten the message and retry.";
}
...
function buildGeneratedWorktreeBranchName(raw: string): string {
  ...
  const withoutPrefix = normalized.replace(/^quantum\//, "");
...
>()("quantum/orchestration/Layers/ProviderCommandReactorConfig") {}
    ...
    if (input.reviewTarget !== undefined) {
      ...
      const replayWithTranscriptBootstrap = (
        ...
        Effect.gen(function* () {
          ...
          // dead cursor and replay once with Quantum transcript context.
          yield* clearStaleProviderResumeState({
            threadId: input.threadId,
            cause,
          ...
          if (!startupRecoveryNotifiedThreads.has(blocker.threadId)) {
            ...
            if (skippedPromptCount > 0) {
              ...
              yield* appendProviderFailureActivity({
                ...
                detail: `Quantum recovered an earlier provider failure, but ${skippedPromptCount} ${noun} skipped while the thread was blocked. Resend ${skippedPromptCount === 1 ? "it" : "them"} to continue.`,
                turnId: null,
                createdAt,
              });
