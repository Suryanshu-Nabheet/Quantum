...
import { PROVIDER_DELIVERY_BLOCK_SUMMARY } from "@quantum/shared/providerDeliveryBlock";
import {
  Duration,
  Effect,
  Exit,
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir = input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "quantum-reactor-"));
  ...
  it("starts an idle active goal with an internal continuation turn", async () => {
    ...
    expect(providerInput).toContain("<quantum_goal>");
  ...
  it("recovers an active idle goal when the reactor restarts", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        threadId: ThreadId.makeUnsafe("thread-1"),
        goal: "Resume after Quantum restarts",
        goalStartBehavior: "defer",
      }),
    );
    await harness.startReactor();
  ...
  it("injects the subagent thread goal into its parent-session steer", async () => {
    ...
    expect(steerInput).toContain("<quantum_goal>");
  ...
  it("renames temporary worktree branches and keeps associated worktree metadata in sync", async () => {
    ...
    await waitFor(async () => {
      const thread = await readHarnessThread(harness);
      return (
        thread?.branch === "quantum/app-startup-crash" &&
        thread.associatedWorktreeBranch === "quantum/app-startup-crash" &&
        thread.associatedWorktreeRef === "quantum/app-startup-crash"
      );
    });

    const thread = await readHarnessThread(harness);
    expect(thread).toMatchObject({
      branch: "quantum/app-startup-crash",
      worktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreeBranch: "quantum/app-startup-crash",
      associatedWorktreeRef: "quantum/app-startup-crash",
    });
  });
  ...
  it("uses the configured Git-writing model when the chat provider cannot generate names", async () => {
    ...
    expect(harness.renameBranch.mock.calls[0]?.[0]).toMatchObject({
      oldBranch: "quantum/cb661f0d",
      newBranch: "quantum/provider-startup-timeouts",
    });

    await waitFor(
      async () =>
        (await readHarnessThread(harness))?.branch === "quantum/provider-startup-timeouts",
    );
  });
