import type { OrchestrationThreadActivity, ProviderRuntimeEvent } from "@quantum/contracts";
import {
  ApprovalRequestId,
  CommandId,
  EventId,
...
describe("provider runtime activity projection", () => {
  ...
  it("bounds pathological tool payloads before persistence", () => {
    ...
    expect(payload.data?.__quantumTruncated).toBe(true);
  ...
  it("projects unmapped passthrough events instead of dropping them", () => {
    ...
    expect(activity).toMatchObject({
      ...
      payload: {
        ...
        detail: "Finished the refactor",
        data: expect.objectContaining({ __quantumTruncated: true }),
      },
    });
