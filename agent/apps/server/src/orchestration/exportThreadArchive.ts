...
// Purpose: Build a ZIP archive that exports a single Quantum thread so a user
...
// The export-eligibility guard lives in @quantum/shared/threadExport so the
// web composer and the HTTP route share one predicate.

import zlib from "node:zlib";

import type { OrchestrationMessage, OrchestrationThread } from "@quantum/contracts";
...
export function threadArchiveFileName(input: {
  ...
  return `quantum-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
