...
// Purpose: Compute Profile-page stats from Quantum's local projection DB only.
...
import { isBuiltInComposerSlashCommandName } from "@quantum/shared/composerSlashCommands";
...
// Builds profile skill rows from every stored Quantum user message, plus the
...
>()("quantum/profileStats/ProfileStatsQuery") {}
...
const makeProfileStatsQuery = Effect.gen(function* () {
  ...
  // Activity = days/hours the user actually sent a Quantum prompt. One day-hour
  ...
  // Token usage for EVERY provider, straight from Quantum's own DB (no external
  ...
  const getProfileStats = (
    ...
    Effect.gen(function* () {
      ...
      // ── Identity ──
      const homeDirBasename = nodePath.basename(config.homeDir) || "quantum";
