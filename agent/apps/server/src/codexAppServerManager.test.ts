...
import {
  ...
  QUANTUM_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS,
...
describe("Codex Quantum harness policy", () => {
  ...
  it("resolves the gateway endpoint when each session environment is built", async () => {
    const homePath = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-gateway-endpoint-"));
    const previousQuantumHome = process.env.QUANTUM_HOME;
    process.env.QUANTUM_HOME = path.join(homePath, "quantum-home");
    ...
    try {
      ...
      if (previousQuantumHome === undefined) {
        delete process.env.QUANTUM_HOME;
      } else {
        process.env.QUANTUM_HOME = previousQuantumHome;
      }
...
describe("codex CLI version gate", () => {
  it("memoizes the version probe per binary and shares concurrent probes", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-version-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("QUANTUM_HOME", path.join(dir, "runtime"));
  ...
  it("does not reuse a general-version verdict for the stricter Auto floor", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-version-auto-floor-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("QUANTUM_HOME", path.join(dir, "runtime"));
  ...
  it("fails closed for Auto when the Codex CLI version cannot be parsed", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-version-auto-unknown-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("QUANTUM_HOME", path.join(dir, "runtime"));
  ...
  it("re-probes when the binary behind an unchanged path is replaced", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-version-swap-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("QUANTUM_HOME", path.join(dir, "runtime"));
    ...
    try {
      ...
      await expect(
        assertSupportedCodexCliVersion({ binaryPath, cwd: dir, homePath }),
      ).rejects.toThrow(/too old for Quantum/);
    } finally {
      reset();
      vi.unstubAllEnvs();
      rmSync(dir, { recursive: true, force: true });
  ...
  it("re-probes when a PATH-resolved codex is replaced behind the same bare name", async () => {
    ...
    const dir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-version-path-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("QUANTUM_HOME", path.join(dir, "runtime"));
  ...
  it("rejects an unsupported codex version without caching the failure", async () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-version-old-"));
    const homePath = path.join(dir, "codex-home");
    mkdirSync(homePath, { recursive: true });
    vi.stubEnv("QUANTUM_HOME", path.join(dir, "runtime"));
...
describe("buildCodexProcessEnv", () => {
  it("hydrates the active custom provider env_key from the effective CODEX_HOME", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-env-"));
    try {
      writeFileSync(
  ...
  it("keeps the private desktop browser host out of the Codex process", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-private-host-"));
    ...
    try {
      const env = await buildCodexProcessEnv({
        env: {
          ...
          QUANTUM_HOME: tempDir,
          QUANTUM_BROWSER_HOST_PIPE_PATH: "/tmp/quantum-browser-host.sock",
          QUANTUM_BROWSER_USE_PIPE_PATH: "/tmp/legacy-browser-use.sock",
      ...
      expect(env.QUANTUM_BROWSER_HOST_PIPE_PATH).toBeUndefined();
      expect(env.QUANTUM_BROWSER_USE_PIPE_PATH).toBeUndefined();
      expect(env.QUANTUM_BROWSER_HOST_CAPABILITY).toBeUndefined();
      expect(env.QUANTUM_BROWSER_HOST_CAPABILITY_FD).toBeUndefined();
  ...
  it("applies durable section suppressions inside Quantum's Codex overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "quantum-runtime-home-"));
    try {
      writeFileSync(
        path.join(tempDir, "config.toml"),
        [
          ...
          "enabled = true",
          "",
          ...QUANTUM_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS.flatMap((header) => [
            header,
            "enabled = true",
            "",
          ]),
      ...
      writeFileSync(
        path.join(overlayHome, "quantum-config-suppressions-v1.json"),
        `${JSON.stringify({
          version: 1,
      ...
      for (const header of QUANTUM_COMPETING_BROWSER_PLUGIN_SECTION_HEADERS) {
        expect(readFileSync(path.join(codexHome, "config.toml"), "utf8")).toContain(
          `${header}\nenabled = false`,
        );
  ...
  it("seeds markerless suppressions for conflicting local browser plugins", async () => {
    ...
    try {
      ...
      const suppressionMarker = JSON.parse(
        readFileSync(path.join(overlayHome, "quantum-config-suppressions-v1.json"), "utf8"),
  ...
  it("repairs stale real files in Quantum's Codex home overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "quantum-runtime-home-"));
    try {
  ...
  it("repairs stale auth.json files in Quantum's Codex home overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "quantum-runtime-home-"));
    try {
  ...
  it("preserves real generated image directories in Quantum's Codex home overlay", async () => {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "quantum-codex-env-"));
    const runtimeHome = mkdtempSync(path.join(os.tmpdir(), "quantum-runtime-home-"));
    try {
      writeFileSync(path.join(tempDir, "config.toml"), 'model = "gpt-5.5"', "utf8");
...
describe("handleStdoutLine", () => {
  ...
  it("ignores multiline and standalone JSON leaked from command output", () => {
    ...
    for (const line of ["{", "[", '{"scripts": {', "{}", "[]", '{"name":"quantum"}']) {
      handleStdoutLine(context, line);
    }
...
describe("startSession", () => {
  ...
  it("uses an isolated scratch workspace path when no cwd is provided", () => {
    ...
    expect(cwd).toContain(`${path.sep}quantum-codex-workspaces${path.sep}thread-1`);
  });

  it("reports a missing project working directory instead of a missing Codex CLI", () => {
    const missingCwd = path.join(os.tmpdir(), `quantum-missing-cwd-${randomUUID()}`, "old-project");
  ...
  it("accepts an existing project working directory", () => {
    const cwd = mkdtempSync(path.join(os.tmpdir(), "quantum-existing-cwd-"));
    try {
  ...
  it("fails session start with missing-cwd guidance instead of missing Codex CLI", async () => {
    ...
    const missingCwd = path.join(
      os.tmpdir(),
      `quantum-missing-session-cwd-${randomUUID()}`,
      "old-project",
    );
  ...
  it("fails fast with an upgrade message when codex is below the minimum supported version", async () => {
    ...
    const versionCheck = vi
      ...
      .mockImplementation(() => {
        throw new Error(
          "Codex CLI v0.36.0 is too old for Quantum. Upgrade to v0.37.0 or newer and restart Quantum.",
        );
      });
