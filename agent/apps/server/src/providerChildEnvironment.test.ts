...
describe("buildProviderChildEnvironment", () => {
  it("strips Quantum control-plane and inherited native capabilities", () => {
    const env = buildProviderChildEnvironment({
      ...
      baseEnv: {
        ...
        QUANTUM_AUTH_TOKEN: "control-plane-secret",
        QUANTUM_BROWSER_USE_PIPE_PATH: "/tmp/browser.sock",
        QUANTUM_BROWSER_HOST_CAPABILITY: "private-desktop-capability",
  ...
  it("admits only explicitly granted capability keys", () => {
    const env = buildProviderChildEnvironment({
      ...
      inheritedQuantumKeys: ["QUANTUM_BROWSER_USE_PIPE_PATH"],
  ...
  it("does not let overlays bypass the capability policy", () => {
    const env = buildProviderChildEnvironment({
      ...
      overrides: {
        ...
        QUANTUM_AUTH_TOKEN: "overlaid-control-plane-secret",
        NODE_OPTIONS: "--require=/tmp/inject.js",
      },
    });
  ...
  it("keeps stripped authority absent in descendants", () => {
    ...
    const descendantScript =
      "process.stdout.write(JSON.stringify({ xai: process.env.XAI_API_KEY, anthropic: process.env.ANTHROPIC_API_KEY, quantum: process.env.QUANTUM_AUTH_TOKEN }))";
