...
import type { BrowserAnnotationEvent, BrowserAnnotationTheme } from "@quantum/contracts";
...
test("a real Electron guest commits and reprojects a continuous annotation session", async () => {
  const mainPath = process.env.QUANTUM_E2E_ELECTRON_MAIN;
  ...
  const home = mkdtempSync(join(tmpdir(), "quantum-browser-annotations-e2e-"));
  ...
  const electronApp = await electron.launch({
    ...
    env: {
      ...
      HOME: home,
      QUANTUM_HOME: home,
      QUANTUM_BROWSER_HOST_PIPE_PATH: pipePath,
      QUANTUM_BROWSER_HOST_CAPABILITY: capability,
      QUANTUM_E2E_SHELL_PATH: shellPath,
      QUANTUM_E2E_THREAD_ID: threadId,
      QUANTUM_E2E_BROWSER_ANNOTATION_PRELOAD: annotationPreloadPath,
    },
  });
  ...
  try {
    ...
    const sendNativeInput = (event: Record<string, unknown>) =>
      electronApp.evaluate(
        (_electron, input) => {
          ...
          ).__quantumVisibleBrowserE2E.browserManager;
    ...
    const annotationEvents = async (): Promise<BrowserAnnotationEvent[]> =>
      electronApp.evaluate(() => {
        const fixture = (
          globalThis as typeof globalThis & {
            __quantumVisibleBrowserE2E: { annotationEvents: BrowserAnnotationEvent[] };
          }
        ).__quantumVisibleBrowserE2E;
