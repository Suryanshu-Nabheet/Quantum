...
import { BROWSER_TOOL_NAMES } from "@quantum/contracts";
...
test("production MCP controls one persistent Electron page across visibility changes", async () => {
  ...
  const home = mkdtempSync(join(tmpdir(), "quantum-visible-browser-e2e-"));
  ...
  try {
    ...
    await electronApp.evaluate(() => {
      (
        globalThis as typeof globalThis & {
          __quantumVisibleBrowserE2E: { setPanelRevealEnabled(enabled: boolean): void };
        }
      ).__quantumVisibleBrowserE2E.setPanelRevealEnabled(false);
    });
    ...
    await electronApp.evaluate(() => {
      ...
      ).__quantumVisibleBrowserE2E.setPanelRevealEnabled(true);
    });
    ...
    const runtimeCount = await electronApp.evaluate(() => {
      const manager = (
        globalThis as typeof globalThis & {
          __quantumVisibleBrowserE2E: { browserManager: { runtimes: Map<string, unknown> } };
        }
