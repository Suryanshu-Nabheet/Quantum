...
import type { BrowserAnnotationEvent, ThreadBrowserState, ThreadId } from "@quantum/contracts";
...
const pipePath = process.env.QUANTUM_BROWSER_HOST_PIPE_PATH;
const capability = process.env.QUANTUM_BROWSER_HOST_CAPABILITY;
const shellPath = process.env.QUANTUM_E2E_SHELL_PATH;
const threadId = process.env.QUANTUM_E2E_THREAD_ID as ThreadId | undefined;
const quantumHome = process.env.QUANTUM_HOME;
const annotationPreloadPath = process.env.QUANTUM_E2E_BROWSER_ANNOTATION_PRELOAD;

if (!pipePath || !capability || !shellPath || !threadId || !quantumHome || !annotationPreloadPath) {
  throw new Error("The visible-browser Electron fixture requires its isolated E2E environment.");
}

app.setPath("userData", path.join(quantumHome, "electron-userdata"));
...
function setPanelVisible(visible: boolean): void {
  ...
  pushState();
  mainWindow?.webContents.send("quantum-e2e:open-panel");
}
function pushState(): void {
  if (shellReady && latestState && mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("quantum-e2e:browser-state", latestState);
  }
}
...
ipcMain.on("quantum-e2e:shell-ready", () => {
  shellReady = true;
  pushState();
});

ipcMain.handle(
  "quantum-e2e:attach-webview",
