...
import { type ProjectId, type ThreadId, type TurnId } from "@quantum/contracts";
...
// Keep the v1 suffix stable while using the Quantum namespace; legacy
// `quantum:*` and `quantum:*` keys are copied over by
// `storageKeyMigration` before this store hydrates, so older payloads still
// flow through the v1 -> v2 schema migration below.
const SPLIT_VIEW_STORAGE_KEY = "quantum:split-view-state:v1";
