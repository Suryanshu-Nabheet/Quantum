import { MessageId, TurnId, type OrchestrationThreadActivity } from "@quantum/contracts";
...
describe("deriveWorkLogEntries", () => {
  ...
  it("keeps created-automation milestones and exposes their card fields despite a null turn id", () => {
    const activities: OrchestrationThreadActivity[] = [
      ...
      makeActivity({
        ...
        summary: "Created automation: Watch Quantum PR 231 - Every 5m",
        tone: "info",
        payload: {
          source: "chat-composer",
          automationId: "automation-7",
          automationName: "Watch Quantum PR 231",
  ...
  it("exposes a provider-independent Quantum thread creation recap", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "quantum-created-threads",
        createdAt: "2026-02-23T00:00:05.000Z",
        turnId: "turn-1",
        kind: "quantum.threads.created",
        summary: "Created 2 Quantum threads",
        tone: "info",
        payload: {
    ...
    expect(entry?.quantumThreadCreation).toEqual({
      operationId: "gateway:create:two-workers",
      requestedCount: 2,
      createdCount: 2,
      threads: [
  ...
  it("hides repeated non-adjacent recovery rows for the same provider turn", () => {
    ...
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        ...
        summary: "Quantum recovered a stale running state",
        turnId: "turn-stale",
              ...
              commandActions: [
                {
                  ...
                  name: "README.md",
                  path: "/Users/developer/Developer/Testing/quantum/README.md",
                },
              ],
            },
          },
        ...
        payload: {
          ...
          data: {
            item: {
              ...
              command: "/bin/zsh -lc 'git status --short'",
              cwd: "/Users/developer/Developer/Testing/quantum",
              status: "completed",
              commandActions: [{ type: "unknown", command: "git status --short" }],
  ...
  it("preserves cancellation when an owning turn aborts", () => {
    const turnId = TurnId.makeUnsafe("turn-with-cancelled-quantum-tool");
    const entries = deriveWorkLogEntries(
      [
        makeActivity({
          id: "cancelled-quantum-start",
          createdAt: "2026-02-23T00:00:01.000Z",
          kind: "tool.started",
          summary: "Quantum create thread",
          turnId,
          payload: {
            itemType: "mcp_tool_call",
            title: "Quantum create thread",
            data: {
              toolCallId: "cancelled-quantum-call",
      ...
      [
        makeActivity({
          ...
          payload: {
            ...
            status: "interrupted",
            data: {
              toolCallId: "interrupted-quantum-call",
              toolName: "mcp__quantum__quantum_create_thread",
            },
          },
        }),
  ...
  it("presents Quantum MCP activity consistently across provider item shapes", () => {
    const activities: OrchestrationThreadActivity[] = [
      makeActivity({
        id: "quantum-mcp-create-thread-progress",
        kind: "tool.updated",
        ...
        payload: {
          ...
          title: "MCP tool call",
          data: {
            toolCallId: "quantum-mcp-create",
            toolName: "mcp__quantum__quantum_create_thread",
          },
        },
      }),
      makeActivity({
        id: "quantum-dynamic-send-message-progress",
        kind: "tool.updated",
        summary: "Tool call",
        payload: {
          itemType: "dynamic_tool_call",
          title: "Quantum__quantum_send_message",
          data: {
            toolCallId: "quantum-dynamic-send",
          },
        },
      }),
      makeActivity({
        id: "quantum-file-change-list-threads-progress",
        kind: "tool.updated",
        summary: "File change",
        payload: {
          itemType: "file_change",
          title: "mcp__Quantum__quantum_list_threads",
          data: {
            toolCallId: "quantum-file-change-list",
          },
        },
      }),
    ];
    ...
    expect(entries.map((entry) => [entry.itemType, entry.toolTitle])).toEqual(
      expect.arrayContaining([
        ["mcp_tool_call", "Quantum is creating a thread"],
        ["dynamic_tool_call", "Quantum is sending a message"],
        ["file_change", "Quantum is listing threads"],
      ]),
    );
    expect(entries).toHaveLength(3);
  });

  it("preserves a failed Quantum MCP result as a failed activity sentence", () => {
    const [entry] = deriveWorkLogEntries(
      [
        makeActivity({
          id: "quantum-create-threads-failed",
          kind: "tool.completed",
          summary: "quantum__quantum_create_threads",
          payload: {
            itemType: "mcp_tool_call",
            status: "failed",
            data: {
              toolCallId: "quantum-create-failed",
    ...
    expect(entry).toMatchObject({
      toolStatus: "failed",
      toolTitle: "Quantum couldn't create threads",
