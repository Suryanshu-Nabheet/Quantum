...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.quantum/worktrees/quantum-mvp/quantum-4e609bb8",
    );
    expect(result).toBe("quantum-4e609bb8");
  });

  it("keeps legacy .quantum worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.quantum/worktrees/quantum-mvp/quantum-legacy123",
    );
    expect(result).toBe("quantum-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.quantum\\worktrees\\quantum-mvp\\quantum-4e609bb8",
    );
    expect(result).toBe("quantum-4e609bb8");
  });

  it("uses the final segment even when outside ~/.quantum/worktrees", () => {
