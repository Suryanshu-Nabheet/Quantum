...
// Purpose: Imports Quantum browser state before renderer stores hydrate after a desktop origin move.
...
export function importQuantumStorageSnapshot(
  snapshot: QuantumStorageSnapshot | null,
...
export function bootstrapQuantumStorageOriginMigration(): void {
  const bridge = globalThis.window?.desktopBridge?.storageMigration;
  if (!bridge) return;

  try {
    const snapshot = bridge.readSnapshot();
    if (snapshot && importQuantumStorageSnapshot(snapshot)) {
...
bootstrapQuantumStorageOriginMigration();
