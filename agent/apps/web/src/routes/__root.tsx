...
import { defaultTerminalTitleForCliKind } from "@quantum/shared/terminalThreads";
import { isThreadDetailEventFor } from "@quantum/shared/threadDetailEvents";
import {
  Outlet,
...
function TransportCompatibilityView({ issue }: { issue: WsCompatibilityError }) {
  const title =
    issue.action === "update-client"
      ? "This Quantum client needs an update."
      : issue.action === "update-server"
        ? "The Quantum server needs an update."
        : "Quantum needs to reconnect with a matching build.";
  const guidance =
    issue.action === "update-client"
      ? "Update or reload this client, then reconnect."
      : issue.action === "update-server"
        ? "Update or restart the server, then reload this client."
        : "Reload the app. If this repeats, restart Quantum so the client and server use matching builds.";
