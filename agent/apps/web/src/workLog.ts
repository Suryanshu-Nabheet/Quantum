...
} from "@quantum/shared/subagents";
import {
  approvalRequestKindFromRequestType,
  type ApprovalRequestKind,
} from "@quantum/shared/threadSummary";
import { summarizeToolRawOutput } from "@quantum/shared/toolOutputSummary";
import { pluralize } from "@quantum/shared/text";
import { PROVIDER_DESCRIPTORS } from "@quantum/shared/providerMetadata";
import {
...
export interface WorkLogEntry {
  ...
  toolName?: string;
  toolCallId?: string;
  toolStatus?: QuantumMcpToolStatus;
  ...
  quantumThreadCreation?: WorkLogQuantumThreadCreation;
...
export interface WorkLogQuantumCreatedThread {
  threadId: string;
  title: string;
...
export interface WorkLogQuantumThreadCreation {
  operationId: string;
  requestedCount: number;
  createdCount: number;
  threads: ReadonlyArray<WorkLogQuantumCreatedThread>;
}
...
function extractWorkLogQuantumThreadCreation(
  payload: Record<string, unknown> | null,
): WorkLogQuantumThreadCreation | null {
  if (!payload) {
    return null;
  }
  ...
  const threads = rawThreads.flatMap((value): WorkLogQuantumCreatedThread[] => {
...
function toDerivedWorkLogEntry(activity: OrchestrationThreadActivity): DerivedWorkLogEntry {
  ...
  if (activity.kind === "quantum.threads.created") {
    const quantumThreadCreation = extractWorkLogQuantumThreadCreation(payload);
    if (quantumThreadCreation) {
      entry.quantumThreadCreation = quantumThreadCreation;
    }
  }
  const readableTitle =
...
function deriveToolLifecycleStatus(
  activityKind: OrchestrationThreadActivity["kind"],
  payload: Record<string, unknown> | null,
): QuantumMcpToolStatus | undefined {
...
function mergeDerivedWorkLogEntries(
  ...
  const quantumThreadCreation = next.quantumThreadCreation ?? previous.quantumThreadCreation;
  ...
  return {
    ...
    ...(quantumThreadCreation ? { quantumThreadCreation } : {}),
