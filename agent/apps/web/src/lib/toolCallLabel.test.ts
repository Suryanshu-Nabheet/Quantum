...
import {
  ...
  deriveQuantumMcpToolTitle,
  extractWebFetchUrl,
  isInspectCommand,
  isQuantumBrowserToolCall,
  normalizeCompactToolLabel,
  resolveCommandVisualKind,
  sanitizeQuantumMcpToolPreview,
} from "./toolCallLabel";
...
describe("deriveQuantumMcpToolTitle", () => {
  it("uses stable action-first names for Quantum browser tools", () => {
    for (const status of ["running", "completed", "failed"] as const) {
      expect(
        deriveQuantumMcpToolTitle({
          toolName: "mcp__quantum__browser_open",
          status,
        }),
    ...
    expect(
      deriveQuantumMcpToolTitle({
        title: "Quantum: Browser Snapshot",
        status: "completed",
      }),
    ).toBe("Snapshot browser page");
  });

  it("has intentional running and completed copy for every Quantum gateway action", () => {
    const cases = [
      ["quantum_context", "Quantum is checking its context", "Quantum checked its context"],
      [
        "quantum_capabilities",
        "Quantum is checking available agents",
        "Quantum checked available agents",
      ],
      ["quantum_list_projects", "Quantum is listing projects", "Quantum listed projects"],
      ["quantum_list_threads", "Quantum is listing threads", "Quantum listed threads"],
      ["quantum_read_thread", "Quantum is reading a thread", "Quantum read a thread"],
      [
        "quantum_read_thread_activity",
        "Quantum is reading thread activity",
        "Quantum read thread activity",
      ],
      [
        "quantum_read_thread_events",
        "Quantum is reading thread events",
        "Quantum read thread events",
      ],
      [
        "quantum_read_thread_runtime_events",
        "Quantum is reading thread runtime events",
        "Quantum read thread runtime events",
      ],
      ["quantum_diagnose_thread", "Quantum is diagnosing a thread", "Quantum diagnosed a thread"],
      ["quantum_create_thread", "Quantum is creating a thread", "Quantum created a thread"],
      ["quantum_create_threads", "Quantum is creating threads", "Quantum created threads"],
      [
        "quantum_wait_for_threads",
        "Quantum is waiting for threads",
        "Quantum finished waiting for threads",
      ],
      ["quantum_send_message", "Quantum is sending a message", "Quantum sent a message"],
      [
        "quantum_interrupt_thread",
        "Quantum is interrupting a thread",
        "Quantum interrupted a thread",
      ],
      ["quantum_set_thread_title", "Quantum is renaming a thread", "Quantum renamed a thread"],
      ["quantum_set_thread_archived", "Quantum is updating a thread", "Quantum updated a thread"],
      [
        "quantum_create_automation",
        "Quantum is creating an automation",
        "Quantum created an automation",
      ],
      ["quantum_list_automations", "Quantum is listing automations", "Quantum listed automations"],
      [
        "quantum_cancel_automation",
        "Quantum is stopping an automation",
        "Quantum stopped an automation",
      ],
      ["quantum_overview", "Quantum is gathering an overview", "Quantum gathered an overview"],
      [
        "quantum_list_allowed_projects",
        "Quantum is listing allowed projects",
        "Quantum listed allowed projects",
      ],
      ["quantum_create_task", "Quantum is creating a task", "Quantum created a task"],
      [
        "quantum_wait_for_task",
        "Quantum is waiting for a task",
        "Quantum finished waiting for a task",
      ],
      ["quantum_read_task", "Quantum is reading a task", "Quantum read a task"],
    ] as const;

    for (const [toolName, running, completed] of cases) {
      expect(deriveQuantumMcpToolTitle({ toolName, status: "running" })).toBe(running);
      expect(deriveQuantumMcpToolTitle({ toolName, status: "completed" })).toBe(completed);
    }

    expect(
      deriveQuantumMcpToolTitle({
        toolName: "quantum_create_threads",
        status: "failed",
      }),
    ).toBe("Quantum couldn't create threads");
    expect(
      deriveQuantumMcpToolTitle({
        toolName: "quantum_create_thread",
        status: "cancelled",
      }),
    ).toBe("Quantum stopped creating a thread");
  });

  it("turns provider-specific create-thread identifiers into activity sentences", () => {
    expect(
      deriveQuantumMcpToolTitle({
        toolName: "Quantum__quantum_create_thread",
        status: "running",
      }),
    ).toBe("Quantum is creating a thread");
    expect(
      deriveQuantumMcpToolTitle({
        toolName: "mcp__quantum__quantum_create_thread",
        status: "completed",
      }),
    ).toBe("Quantum created a thread");
  });

  it("recognizes bare and already-humanized Quantum tool names", () => {
    expect(deriveQuantumMcpToolTitle({ toolName: "quantum_send_message", status: "running" })).toBe(
      "Quantum is sending a message",
    );
    expect(
      deriveQuantumMcpToolTitle({ title: "Quantum: Quantum List Threads", status: "completed" }),
    ).toBe("Quantum listed threads");
  });
  ...
  it("keeps future Quantum actions branded without exposing raw identifiers", () => {
    expect(
      deriveQuantumMcpToolTitle({
        toolName: "mcp__quantum__quantum_delete_project",
        status: "running",
      }),
    ).toBe("Quantum is handling delete project");
    expect(
      deriveQuantumMcpToolTitle({
        toolName: "Quantum__quantum_delete_project",
        status: "completed",
      }),
    ).toBe("Quantum handled delete project");
    expect(
      deriveQuantumMcpToolTitle({
        toolName: "quantum_is_handling_delete_project",
        status: "completed",
      }),
    ).toBe("Quantum handled delete project");
  });
  ...
  it("does not reinterpret free text beginning with fallback status copy", () => {
    expect(
      deriveQuantumMcpToolTitle({
        title: "Quantum is handling delete project after recovery",
        status: "completed",
      }),
    ).toBeNull();
    expect(
      deriveQuantumMcpToolTitle({
        title: "Quantum handled delete project after recovery",
        status: "running",
      }),
    ).toBeNull();
    expect(
      deriveQuantumMcpToolTitle({
        title: "Quantum couldn't handle delete project after recovery",
        status: "failed",
      }),
    ).toBeNull();
  });

  it("leaves free-text activity summaries starting with Quantum untouched", () => {
    expect(
      deriveQuantumMcpToolTitle({
        title: "Quantum recovered a stale running state",
        status: "completed",
      }),
    ).toBeNull();
    expect(
      deriveQuantumMcpToolTitle({
        fallbackLabel: "Quantum restarted the provider session",
        status: "running",
      }),
    ).toBeNull();
  });

  it("removes transport identifiers without hiding meaningful Quantum details", () => {
    expect(
      sanitizeQuantumMcpToolPreview({
        preview: "Quantum__quantum_create_threads",
        heading: "Quantum created threads",
        status: "completed",
      }),
    ).toBeNull();
    expect(
      sanitizeQuantumMcpToolPreview({
        ...
        heading: "Quantum couldn't create threads",
        status: "failed",
      }),
...
describe("isQuantumBrowserToolCall", () => {
  it("recognizes canonical presentation titles without a tool identifier", () => {
    expect(isQuantumBrowserToolCall({ title: "Open browser tab" })).toBe(true);
    expect(isQuantumBrowserToolCall({ fallbackLabel: "Snapshot browser page" })).toBe(true);
    expect(isQuantumBrowserToolCall({ title: "Quantum listed threads" })).toBe(false);
  });
});
...
describe("deriveReadableCommandDisplay", () => {
  ...
  it("removes env and timeout wrappers from inline command summaries", () => {
    expect(
      deriveReadableCommandDisplay(
        "env -u QUANTUM_AUTH_TOKEN QUANTUM_PORT_OFFSET=3158 timeout 180s bun run dev",
        true,
      ),
    ).toEqual({
      verb: "Running",
      target: "bun run dev",
      fullCommand: "env -u QUANTUM_AUTH_TOKEN QUANTUM_PORT_OFFSET=3158 timeout 180s bun run dev",
    });
  });
