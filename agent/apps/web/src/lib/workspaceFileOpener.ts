...
import { isSupportedLocalPreviewFilePath } from "@quantum/shared/localPreviewFiles";
import {
  isLocalAbsolutePath,
  isWorkspaceRelativePathSafe,
  workspaceRelativePathOf,
} from "@quantum/shared/path";
import { isScratchWorkspacePath } from "@quantum/shared/threadWorkspace";
...
const QUANTUM_PUBLIC_ASSET_PATH_PREFIXES = [
  "/central-icons-reversed/",
  "/central-icons-fill/",
] as const;
const QUANTUM_WEB_PUBLIC_WORKSPACE_DIR = "apps/web/public";

function resolveQuantumPublicAssetOpenTarget(path: string, workspaceRoot: string | null) {
  if (!workspaceRoot) {
    return null;
  }
  const normalizedPath = path.replace(/\\/g, "/");
  if (!QUANTUM_PUBLIC_ASSET_PATH_PREFIXES.some((prefix) => normalizedPath.startsWith(prefix))) {
    return null;
  }
  const relativePath = `${QUANTUM_WEB_PUBLIC_WORKSPACE_DIR}${normalizedPath}`;
...
export function resolveWorkspaceFileOpenTarget(
  ...
  return resolveQuantumPublicAssetOpenTarget(withoutPosition, workspaceRoot);
}
