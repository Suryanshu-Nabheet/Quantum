...
import type { NativeApi } from "@quantum/contracts";
...
describe("providerModelsQueryOptions", () => {
  ...
  it("disables automatic retry so discovery failures settle immediately", () => {
    expect(providerModelsQueryOptions({ provider: "codex" }).retry).toBe(0);
