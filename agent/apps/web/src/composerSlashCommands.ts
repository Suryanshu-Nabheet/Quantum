...
} from "@quantum/shared/composerSlashCommands";
...
> = {
  ...
  feedback: {
    command: "feedback",
    label: "/feedback",
    description: "Send feedback to the Quantum team",
    source: "app",
  },
  automation: {
    command: "automation",
  ...
  const availableCommands: ComposerSlashCommand[] =
    input.provider !== "claudeAgent"
      ...
      : [
          ...
          // creates a Quantum split/context clone before the provider sees the first turn.
          // /export is app-level too — Quantum owns the thread transcript, so the download
