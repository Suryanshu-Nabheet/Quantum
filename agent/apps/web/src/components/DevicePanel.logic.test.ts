import type { DeviceDescriptor, DeviceUdid } from "@quantum/contracts";
...
describe("hardware button shortcuts", () => {
  ...
  it("leaves unrelated chords to the app", () => {
    ...
    // Cmd+Shift only maps Home; other Cmd+Shift chords stay with Quantum.
    expect(
  ...
  it("shows the picker when only the helper build is left", () => {
    ...
    const view = resolveDeviceAvailabilityView({
      ...
      steps: [
        ...
        { id: "build-device-helper", label: "Build the Quantum device helper", done: false },
      ],
    });
