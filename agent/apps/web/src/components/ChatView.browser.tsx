...
} from "@quantum/shared/binaryTransfer";
  ...
  return {
    ...
    projects: [
      ...snapshot.projects,
      {
        ...
        kind: "studio",
        title: "Studio",
        workspaceRoot: "/Users/tester/Documents/Quantum/Studio",
        defaultModelSelection: {
          provider: "codex",
          model: "gpt-5",
        },
...
function resolveWsRpc(body: WsRequestEnvelope["body"]): unknown {
  ...
  if (tag === WS_METHODS.gitCreateDetachedWorktree) {
    return {
      worktree: {
        path: "/repo/.codex/worktrees/generated/quantum",
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("opens the project cwd for draft threads without a worktree path", async () => {
    ...
    try {
      const openInQuantumTrigger = await waitForElement(
        () =>
          Array.from(document.querySelectorAll<HTMLButtonElement>("button")).find(
            (button) => button.textContent?.trim() === "Open in Quantum",
          ) ?? null,
        "Unable to find Open in Quantum environment row.",
      );
      openInQuantumTrigger.click();

      const quantumOption = await waitForElement(
        () =>
          Array.from(document.querySelectorAll<HTMLElement>('[data-slot="menu-radio-item"]')).find(
            (item) => item.textContent?.trim() === "Quantum",
          ) ?? null,
        "Unable to find Quantum editor option.",
      );
      quantumOption.click();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            cwd: "/repo/project",
            env: {
              QUANTUM_PROJECT_ROOT: "/repo/project",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              QUANTUM_PROJECT_ROOT: "/repo/project",
              QUANTUM_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
  ...
  it("steers a running turn when Follow-up behavior is set to Steer", async () => {
    localStorage.setItem("quantum:app-settings:v1", JSON.stringify({ followUpBehavior: "steer" }));
    ...
    const mounted = await mountChatView({
      ...
      configureFixture: (nextFixture) => {
        nextFixture.welcome = {
          ...
          chatWorkspaceRoot: "/Users/tester/Documents/Quantum",
          studioWorkspaceRoot: "/Users/tester/Documents/Quantum/Studio",
        };
      },
    });
    ...
    try {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(temporaryBranch).toMatch(/^quantum\/[0-9a-f]{8}$/);
          ...
          expect(createThreadRequest?.command).toMatchObject({
            ...
            worktreePath: "/repo/.codex/worktrees/generated/quantum",
            associatedWorktreePath: "/repo/.codex/worktrees/generated/quantum",
          ...
          expect(
            wsRequests.some(
              (candidate) =>
                ...
                candidate.path === "/repo/.codex/worktrees/generated/quantum" &&
  ...
  it("runs the setup action from the newly-created worktree before starting the turn", async () => {
    ...
    try {
      ...
      const worktreePath = "/repo/.codex/worktrees/generated/quantum";
      ...
      expect(terminalOpenRequest).toMatchObject({
        ...
        env: {
          QUANTUM_PROJECT_ROOT: "/repo/project",
          QUANTUM_WORKTREE_PATH: worktreePath,
        },
      });
  ...
  it("applies the selected chat width to the transcript column", async () => {
    localStorage.setItem("quantum:app-settings:v1", JSON.stringify({ chatWidth: "wide" }));
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
