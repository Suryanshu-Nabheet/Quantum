...
import type { GitPullRequestComment, PullRequestComment } from "@quantum/contracts";
...
describe("buildResolveConflictsPrompt", () => {
  ...
  it("points at the current checkout instead of asserting the local branch name", () => {
    // Fork threads check the PR out under `quantum/pr-N/<branch>`, so the prompt must not
