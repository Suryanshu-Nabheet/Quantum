...
import { type MessageId, type TurnId } from "@quantum/contracts";
...
function workLogQuantumThreadCreationsEqual(
  a: WorkLogEntry["quantumThreadCreation"],
  b: WorkLogEntry["quantumThreadCreation"],
) {
  if (a === b) return true;
  if (!a || !b) return false;
  if (
...
function workLogEntryContentEqual(a: WorkLogEntry, b: WorkLogEntry): boolean {
  return (
    ...
    workLogQuantumThreadCreationsEqual(a.quantumThreadCreation, b.quantumThreadCreation) &&
