...
// Purpose: Identify the source thread for conversations created by another Quantum agent.
// Layer: Chat transcript UI

import { type ProviderKind, type ThreadId } from "@quantum/contracts";
...
// Quantum thread, so it always reads as "Sent by Quantum" with the Quantum mark
// (the origin provider is not surfaced here to keep one consistent label).
function OriginContent(): ReactNode {
  return (
    <>
      <span className="flex size-4 shrink-0 items-center justify-center text-muted-foreground/70">
        <QuantumLogo className="h-4 w-auto" aria-label="Quantum" />
      </span>
      <span className="truncate">Sent by Quantum from another thread</span>
    </>
  );
}
