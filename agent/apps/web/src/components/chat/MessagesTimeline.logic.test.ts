import { CheckpointRef, MessageId, OrchestrationProposedPlanId, TurnId } from "@quantum/contracts";
...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("preserves Quantum tool calls when a separate creation recap is present", () => {
    const createTool = workEntry(
      "quantum-create-tool",
      "2026-01-01T00:00:01Z",
      "Quantum created threads",
    );
    ...
    expect(collapsedSignature(messageRow(rows, "a1")!)).toEqual([
      "work:quantum-create-tool",
      "work:quantum-create-recap",
    ]);
  });
