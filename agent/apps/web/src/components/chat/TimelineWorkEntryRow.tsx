...
import type { TurnId } from "@quantum/contracts";
import {
  createElement,
  memo,
  useCallback,
...
const QuantumToolIcon: LucideIcon = ({ className, width, height, color: _color, ..._props }) => (
  <QuantumLogo
    className={cn("size-4 shrink-0 object-contain text-current", className)}
...
export function workEntryLeftIcon(workEntry: TimelineWorkEntry): LucideIcon {
  ...
  if (isQuantumBrowserWorkEntry(workEntry)) return GlobeIcon;
  if (isQuantumToolCall(workEntry)) return QuantumToolIcon;
...
// Quantum's own agent-gateway tools (quantum_list_threads, quantum_create_thread,
// ...) get the Quantum mark instead of the generic MCP glyph. Providers report
// the call differently: Claude prefixes the MCP server (mcp__quantum__*), ACP
// agents surface the bare tool name (quantum_*), and Codex reports server/tool
// pairs that the label humanizer renders as "Quantum: ...".
function toolWorkEntryStatus(workEntry: TimelineWorkEntry): QuantumMcpToolStatus {
...
function isQuantumBrowserWorkEntry(workEntry: TimelineWorkEntry): boolean {
  return isQuantumBrowserToolCall({
...
function isQuantumToolCall(workEntry: TimelineWorkEntry): boolean {
  return (
    deriveQuantumMcpToolTitle({
      toolName: workEntry.toolName,
      title: workEntry.toolTitle,
...
function toolWorkEntryHeading(workEntry: TimelineWorkEntry): string {
  ...
  const quantumTitle = deriveQuantumMcpToolTitle({
    toolName: workEntry.toolName,
    title: workEntry.toolTitle,
    fallbackLabel: workEntry.label,
    status: toolWorkEntryStatus(workEntry),
...
export const TimelineWorkEntryRow = memo(function TimelineWorkEntryRow(props: {
  ...
  const isQuantumBrowserToolRow = !isGitHubToolRow && isQuantumBrowserWorkEntry(workEntry);
  const isQuantumToolRow =
    !isGitHubToolRow && !isQuantumBrowserToolRow && isQuantumToolCall(workEntry);
  const isMcpToolRow =
    workEntry.itemType === "mcp_tool_call" &&
    !isGitHubToolRow &&
    !isQuantumBrowserToolRow &&
    !isQuantumToolRow;
  ...
  const preview =
    isQuantumBrowserToolRow || isQuantumToolRow
      ? sanitizeQuantumMcpToolPreview({
          preview: rawPreview,
          heading,
          status: toolWorkEntryStatus(workEntry),
