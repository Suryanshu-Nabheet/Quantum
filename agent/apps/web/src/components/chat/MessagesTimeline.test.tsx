...
import { CheckpointRef, MessageId, ThreadId, TurnId } from "@quantum/contracts";
...
describe("MessagesTimeline", () => {
  ...
  it("labels only the first message when another task created the conversation", async () => {
    ...
    expect(markup).toContain("Sent by Quantum from another thread");
    expect(markup).toContain('aria-label="Open source thread"');
    expect(markup.indexOf("Sent by Quantum from another thread")).toBeLessThan(
  ...
  it("shows the Quantum mark for every provider-specific tool row shape", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    ...
    const claudeMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-quantum-claude",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-quantum-claude",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "dynamic_tool_call",
              toolTitle: "Quantum__quantum_create_thread",
              toolName: "Quantum__quantum_create_thread",
              detail: "Quantum__quantum_create_thread",
    ...
    expect(claudeMarkup).toContain('data-tool-icon="quantum"');
    expect(claudeMarkup).not.toContain('data-tool-icon="mcp"');
    expect(claudeMarkup).toContain("Quantum is creating a thread");
    expect(claudeMarkup).not.toContain("Quantum__quantum_create_thread");
    ...
    const codexMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-quantum-codex",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-quantum-codex",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "file_change",
              toolTitle: "mcp__Quantum__quantum_list_threads",
              detail: "mcp__Quantum__quantum_list_threads",
            },
          },
        ]}
      />,
    );
    expect(codexMarkup).toContain('data-tool-icon="quantum"');
    expect(codexMarkup).toContain("Quantum listed threads");
    expect(codexMarkup).not.toContain("mcp__Quantum__quantum_list_threads");
    ...
    const failedMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          {
            id: "entry-inline-quantum-failed",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-quantum-failed",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "mcp_tool_call",
              toolName: "mcp__quantum__quantum_create_threads",
              toolStatus: "failed",
    ...
    expect(failedMarkup).toContain("Quantum couldn&#x27;t create threads");
    expect(failedMarkup).toContain("Claude rejected reasoningEffort");
  });

  // Browser calls get the globe rather than the generic Quantum mark: a browsing
  // row is about a page, and the surface it acted on is the first thing to read.
  it("uses the browser icon and action name for Quantum browser calls", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...makeTimelineBaseProps()}
        timelineEntries={[
          {
            id: "entry-inline-quantum-browser",
            kind: "work",
            createdAt: "2026-03-17T19:12:28.000Z",
            entry: {
              id: "work-inline-quantum-browser",
              createdAt: "2026-03-17T19:12:28.000Z",
              label: "MCP tool call",
              tone: "tool",
              itemType: "mcp_tool_call",
    ...
    expect(markup).not.toContain("Quantum: Browser Open");
  ...
  it("hides raw `ToolName: {json}` argument details behind the humanized heading", async () => {
    ...
    const readThreadMarkup = renderSingleToolRow({
      id: "work-quantum-read-thread-args",
      createdAt: "2026-03-17T19:12:28.000Z",
      label: "MCP tool call",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolName: "mcp__quantum__quantum_read_thread",
      detail: 'mcp__quantum__quantum_read_thread: {"threadId":"c357d8c5-b4c1-47d0"}',
      activityKind: "tool.completed",
    });
    expect(readThreadMarkup).toContain("Quantum read a thread");
    expect(readThreadMarkup).not.toContain("mcp__quantum__quantum_read_thread:");
    expect(readThreadMarkup).not.toContain("threadId");

    const diagnoseMarkup = renderSingleToolRow({
      id: "work-quantum-diagnose-args",
      createdAt: "2026-03-17T19:12:28.000Z",
      label: "MCP tool call",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolName: "mcp__quantum__quantum_diagnose_thread",
      detail: 'mcp__quantum__quantum_diagnose_thread: {"threadId":"09a1615d-084f-40b9"}',
      activityKind: "tool.completed",
    });
    expect(diagnoseMarkup).toContain("Quantum diagnosed a thread");
    expect(diagnoseMarkup).not.toContain("mcp__quantum__quantum_diagnose_thread:");
    ...
    const dynamicToolMarkup = renderSingleToolRow({
      ...
      detail: 'ToolSearch: {"query":"select:mcp__quantum__quantum_read_thread_events"}',
      activityKind: "tool.completed",
    });
    ...
    expect(failedArgsMarkup).toContain("Quantum couldn&#x27;t create threads");
    expect(failedArgsMarkup).toContain("Invalid params");
  });

  it("keeps Quantum tool calls and adds a thread creation recap at the end of the turn", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const assistantMessageId = MessageId.makeUnsafe("message-quantum-recap");
    const workEntries = [
      {
        id: "entry-quantum-create-tool",
        kind: "work",
        createdAt: "2026-03-17T19:12:28.000Z",
        entry: {
          id: "work-quantum-create-tool",
          createdAt: "2026-03-17T19:12:28.000Z",
          label: "MCP tool call",
          tone: "tool",
          itemType: "mcp_tool_call",
          toolName: "mcp__quantum__quantum_create_threads",
          toolTitle: "Quantum created threads",
          activityKind: "tool.completed",
        },
      },
      {
        id: "entry-quantum-create-recap",
        kind: "work",
        createdAt: "2026-03-17T19:12:29.000Z",
        entry: {
          id: "work-quantum-create-recap",
          createdAt: "2026-03-17T19:12:29.000Z",
          label: "Created 2 Quantum threads",
          tone: "info",
          activityKind: "quantum.threads.created",
          quantumThreadCreation: {
    ...
    expect(liveMarkup).toContain("Quantum created threads");
    expect(liveMarkup).not.toContain('data-quantum-thread-creation-card="true"');
    ...
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        timelineEntries={[
          ...workEntries,
          {
            id: "entry-quantum-recap-assistant",
            kind: "message",
            createdAt: "2026-03-17T19:12:30.000Z",
            message: {
              id: assistantMessageId,
    ...
    expect(markup).toContain("Worked for");
    expect(markup).toContain('data-quantum-thread-creation-card="true"');
    ...
    expect(markup.indexOf("Both threads are running.")).toBeLessThan(
      markup.indexOf('data-quantum-thread-creation-card="true"'),
    );
  });
          ...
          new Map([
            [
              assistantMessageId,
              {
                ...
                checkpointRef: CheckpointRef.makeUnsafe("refs/quantum/checkpoints/thread/turn/1"),
                status: "ready",
                completedAt: "2026-03-17T19:12:30.000Z",
                assistantMessageId,
                files: [
