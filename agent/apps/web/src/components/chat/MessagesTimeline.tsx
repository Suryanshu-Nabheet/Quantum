...
import { QuantumThreadCreationCard } from "./QuantumThreadCreationCard";
...
interface MessagesTimelineProps {
  ...
  /** Provenance for a conversation created from another Quantum task. */
    ...
    >
      ...
      {row.kind === "work" &&
        (() => {
          ...
          // The provider's actual Quantum MCP tool rows remain visible here.
          const groupedEntries = row.groupedEntries.filter(
            (workEntry) => !workEntry.quantumThreadCreation,
          );
            ...
            <div className="flex w-full flex-col gap-3">
              ...
              <div className="flex w-full justify-end">
                ...
                >
                  ...
                  {/* The cross-task origin label already attributes this turn to another Quantum thread,
      ...
      {row.kind === "message" &&
        ...
        (() => {
          ...
          const buildWorkDisplay = (workEntries: WorkLogEntry[], workGroupId: string | null) => {
            const displayEntries = workEntries.filter((entry) => !entry.quantumThreadCreation);
          ...
          const quantumThreadCreationRecaps = [
            ...new Map(
              allTurnWorkEntries.flatMap((entry) =>
                entry.quantumThreadCreation
                  ? [
                      [
                        entry.quantumThreadCreation.operationId,
                        entry.quantumThreadCreation,
                      ] as const,
                    ]
                  : [],
          ...
          const collapsedTurnItems = row.collapsedTurnItems?.filter(
            (item) => item.kind !== "work" || !item.entry.quantumThreadCreation,
          );
            ...
            <>
              ...
              <div className="group min-w-0 py-0.5">
                ...
                {!row.assistantTurnInProgress && row.showAssistantCopyButton
                  ? quantumThreadCreationRecaps.map((creation) => (
                      <div key={creation.operationId} className="mt-2 mb-4">
                        <QuantumThreadCreationCard
                          creation={creation}
                          {...(onOpenThread
                            ? {
