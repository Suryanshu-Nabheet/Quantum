...
import { normalizeModelSlug } from "@quantum/shared/model";
import { buildQuantumBranchName } from "@quantum/shared/git";
import { isGenericChatThreadTitle } from "@quantum/shared/chatThreads";
import { isGenericTerminalThreadTitle } from "@quantum/shared/terminalThreads";
import {
  type ChatMessage,
  type SessionPhase,
  type Thread,
...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "quantum:last-invoked-script-by-project";
export const DISMISSED_PROVIDER_HEALTH_BANNERS_KEY = "quantum:dismissed-provider-health-banners";
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for Quantum, then try again.";
  }
...
export function shouldShowComposerModelBootstrapSkeleton(input: {
  ...
  if (persistedSelection && persistedSelection.provider !== input.selectedProvider) {
    return true;
  }

  if (input.requiresDiscoveredModels === true && input.providerModelsLoading) {
    return !persistedSelection && !draftSelection;
  }
...
export function buildSuggestedWorktreeName(input: {
  ...
  return buildQuantumBranchName(input.associatedWorktreeBranch ?? input.title);
}
