...
import type { DeviceUdid } from "@quantum/contracts";
import type { DeviceFrame } from "@quantum/shared/deviceFrame";
...
export function useDeviceVideoStream(input: {
  ...
  useEffect(() => {
    ...
    const configureDecoder = (frame: DeviceFrame) => {
      ...
      if (!codec) {
        failStream("The simulator stream sent parameters Quantum could not read.");
        return;
      }
      teardownDecoder();
      ...
      setStatus({
        ...
        message:
          reason === "decode-failed"
            ? "The simulator stream sent a frame Quantum could not read."
