...
import { isGenericChatThreadTitle } from "@quantum/shared/chatThreads";
import { getDefaultModel } from "@quantum/shared/model";
import { pluralize } from "@quantum/shared/text";
import { resolveThreadWorkspaceCwd } from "@quantum/shared/threadEnvironment";
...
type DebugFeatureFlagsWindow = Window & {
  quantumShowFeatureFlags?: () => void;
  quantumHideFeatureFlags?: () => void;
};
      ...
      <Menu>
        ...
        <ComposerPickerMenuPopup align="end" side="top" className="w-64 min-w-64">
          ...
          <MenuGroup>
            ...
            <MenuItem
              className={SIDEBAR_CONTEXT_MENU_ITEM_CLASS_NAME}
              onClick={() => openExternalLink(QUANTUM_DOCS_URL)}
            >
 ...
 */
const ACTIVITY_ONBOARDING_STORAGE_KEY = "quantum:activity-onboarding:v1";
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    debugWindow.quantumShowFeatureFlags = showFeatureFlags;
    debugWindow.quantumHideFeatureFlags = hideFeatureFlags;
    window.addEventListener("storage", updateVisibility);
    updateVisibility();

    return () => {
      window.removeEventListener("storage", updateVisibility);
      if (debugWindow.quantumShowFeatureFlags === showFeatureFlags) {
        delete debugWindow.quantumShowFeatureFlags;
      }
      if (debugWindow.quantumHideFeatureFlags === hideFeatureFlags) {
        delete debugWindow.quantumHideFeatureFlags;
      }
    };
  }, []);
  ...
  // "Quantum" for project-less chats) with the hover cards and Activity rows.
  ...
  function renderProjectItem(
    ...
    // A project reads as "running" when Quantum tracks a run for it or when a
    // local server (possibly started outside Quantum) is attributed by cwd.
  ...
  const searchPaletteActions = useMemo<SidebarSearchAction[]>(
    () => [
      ...
      {
        id: "feedback",
        label: "Feedback Quantum",
        description: "Send feedback or report an issue to the Quantum team.",
        keywords: ["feedback", "bug", "issue", "problem", "report", "support", "quantum"],
      },
      {
        id: "settings",
        label: "Settings",
        ...
        .then((nextState) => {
          ...
          if (nextState.status === "available") {
            toastManager.add({
              ...
              description: `Quantum is preparing version ${nextState.availableVersion ?? "available"} in the background.`,
            });
            return;
          }
          ...
          if (nextState.status === "downloading") {
            toastManager.add({
              type: "info",
              title: "Preparing update",
              description: "Quantum is downloading the update in the background.",
            });
            return;
          }
          ...
          if (nextState.status === "up-to-date") {
            toastManager.add({
              ...
              description: `Quantum ${nextState.currentVersion} is already the newest version.`,
            });
            return;
          }
