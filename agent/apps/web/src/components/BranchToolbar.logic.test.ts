import type { GitBranch } from "@quantum/contracts";
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.quantum/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.quantum/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("keeps legacy .quantum worktree paths working for migrated threads", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.quantum/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/quantum-pi",
        existingAssociatedWorktreeBranch: "quantum/pi",
        existingAssociatedWorktreeRef: "quantum/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/quantum-pi",
      associatedWorktreeBranch: "quantum/pi",
      associatedWorktreeRef: "quantum/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.quantum/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.quantum/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.quantum/worktrees/feature-b",
      nextWorktreePath: "/repo/.quantum/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.quantum/worktrees/feature-a",
      nextWorktreePath: "/repo/.quantum/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
