...
import { DEVICE_CAPABILITY_LABELS } from "@quantum/contracts";
  ...
  switch (availability.kind) {
    ...
    case "unsupported-platform":
      return {
        ...
        description: `This Quantum server runs on ${availability.platform}. Simulators are only available when the server runs on a Mac with Xcode installed.`,
        steps: [],
        retryable: false,
      };
    case "setup-required":
 ...
 * destination Quantum can send them to — every other step either completes on
