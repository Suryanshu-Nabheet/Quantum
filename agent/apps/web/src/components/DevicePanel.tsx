...
export default function DevicePanel(props: {
  ...
  useEffect(() => {
    ...
    // accepting its licence or downloading a runtime all happen outside Quantum
        ...
        try {
          if (entry.action.kind === "boot-then-attach") {
            ...
            if (result.kind === "boot-limit-reached") {
              ...
              setBootLimit({
                limit: result.limit,
                candidates: result.quantumBooted,
                pendingUdid: udid,
    ...
    (candidate: DeviceDescriptor) => {
      ...
      void runDeviceAction(async () => {
        try {
          ...
          if (result.kind === "boot-limit-reached") {
            setPendingDevice(null);
            setBootLimit({ ...pending, limit: result.limit, candidates: result.quantumBooted });
            return;
          }
  ...
  const handleKey = useCallback(
    (event: React.KeyboardEvent<HTMLCanvasElement>, direction: "down" | "up") => {
      ...
      // Every other Cmd chord belongs to Quantum (Cmd+W, Cmd+R, the dock
  ...
  if (status.kind === "unsupported") {
    return (
      <p className="text-balance text-center text-[10px] text-white/70 leading-snug">
        ...
        WebCodecs video decoder Quantum uses.
      </p>
    );
  }
    ...
    <Dialog open={state !== null} onOpenChange={(open) => (open ? undefined : props.onDismiss())}>
      <DialogPopup>
        <DialogHeader>
          ...
          */}
          <DialogDescription>
            Quantum keeps at most {state?.limit ?? 0} simulators running at once, because each one
