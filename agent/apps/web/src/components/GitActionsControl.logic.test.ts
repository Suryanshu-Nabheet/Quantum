import type { GitStatusResult } from "@quantum/contracts";
...
describe("resolveDefaultCreateBranchName", () => {
  it("uses Quantum as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "quantum/fix-toast-copy");
  });

  it("normalizes an existing legacy quantum namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "quantum/refine-toolbar-actions");
    assert.equal(branch, "quantum/refine-toolbar-actions");
  });

  it("preserves nested namespaces under Quantum", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "quantum/feature/refine-toolbar-actions");
  });

  it("increments suffix when the Quantum branch already exists", () => {
    const branch = resolveDefaultCreateBranchName(
      ["main", "quantum/fix-toast-copy", "quantum/fix-toast-copy-2"],
      "fix toast copy",
    );
    assert.equal(branch, "quantum/fix-toast-copy-3");
  });

  it("falls back to quantum/update when no preferred name is provided", () => {
    const branch = resolveDefaultCreateBranchName(["main"]);
    assert.equal(branch, "quantum/update");
  });
});
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "quantum/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "quantum/deadbeef";
