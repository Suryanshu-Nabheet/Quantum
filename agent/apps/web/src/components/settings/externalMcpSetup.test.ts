...
describe("external MCP guided setup", () => {
  it("builds copyable Codex and Claude Code commands without embedding a credential", () => {
    ...
    expect(codex.value).toBe(
      "codex mcp add quantum --env ELECTRON_RUN_AS_NODE=1 -- /Applications/Quantum.app/Contents/MacOS/Quantum server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Quantum home'",
    );
    expect(claude.value).toBe(
      "claude mcp add --scope user quantum -e ELECTRON_RUN_AS_NODE=1 -- /Applications/Quantum.app/Contents/MacOS/Quantum server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Quantum home'",
    );
  ...
  it("builds standard JSON configuration for desktop and other clients", () => {
    ...
    const parsed = JSON.parse(desktop.value) as {
      mcpServers: { quantum: { command: string; args: ReadonlyArray<string> } };
    };

    expect(desktop.format).toBe("json");
    expect(parsed.mcpServers.quantum).toEqual(stdio);
  });
  ...
  it("builds a project-specific prompt without exposing implementation identifiers", () => {
    const prompt = buildExternalMcpExamplePrompt("Quantum app");

    expect(prompt).toContain('project named "Quantum app"');
  ...
  it("builds one agent-facing setup prompt covering pairing, registration, and verification", () => {
    const prompt = buildExternalMcpSetupPrompt({
      setupCommand: "quantum mcp pair --code syn_pair_v1_example --home-dir /tmp/home",
      stdio,
    });

    expect(prompt).toContain("syn_pair_v1_example");
    expect(prompt).toContain("codex mcp add quantum");
    expect(prompt).toContain("claude mcp add --scope user quantum");
    expect(prompt).toContain('"mcpServers"');
    expect(prompt).toContain("quantum_overview");
  ...
  it("describes scopes without exposing capability identifiers", () => {
    ...
    expect(description).toBe("Create and follow its own tasks · Use the shared project checkout");
