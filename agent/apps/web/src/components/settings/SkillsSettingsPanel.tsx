...
// catalog (~/.quantum/skills plus each provider's skills folder), shows which provider
...
import type { ProviderKind, ServerSettings } from "@quantum/contracts";
...
export function SkillsSettingsPanel() {
  ...
  const quantumSkillsDir = catalogQuery.data?.quantumSkillsDir;

  return (
    <div className="space-y-8">
      <SettingsSection title="Portable skills">
        <SettingsRow
          title="Quantum skills folder"
          description="Skills placed here are available on every provider. When a provider already ships its own copy of a skill, that copy is used; otherwise Quantum's copy is the fallback."
          status={
            quantumSkillsDir ? (
      ...
      {catalogQuery.isError ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Quantum could not scan the skill folders. Retry after checking that the server is running."
          />
        </SettingsSection>
      ) : null}
      ...
      {!catalogQuery.isLoading && !catalogQuery.isError && totalSkills === 0 ? (
        <SettingsSection title="Skills">
          <SettingsRow
            ...
            description="Add a skill folder containing a SKILL.md to the Quantum skills folder above, or install skills for any supported provider."
          />
        </SettingsSection>
      ) : null}
