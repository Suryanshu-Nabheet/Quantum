...
export function buildExternalMcpClientConfiguration(
  ...
  if (client === "codex") {
    ...
    return {
      format: "command",
      value: shellCommand(
        ["codex", "mcp", "add", "quantum", ...environment, "--", stdio.command, ...stdio.args],
        platform,
      ),
  ...
  if (client === "claudeCode") {
    ...
    return {
      ...
      instruction: /win/i.test(platform)
        ? "Run this command in PowerShell. Claude Code will make Quantum available in all your projects."
        : "Run this command in Terminal. Claude Code will make Quantum available in all your projects.",
    };
  }
...
export function buildExternalMcpExamplePrompt(projectTitle: string | null): string {
  return [
    projectTitle === null
      ? "Use Quantum to create a new task: call quantum_overview first, pick the most relevant project, and tell me which one you chose."
      : `Use Quantum to create a new task in the project named ${JSON.stringify(projectTitle)}.`,
    "First inspect Quantum's capabilities and choose an exact available provider and model; do not guess model names.",
...
// another MCP-capable app). The agent pairs the machine, registers Quantum in
...
export function buildExternalMcpSetupPrompt(input: {
  ...
  const sections: string[] = [
    "Connect this coding agent to Quantum via MCP. Complete every step yourself, in order, and report what happened.",
  ];
  ...
  sections.push(
    [
      'Step 2 — Register Quantum as a stdio MCP server named "quantum" in your own configuration, using whichever mechanism your app supports:',
      "",
    ...
    'Step 3 — Verify. Reload your MCP servers if needed, then call the "quantum_overview" tool and summarize the projects, providers, and permissions it returns.',
  );
