...
import { describeErrorMessage } from "@quantum/shared/errorMessages";
import {
  TERMINAL_MAX_COLS,
  TERMINAL_MAX_ROWS,
  TERMINAL_MIN_COLS,
...
import type { TerminalSessionSnapshot } from "@quantum/contracts";
...
const TERMINAL_PARKING_CONTAINER_ID = "quantum-terminal-parking";

type QuantumTerminalOptions = NonNullable<ConstructorParameters<typeof Terminal>[0]> & {
...
const TERMINAL_CURSOR_STYLE: NonNullable<QuantumTerminalOptions["cursorStyle"]> = "bar";
const TERMINAL_INACTIVE_CURSOR_STYLE: NonNullable<QuantumTerminalOptions["cursorInactiveStyle"]> =
  "bar";
...
function syncTheme(entry: TerminalRuntimeEntry): void {
  ...
  const terminalOptions = entry.terminal.options as QuantumTerminalOptions;
...
export function createRuntimeEntry(config: TerminalRuntimeConfig): TerminalRuntimeEntry {
  ...
  const terminalOptions: QuantumTerminalOptions = {
    cursorBlink: true,
