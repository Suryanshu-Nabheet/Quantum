...
import { localServerPrimaryLabel } from "@quantum/shared/localServers";
import { resolveDesktopDipRectFromCssRect } from "@quantum/shared/desktopChrome";
import {
  BROWSER_BLANK_URL,
  isBlankBrowserTabUrl,
  resolveCopyableBrowserTabUrl,
} from "@quantum/shared/browserSession";
import {
  BROWSER_COPY_LINK_TOAST_TITLE,
  isBrowserCopyLinkChord,
} from "@quantum/shared/browserShortcuts";
...
const BROWSER_WEBVIEW_PARTITION = "persist:quantum-browser";
...
function isBrowserPerfLoggingEnabled(): boolean {
  ...
  try {
    return window.localStorage.getItem("quantum:browser-perf") === "1";
  } catch {
    return false;
  }
}
...
export function BrowserPanel({
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      console.info(`[${QUANTUM_BROWSER_LABEL} panel perf]`, {
        threadId,
