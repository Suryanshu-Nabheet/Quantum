...
function buildFingerprint({ arch, release, sources, targets }) {
  ...
  hash.update("quantum-appsnap-helper-build-v1\0");
  hash.update(arch);
  hash.update("\0");
...
export function buildAppSnapHelper({
  ...
  const temporaryDirectory = mkdtempSync(join(tmpdir(), "quantum-appsnap-helper-"));
  ...
  try {
    ...
    for (const target of targets) {
      const thinBinary = join(temporaryDirectory, `quantum-appsnap-helper-${target.arch}`);
    ...
    const unsignedBinary = join(temporaryDirectory, "quantum-appsnap-helper");
