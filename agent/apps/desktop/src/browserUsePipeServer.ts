...
import type { BrowserToolName, ThreadId } from "@quantum/contracts";
...
const PIPE_DIR = "quantum-browser-host";
const PIPE_NAME_PREFIX = "quantum-browser-host";

export const QUANTUM_BROWSER_HOST_PIPE_ENV = "QUANTUM_BROWSER_HOST_PIPE_PATH";
export const QUANTUM_BROWSER_HOST_CAPABILITY_ENV = "QUANTUM_BROWSER_HOST_CAPABILITY";
export const QUANTUM_BROWSER_HOST_CAPABILITY_FD_ENV = "QUANTUM_BROWSER_HOST_CAPABILITY_FD";
/** @deprecated Read/written only while old backend builds are still supported. */
export const QUANTUM_BROWSER_USE_PIPE_ENV = "QUANTUM_BROWSER_USE_PIPE_PATH";
...
export function resolveConfiguredBrowserHostPipePath(
  ...
  const configured =
    env[QUANTUM_BROWSER_HOST_PIPE_ENV]?.trim() || env[QUANTUM_BROWSER_USE_PIPE_ENV]?.trim();
...
export const QUANTUM_BROWSER_HOST_PIPE_PATH = resolveConfiguredBrowserHostPipePath();
/** @deprecated Compatibility alias for old packaged backend builds. */
export const QUANTUM_BROWSER_USE_PIPE_PATH = QUANTUM_BROWSER_HOST_PIPE_PATH;
...
export function resolveBrowserHostPipeBackendEnv(
  ...
  delete backendEnv[QUANTUM_BROWSER_HOST_PIPE_ENV];
  delete backendEnv[QUANTUM_BROWSER_USE_PIPE_ENV];
  delete backendEnv[QUANTUM_BROWSER_HOST_CAPABILITY_ENV];
  delete backendEnv[QUANTUM_BROWSER_HOST_CAPABILITY_FD_ENV];
  const pipePath = activePipePath?.trim();
  if (pipePath && Number.isInteger(capabilityFd) && (capabilityFd ?? 0) >= 3) {
    backendEnv[QUANTUM_BROWSER_HOST_PIPE_ENV] = pipePath;
    backendEnv[QUANTUM_BROWSER_USE_PIPE_ENV] = pipePath;
    backendEnv[QUANTUM_BROWSER_HOST_CAPABILITY_FD_ENV] = String(capabilityFd);
  }
  return backendEnv;
}
...
export class BrowserHostPipeServer {
  ...
  constructor(
    ...
    options: BrowserHostPipeServerOptions | string = QUANTUM_BROWSER_HOST_PIPE_PATH,
  ) {
    const normalized = typeof options === "string" ? { pipePath: options } : options;
    this.platform = normalized.platform ?? process.platform;
    this.pipePath = normalized.pipePath ?? QUANTUM_BROWSER_HOST_PIPE_PATH;
  ...
  private getInfo(client: PipeClient, params: unknown): unknown {
    ...
    return {
      name: "Quantum Browser Host",
      version: "1.0.0",
      type: "quantum-browser-host",
      metadata: {
        sessionId,
        protocolVersion: 1,
