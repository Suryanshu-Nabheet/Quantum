...
import type { ContextMenuItem } from "@quantum/contracts";
import { isKeyboardShortcutsHelpChord } from "@quantum/shared/browserShortcuts";
import { getMacTrafficLightPosition } from "@quantum/shared/desktopChrome";
import { DEVICE_HELPER_SOURCE_DIR_ENV } from "@quantum/shared/deviceHelperCache";
import {
  QUANTUM_DESKTOP_UPDATE_CHANNEL,
  resolveQuantumDesktopFlavor,
  quantumDesktopIdentity,
} from "@quantum/shared/desktopIdentity";
import { NetService } from "@quantum/shared/Net";
import { applyShellEnvironmentHydrationMarker } from "@quantum/shared/shell";
import { RotatingFileSink } from "@quantum/shared/logging";
import { ensureStaticSnapshot, findAsarArchivePath } from "@quantum/shared/staticSnapshot";
...
import {
  ...
  QUANTUM_BROWSER_HOST_PIPE_PATH,
...
import {
  ...
  resolveQuantumStorageSnapshotPath,
...
// `BASE_DIR` prefers QUANTUM_HOME, which the Windows registry read hydrates whenever the
...
const desktopFlavor = resolveQuantumDesktopFlavor({
  isDevelopment,
  requestedFlavor: process.env.QUANTUM_DESKTOP_FLAVOR,
});
const desktopIdentity = quantumDesktopIdentity(desktopFlavor);
const BASE_DIR =
  process.env.QUANTUM_HOME?.trim() ||
...
const BACKEND_MAX_OLD_SPACE_ENV_KEYS = ["QUANTUM_BACKEND_MAX_OLD_SPACE_MB"] as const;
...
const QUANTUM_BROWSER_LABEL = "Quantum browser";
const browserPerfLoggingEnabled = process.env.QUANTUM_BROWSER_PERF === "1";
...
function startBrowserPerformanceLogging(): void {
  ...
  browserPerfInterval = setInterval(() => {
    ...
    console.info(`[${QUANTUM_BROWSER_LABEL} perf]`, {
      ...snapshot.counters,
...
async function ensureBrowserHostPipeServer(): Promise<void> {
  if (browserHostPipeServer || !QUANTUM_BROWSER_HOST_PIPE_PATH) {
    return;
  }
...
async function reserveBackendEndpoint(reason: string): Promise<void> {
  ...
  process.env.QUANTUM_DESKTOP_WS_URL = backendWsUrl;
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { quantumCommitHash?: unknown };
    return normalizeCommitHash(parsed.quantumCommitHash);
  } catch {
    return null;
  }
}

declare const __QUANTUM_WINDOWS_UPDATER_PUBLISHER__: string;
...
function resolveEmbeddedWindowsPublisherSubjects(): string[] {
  ...
  const subject = __QUANTUM_WINDOWS_UPDATER_PUBLISHER__.trim();
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.QUANTUM_COMMIT_HASH);
  if (envCommitHash) {
...
async function handleDesktopMigrationRecovery(): Promise<DesktopMigrationRecoveryOutcome> {
  ...
  const outcome = await recoverDesktopMigrationIfRequired({
    ...
    choose: async ({ previousFailure }) => {
      // The user is here because Quantum cannot open its database, so the
      ...
      if (canInstallUpdate) {
        choices.push({
          label: "Update Quantum and restart",
          detail: "install the newest Quantum release, which may already contain the fix",
      ...
      if (releaseUrl !== null) {
        choices.push({
          ...
          detail: `${canInstallUpdate ? "download that release" : "download the latest Quantum release"} in a browser`,
      ...
      const result = await dialog.showMessageBox({
        ...
        title:
          previousFailure === null
            ? "Quantum needs to recover its database"
            : restoreFailed
              ? "Migration recovery failed"
              : "Quantum could not update itself",
        message:
          previousFailure === null
            ? "Quantum stopped a database migration before it could finish safely."
            : restoreFailed
              ? "The saved database backup could not be restored."
              : "The newest Quantum release could not be installed.",
...
// quantum:// protocol here and, via QUANTUM_STATIC_DIR, for the backend's HTTP static
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Quantum failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
...
function hasConfiguredUpdateFeed(): boolean {
  return readAppUpdateYml() !== null || Boolean(process.env.QUANTUM_DESKTOP_MOCK_UPDATES);
}
...
function resolveAutoUpdateDisabledReason(): string | null {
  return getAutoUpdateDisabledReason({
    ...
    disabledByEnv:
      desktopIdentity.usesScriptedUpdates || process.env.QUANTUM_DISABLE_AUTO_UPDATE === "1",
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Quantum ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
    ...
    void dialog.showMessageBox({
      type: "info",
      title: "Update found",
      message: "Quantum is preparing the update in the background.",
      buttons: ["OK"],
    });
...
function resolveNotificationIconPath(): string | null {
  ...
  if (process.platform === "win32") {
    return resolveResourcePath("quantum.png") ?? resolveIconPath("ico");
  }
  return resolveResourcePath("quantum.png") ?? resolveIconPath("png");
}

function resolveAppSnapHelperPath(): string {
  if (app.isPackaged) {
    return Path.resolve(process.resourcesPath, "..", "Helpers", "quantum-appsnap-helper");
  }
  return Path.resolve(__dirname, "..", ".electron-runtime", "appsnap", "quantum-appsnap-helper");
}
 ...
 * package.json. We override it to a clean lowercase Quantum name.
 */
...
function repairBrowserProfileBeforeElectronReady(userDataPath: string): void {
  ...
  if (browserProfileRepair.status === "repaired") {
    console.info("[desktop] Completed Quantum browser profile bridge repair", {
    ...
    console.warn("[desktop] Failed to complete Quantum browser profile bridge repair", {
      sourcePath: browserProfileRepair.sourcePath,
      targetPath: browserProfileRepair.targetPath,
...
function configureAppIdentity(): void {
  ...
  const aboutIconPath =
    resolveResourcePath("app-icon-macos.png") ?? resolveResourcePath("icon.png");
  app.setAboutPanelOptions({
    applicationName: APP_DISPLAY_NAME,
    applicationVersion: app.getVersion(),
    version: commitHash ?? "unknown",
    copyright: `© ${new Date().getFullYear()} Suryanshu Nabheet. MIT License.`,
...
function restartAfterStartupBundleSwap(error: BundleChangedDuringStartupError): void {
  ...
  void dialog
    .showMessageBox({
      type: "warning",
      title: "Quantum needs to restart",
      message: "Quantum changed while it was opening.",
      detail:
        "The current process cannot safely read the replaced application bundle. Restart Quantum to finish opening with one consistent version.",
      buttons: ["Restart Quantum"],
      defaultId: 0,
    })
    .catch(() => undefined)
    .then(() => {
...
// every archive read in this process — the quantum:// protocol, the backend's static
...
function startBundleSwapWatcher(): void {
  ...
  bundleSwapPollTimer = setInterval(() => {
    ...
    void dialog
      .showMessageBox({
        type: "warning",
        title: "Quantum was replaced on disk",
        message: "The installed Quantum app changed while it was running.",
        detail:
...
function processInstallMarkerOnStartup(): void {
  ...
  const message = `Quantum restarted, but update ${marker.toVersion} was not installed. Try again.`;
  setUpdateState(
    reduceDesktopUpdateStateOnInstallRestartFailure(
      updateState,
      marker.toVersion,
...
async function installLatestUpdateForMigrationRecovery(): Promise<string | null> {
  ...
  if (updateState.status === "up-to-date") {
    return `Quantum ${app.getVersion()} is already the newest release, so updating cannot repair this database.`;
  }
  if (updateState.status !== "downloaded") {
...
function configureAutoUpdater(): void {
  ...
  // default feed while Quantum versions advance independently.
  autoUpdater.channel = QUANTUM_DESKTOP_UPDATE_CHANNEL;
...
// Builds process-local Node args so provider/tool children do not inherit Quantum's heap guard.
...
function backendEnv(): NodeJS.ProcessEnv {
  ...
  const env: NodeJS.ProcessEnv = {
    ...resolveBrowserHostPipeBackendEnv(
      process.env,
      browserHostPipeServer ? QUANTUM_BROWSER_HOST_PIPE_PATH : null,
    ...
    // quantum:// protocol serves, so both surfaces survive app.asar being replaced.
    ...(servedStaticRoot?.snapshotted ? { QUANTUM_STATIC_DIR: servedStaticRoot.dir } : {}),
    ...(app.isPackaged
      ? { [DEVICE_HELPER_SOURCE_DIR_ENV]: Path.join(process.resourcesPath, "device-helper") }
      : {}),
    QUANTUM_MODE: "desktop",
    QUANTUM_NO_BROWSER: "1",
    QUANTUM_PORT: String(backendPort),
    QUANTUM_HOME: BASE_DIR,
    QUANTUM_AUTH_TOKEN: backendAuthToken,
    QUANTUM_DESKTOP_SHUTDOWN_TOKEN: DESKTOP_BACKEND_SHUTDOWN_TOKEN,
  };
...
function backendFailureDialogDetail(reason: string): string {
  ...
  return [
    cause,
    "Quantum paused automatic restarts so a failing backend can't keep respawning in the background.",
...
function presentBackendStartupGiveUp(reason: string): void {
  ...
  const task = (async () => {
    for (;;) {
      const result = await dialog.showMessageBox({
        type: "error",
        title: "Quantum's backend didn't start",
        message: `Quantum's backend failed to start ${BACKEND_MAX_CONSECUTIVE_START_FAILURES} times in a row.`,
        detail,
...
function handleBackendStartupBlock(block: BackendStartupBlock): void {
  ...
  const task = (async () => {
    if (block.kind === "migration-recovery-required") {
      const result = await dialog.showMessageBox({
        type: "warning",
        title: "Quantum needs to recover its database",
        message: "A database migration did not finish safely.",
        detail:
          "Restart Quantum to open the verified backup recovery flow. Provider and chat processes will remain stopped until recovery completes.",
    ...
    const processDetail =
      block.ownerPid === null
        ? "Another Quantum server is already using this database."
        : `Another Quantum server (process ${block.ownerPid}) is already using this database.`;
    const result = await dialog.showMessageBox({
      type: "warning",
      title: "Quantum is already running elsewhere",
      message: "Your local Quantum data is in use by another process.",
      detail: `${processDetail}\n\nStop the other Quantum app or development server, then try again. Your data has not been changed.`,
...
function startBackend(trigger: BackendStartTrigger = "lifecycle"): void {
  ...
  const child = ChildProcess.spawn(process.execPath, [...backendNodeArgs(), backendEntry], {
    ...
    env: {
      ...
      QUANTUM_SERVER_ENTRY: backendEntry,
    },
...
function getTitleBarOptions(): BrowserWindowConstructorOptions {
  ...
  return {
    ...
    // Derived from the shared chat-surface header geometry (@quantum/shared/desktopChrome)
...
function createWindow(): BrowserWindow {
  ...
  if (isDevelopment) {
    ...
    if (process.env.QUANTUM_DEV_OPEN_DEVTOOLS === "1") {
...
function presentRendererCrashRecovery(
  ...
  const message =
    response.cause === "reload-budget-exhausted"
      ? `Quantum's window crashed ${response.crashes} times in a row.`
      : "Quantum's window stopped unexpectedly.";
  const detail = [
    `The window's renderer process exited (${reason}).`,
    response.cause === "reload-budget-exhausted"
      ? "Quantum paused automatic reloads so a repeating crash can't keep reloading in the background."
      : "This exit reason repeats on reload, so Quantum did not retry automatically.",
  ...
  const task = (async () => {
    for (;;) {
      const result = await dialog.showMessageBox({
        type: "error",
        title: "Quantum's window stopped",
        message,
        detail,
...
function configureMediaPermissions(): void {
  ...
  const permissionTargets = [
    ...
    },
    {
      ...
      // microphone grant used by Quantum's own voice-composer renderer.
...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    console.warn("[Quantum browser] Failed to start browser host pipe", error);
  }
  startBackend();
