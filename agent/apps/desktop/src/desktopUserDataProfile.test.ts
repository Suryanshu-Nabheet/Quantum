...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "quantum-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical Quantum profile names", () => {
    const appDataBase = "/Users/tester/Library/Application Support";
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "quantum-dev" })).toBe(
      "/Users/tester/Library/Application Support/quantum-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "quantum" })).toBe(
      "/Users/tester/Library/Application Support/quantum",
    );
    expect(
      resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "quantum-canary" }),
    ).toBe("/Users/tester/Library/Application Support/quantum-canary");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "quantum");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "quantum-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "quantum-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Quantum profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Quantum database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "quantum");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".quantum-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "quantum");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "quantum-profile-seed.json"), "{");
