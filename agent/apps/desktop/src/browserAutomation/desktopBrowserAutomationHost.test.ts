import { ThreadId, type BrowserElementRef, type BrowserSnapshotId } from "@quantum/contracts";
...
const createWebContents = () => {
  ...
  const sendCommand = vi.fn(async (method: string, params?: Record<string, unknown>) => {
    ...
    if (method === "Runtime.evaluate") {
      ...
      if (
        expression.includes('const key = "__quantumBrowserAutomationV1"') &&
      ...
      if (expression.includes("globalThis.__quantumBrowserAutomationV1.currentTarget")) {
...
describe("DesktopBrowserAutomationHost", () => {
  ...
  it("executes the complete 22-tool catalogue against one shared visible runtime", async () => {
    ...
    const workspaceRoot = await mkdtemp(join(tmpdir(), "quantum-browser-host-upload-"));
    const uploadUserDataRoot = await mkdtemp(join(tmpdir(), "quantum-browser-host-user-data-"));
