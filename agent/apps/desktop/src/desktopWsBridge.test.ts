...
describe("desktopWsBridge", () => {
  ...
  it("reads only the canonical Quantum desktop URL environment value", () => {
    expect(
      resolveDesktopWsUrlFromEnv({
        QUANTUM_DESKTOP_WS_URL: "ws://127.0.0.1:6000/?token=quantum",
        UNRELATED_DESKTOP_WS_URL: "ws://127.0.0.1:5000/?token=ignored",
      }),
    ).toBe("ws://127.0.0.1:6000/?token=quantum");
    expect(
      resolveDesktopWsUrlFromEnv({
        UNRELATED_DESKTOP_WS_URL: "ws://127.0.0.1:5000/?token=ignored",
      }),
