...
function makeIsolatedServerEnvironment(input: {
  ...
  return {
    ...environment,
    QUANTUM_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "0",
    QUANTUM_CLAUDE_KEEPALIVE: "0",
    QUANTUM_DESKTOP_SHUTDOWN_TOKEN: input.shutdownToken,
    QUANTUM_HOME: input.homeDir,
    QUANTUM_HOST: "127.0.0.1",
    QUANTUM_MODE: "desktop",
    QUANTUM_NO_BROWSER: "1",
    QUANTUM_PORT: String(input.port),
  };
}
...
describeWindows("Windows desktop backend shutdown integration", () => {
  ...
  it("authenticates a real server, deduplicates shutdown, drains its runtime, and clears its finalizer artifact", async () => {
    ...
    try {
      const homeDir = await resources.makeTempDir("quantum-windows-shutdown-");
