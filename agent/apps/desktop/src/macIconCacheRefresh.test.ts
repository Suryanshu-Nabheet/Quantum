...
describe("resolveLaunchVersionRecordPath", () => {
  it("places the record file inside the userData directory", () => {
    expect(resolveLaunchVersionRecordPath("/home/me/AppData/Quantum")).toBe(
      "/home/me/AppData/Quantum/last-launch-version.json",
    );
  });
});
...
describe("resolveMacAppBundlePath", () => {
  it("resolves the .app bundle from the Electron executable on macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/Quantum.app/Contents/MacOS/Quantum", "darwin"),
    ).toBe("/Applications/Quantum.app");
  });

  it("returns null off macOS", () => {
    expect(
      resolveMacAppBundlePath("/Applications/Quantum.app/Contents/MacOS/Quantum", "linux"),
    ).toBeNull();
  });

  it("returns null when the executable is not inside a .app bundle", () => {
    expect(resolveMacAppBundlePath("/usr/local/bin/quantum", "darwin")).toBeNull();
  });
});
