...
import {
  ...
  buildQuantumBranchName,
  buildTemporaryWorktreeBranchName,
  isTemporaryWorktreeBranch,
  resolveUniqueQuantumBranchName,
...
describe("buildQuantumBranchName", () => {
  it("uses quantum as the branch namespace", () => {
    expect(buildQuantumBranchName("fix toast copy")).toBe("quantum/fix-toast-copy");
  });

  it("keeps non-Quantum namespaces inside the Quantum branch", () => {
    expect(buildQuantumBranchName("feature/refine-toolbar-actions")).toBe(
      "quantum/feature/refine-toolbar-actions",
    );
  });

  it("normalizes legacy prefixes before rebuilding the branch", () => {
    for (const namespace of PRE_CUTOVER_NAMESPACE_FIXTURES) {
      expect(buildQuantumBranchName(`${namespace}/refine toolbar actions`)).toBe(
        "quantum/refine-toolbar-actions",
      );
    }
  });

  it("falls back to quantum/update when no preferred name is provided", () => {
    expect(buildQuantumBranchName()).toBe("quantum/update");
  });
});

describe("resolveUniqueQuantumBranchName", () => {
