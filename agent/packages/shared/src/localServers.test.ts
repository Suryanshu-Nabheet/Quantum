...
import type { ServerLocalServerProcess } from "@quantum/contracts";
...
describe("localServerPrimaryLabel", () => {
  it("prefers the live page title when one was resolved", () => {
    expect(localServerPrimaryLabel(makeServer({ pageTitle: "Quantum", displayName: "Vite" }))).toBe(
      "Quantum",
    );
  });
...
describe("localServerFolderLabel", () => {
  it("returns the final segment of a POSIX cwd", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/quantum-website" }))).toBe(
      "quantum-website",
    );
  });

  it("ignores a trailing separator", () => {
    expect(localServerFolderLabel(makeServer({ cwd: "/Users/me/Developer/quantum/" }))).toBe(
      "quantum",
    );
  });
