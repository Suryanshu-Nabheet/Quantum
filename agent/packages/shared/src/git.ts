 ...
 */
export const WORKTREE_BRANCH_PREFIX = "quantum";
...
const QUANTUM_BRANCH_FALLBACK = "update";
...
export function buildQuantumBranchName(preferredBranch?: string | null): string {
  const preferred = preferredBranch?.trim() ?? "";
  ...
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || QUANTUM_BRANCH_FALLBACK,
  )}`;
}

export function resolveUniqueQuantumBranchName(
  existingBranchNames: readonly string[],
  preferredBranch?: string | null,
): string {
  const resolvedBase = buildQuantumBranchName(preferredBranch);
  return resolveUniqueBranchName(existingBranchNames, resolvedBase);
}
