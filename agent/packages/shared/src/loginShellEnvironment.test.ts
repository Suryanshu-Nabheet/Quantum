...
function makeTemporaryRoot(): string {
  const root = FS.mkdtempSync(Path.join(OS.tmpdir(), "quantum-login-shell-"));
...
describe("createCachedLoginShellEnvironmentReader", () => {
  ...
  it("runs uncached when the home directory cannot be resolved", () => {
    ...
    // call-count assertion below) instead of touching the developer's real Quantum home.
    const env = { QUANTUM_HOME: Path.dirname(Path.dirname(fixture.cachePath)) };
...
describe("loginShellEnvironmentCachePath", () => {
  it("anchors the cache in the Quantum home both processes resolve", () => {
    expect(loginShellEnvironmentCachePath({ env: { QUANTUM_HOME: "/tmp/quantum-home" } })).toBe(
      Path.join("/tmp/quantum-home", "cache", "login-shell-environment.json"),
    );
    expect(loginShellEnvironmentCachePath({ env: {}, homeDirectory: "/users/test" })).toBe(
      Path.join("/users/test", ".quantum", "cache", "login-shell-environment.json"),
    );
  });
});
