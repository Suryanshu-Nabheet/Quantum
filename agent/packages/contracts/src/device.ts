...
/**
 * Who owns the boot. Quantum only auto-shuts down devices it booted itself;
 * anything the user started (pane picker, Simulator.app) outlives the session.
 */
export const DeviceBootSource = Schema.Literals(["quantum", "user"]);
...
export const ThreadDeviceState = Schema.Struct({
  ...
  /** Devices the pane may show: booted devices plus anything Quantum is booting. */
...
/** Devices Quantum itself booted are capped globally; viewing already-booted devices is not. */
export const DEVICE_QUANTUM_BOOT_LIMIT = 3;
...
/**
 * Boot is refusable rather than fatal: past DEVICE_QUANTUM_BOOT_LIMIT the caller
...
export const DeviceBootResult = Schema.Union([
  ...
  Schema.Struct({
    ...
    quantumBooted: Schema.Array(DeviceDescriptor).check(Schema.isMaxLength(64)),
  }),
]);
...
export const DeviceInstallAppInput = Schema.Struct({
  udid: DeviceUdid,
  /** Absolute path to a built `.app` bundle; Quantum never runs the build itself. */
 ...
 * the bitstream. Runtime encode/decode lives in `@quantum/shared/deviceFrame`;
