...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  it("allows every generated runtime setting through Turbo", () => {
    ...
    for (const name of [
      ...
      "QUANTUM_AUTH_TOKEN",
      "QUANTUM_PUBLIC_URL",
      "QUANTUM_ALLOW_INSECURE_REMOTE",
      "QUANTUM_HOST",
      "QUANTUM_LOG_WS_EVENTS",
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit QUANTUM_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "QUANTUM_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid QUANTUM_PORT_OFFSET"));
      }),
    );
  });
  ...
  describe("boolean precedence", () => {
    ...
    it.effect("rejects invalid boolean environment values", () =>
      Effect.gen(function* () {
        const error = yield* Effect.flip(
          readDevRunnerBooleanEnvironment({ QUANTUM_LOG_WS_EVENTS: "sometimes" }),
        );
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("marks an inherited terminal PATH as already hydrated", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.QUANTUM_PATH_HYDRATED, "1");
        assert.match(env.PATH ?? "", /\/opt\/homebrew\/bin/);
      }),
    );

    it.effect("defaults QUANTUM_HOME to ~/.quantum when not provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.QUANTUM_HOME, resolve(homedir(), ".quantum"));
        assert.equal(env.QUANTUM_HOST, "127.0.0.1");
    ...
    it.effect("normalizes bracketed IPv6 hosts for listen and client URL syntax", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.QUANTUM_HOST, "::1");
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          quantumHome: "/tmp/custom-quantum",
          authToken: "secret",
          noBrowser: true,
          autoBootstrapProjectFromCwd: false,
          logWebSocketEvents: true,
        ...
        assert.equal(env.QUANTUM_HOME, resolve("/tmp/custom-quantum"));
        assert.equal(env.QUANTUM_PORT, "4222");
        assert.equal(env.QUANTUM_NO_BROWSER, "1");
        assert.equal(env.QUANTUM_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.QUANTUM_LOG_WS_EVENTS, "1");
        assert.equal(env.QUANTUM_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            QUANTUM_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          quantumHome: undefined,
        ...
        assert.equal(env.QUANTUM_MODE, "web");
        assert.equal(env.QUANTUM_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.QUANTUM_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom quantumHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.QUANTUM_HOME, resolve("/tmp/my-quantum"));
        assert.equal(env.QUANTUM_HOME, resolve("/tmp/my-quantum"));
        assert.equal(env.QUANTUM_HOME, resolve("/tmp/my-quantum"));
      }),
    );
