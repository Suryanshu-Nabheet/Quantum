...
export const DEFAULT_QUANTUM_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".quantum"),
);
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@quantum/contracts",
    "--filter=@quantum/web",
    "--filter=@quantum/server",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=@quantum/server"],
  "dev:web": ["run", "dev", "--filter=@quantum/web"],
  "dev:desktop": ["run", "dev", "--filter=@quantum/desktop", "--filter=@quantum/web", "--parallel"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("QUANTUM_PORT_OFFSET"),
  devInstance: optionalStringConfig("QUANTUM_DEV_INSTANCE"),
});
const HomeConfig = optionalStringConfig("QUANTUM_HOME");
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid QUANTUM_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `QUANTUM_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric QUANTUM_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed QUANTUM_DEV_INSTANCE=${seed}` };
}
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_QUANTUM_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly quantumHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(quantumHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      QUANTUM_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://${formattedClientHost}:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      QUANTUM_HOME: resolvedBaseDir,
      QUANTUM_HOST: serverHost,
    };
    ...
    if (authToken !== undefined) {
      output.QUANTUM_AUTH_TOKEN = authToken;
    } else {
      delete output.QUANTUM_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.QUANTUM_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.QUANTUM_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.QUANTUM_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.QUANTUM_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.QUANTUM_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.QUANTUM_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.QUANTUM_MODE = "web";
      delete output.QUANTUM_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read QUANTUM_PORT_OFFSET/QUANTUM_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.QUANTUM_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.QUANTUM_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  quantumHome: Flag.string("home-dir").pipe(
  ...
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to QUANTUM_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("QUANTUM_AUTH_TOKEN")),
  ),
  noBrowser: optionalBooleanFlag("no-browser", {
    description: "Disable browser auto-open (equivalent to QUANTUM_NO_BROWSER).",
  ...
  autoBootstrapProjectFromCwd: optionalBooleanFlag("auto-bootstrap-project-from-cwd", {
    description:
      "Enable project auto-bootstrap (equivalent to QUANTUM_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
  }),
  logWebSocketEvents: optionalBooleanFlag("log-websocket-events", {
    description: "Enable WebSocket event logging (equivalent to QUANTUM_LOG_WS_EVENTS).",
    aliases: ["log-ws-events"],
  }),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to QUANTUM_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("QUANTUM_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to QUANTUM_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("QUANTUM_PORT")),
  ),
