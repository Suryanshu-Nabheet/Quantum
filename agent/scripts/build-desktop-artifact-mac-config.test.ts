...
describe("createDesktopPlatformBuildConfig", () => {
  it("adds explicit microphone entitlements to macOS builds", () => {
    ...
    assert.equal(MAC_APPSNAP_HELPER_BUNDLE_PATH, "Contents/Helpers/quantum-appsnap-helper");
    assert.deepStrictEqual(mac.binaries, ["Contents/Helpers/quantum-appsnap-helper"]);
    assert.equal(mac.x64ArchFiles, "Contents/Helpers/quantum-appsnap-helper");
    assert.equal(
      MAC_APPSNAP_HELPER_STAGE_PATH,
      "apps/desktop/native/appsnap/build/quantum-appsnap-helper",
    );
    ...
    assert.deepStrictEqual(config.extraFiles, [
      {
        from: "apps/desktop/native/appsnap/build/quantum-appsnap-helper",
        to: "Helpers/quantum-appsnap-helper",
      },
      {
  ...
  it("leaves non-macOS platform configs unchanged", () => {
    ...
    const win = createDesktopPlatformBuildConfig({
      platform: "win",
      target: "nsis",
      windowsAzureSignOptions: { publisherName: "Quantum" },
    });
    ...
    assert.deepStrictEqual(linux.linux, {
      target: ["AppImage"],
      executableName: "quantum",
      icon: "icon.png",
      category: "Development",
      desktop: {
        entry: {
          StartupWMClass: "quantum",
        },
      },
    });
    ...
    assert.deepStrictEqual(win.win, {
      target: ["nsis"],
      icon: "icon.ico",
      publisherName: "Quantum",
      azureSignOptions: { publisherName: "Quantum" },
    });
  });
