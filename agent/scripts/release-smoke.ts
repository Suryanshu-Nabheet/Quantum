...
files:
  - url: Quantum-9.9.9-smoke.0-arm64.zip
    sha512: arm64zip
    size: 125621344
path: Quantum-9.9.9-smoke.0-arm64.zip
sha512: arm64zip
...
`,
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: Quantum-9.9.9-smoke.0-x64.zip
    sha512: x64zip
    size: 132000112
path: Quantum-9.9.9-smoke.0-x64.zip
sha512: x64zip
...
function verifyCanonicalIdentity(): void {
  ...
  const expectedBinaries = {
    ...
    "quantum-restore-migration-backup": "dist/restoreMigrationBackup.mjs",
  };
  if (JSON.stringify(serverPackage.bin ?? {}) !== JSON.stringify(expectedBinaries)) {
    throw new Error(
      "Expected the CLI to expose only the Quantum entry point and migration recovery binary.",
    );
  }
  if (QUANTUM_PRODUCTION_BUNDLE_ID !== "com.suryanshunabheet.quantum") {
    throw new Error(`Unexpected production bundle ID: ${QUANTUM_PRODUCTION_BUNDLE_ID}.`);
  }
  if (QUANTUM_DESKTOP_UPDATE_CHANNEL !== "quantum") {
    throw new Error(`Unexpected desktop update channel: ${QUANTUM_DESKTOP_UPDATE_CHANNEL}.`);
  }
  ...
  if (
    ...
    throw new Error("Expected stable clean Quantum releases to publish on GitHub Latest.");
  }
}
...
function verifyReleaseWorkflowSafety(): void {
  ...
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.QUANTUM_PUBLISH_CLI == '1'",
    "Expected CLI publication to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "needs.preflight.outputs.publish_release == 'true' && vars.QUANTUM_FINALIZE_RELEASE == '1'",
    "Expected release finalization to require explicit publication mode.",
  );
  assertContains(
    workflow,
    "QUANTUM_PUBLISH_RELEASE: ${{ needs.preflight.outputs.publish_release }}",
  ...
  assertContains(
    desktopBuildConfig,
    "__QUANTUM_WINDOWS_UPDATER_PUBLISHER__",
...
const tempRoot = mkdtempSync(join(tmpdir(), "quantum-release-smoke-"));
...
try {
  ...
  assertContains(
    mergedManifest,
    "Quantum-9.9.9-smoke.0-arm64.zip",
    "Merged manifest is missing the arm64 asset.",
  );
  assertContains(
    mergedManifest,
    "Quantum-9.9.9-smoke.0-x64.zip",
