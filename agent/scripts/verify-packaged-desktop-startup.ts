...
function prepareWindowsLaunch(assetsDirectory: string, extractionRoot: string): LaunchCommand {
  ...
  const executables = findFiles(applicationRoot, (candidate) =>
    /[/\\]Quantum\.exe$/i.test(candidate),
  );
  if (executables.length !== 1) {
    throw new Error(`Expected one extracted Quantum.exe, found ${executables.length}.`);
  }
...
export function createPackagedDesktopSmokeEnvironment(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    QUANTUM_HOME: join(root, "quantum-home"),
  ...
  if (options.platform === "mac") {
    const userDataPath = join(env.HOME!, "Library", "Application Support", "quantum");
...
export async function verifyPackagedDesktopStartup(
  ...
  const temporaryRoot = mkdtempSync(join(tmpdir(), `quantum-packaged-smoke-${options.platform}-`));
  ...
  try {
    ...
    const logPath = join(env.QUANTUM_HOME!, "userdata", "logs", "desktop-main.log");
