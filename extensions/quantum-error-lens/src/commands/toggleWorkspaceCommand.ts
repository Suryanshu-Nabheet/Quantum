...
export async function toggleWorkspaceCommand(): Promise<void> {
	...
	if ($config.excludeWorkspaces?.includes(workspaceFsPath)) {
		...
		showResultNotification(`"${workspaceFsPath}" - Removed from "quantumErrorLens.excludeWorkspaces"`);
	} else {
		newExcludeWorkspaceList = [...($config.excludeWorkspaces || []), workspaceFsPath];
		showResultNotification(`"${workspaceFsPath}" - Added to "quantumErrorLens.excludeWorkspaces" (now ignored)`);
	}

	vscodeUtils.updateGlobalSetting('quantumErrorLens.excludeWorkspaces', newExcludeWorkspaceList);
}
