...
APP_NAME="${APP_NAME:-Quantum}"
APP_NAME_LC="$( echo "${APP_NAME}" | awk '{print tolower($0)}' )"
ASSETS_REPOSITORY="${ASSETS_REPOSITORY:-Suryanshu-Nabheet/Quantum}"
BINARY_NAME="${BINARY_NAME:-quantum}"
DEVELOPER_NAME="${DEVELOPER_NAME:-Suryanshu Nabheet}"
GH_REPO_PATH="${GH_REPO_PATH:-Suryanshu-Nabheet/Quantum}"
ORG_NAME="${ORG_NAME:-Suryanshu-Nabheet}"
PRODUCT_TAGLINE="${PRODUCT_TAGLINE:-The open source AI code editor}"
# Quantum's own product version — independent of the upstream VS Code version
# (which flows through RELEASE_VERSION into package.json "version").
QUANTUM_VERSION="${QUANTUM_VERSION:-0.0.1}"
