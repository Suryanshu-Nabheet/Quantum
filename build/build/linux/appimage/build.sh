...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i 's/generate_type2_appimage/generate_type2_appimage -u "gh-releases-zsync|Quantum|quantum-insiders|latest|*.AppImage.zsync"/' pkg2appimage.AppDir/AppRun
  else
    sed -i 's/generate_type2_appimage/generate_type2_appimage -u "gh-releases-zsync|Suryanshu-Nabheet|quantum|latest|*.AppImage.zsync"/' pkg2appimage.AppDir/AppRun
  fi
  ...
  # workaround that enforces x86 ARCH for pkg2appimage having /__w/quantum/quantum/build/linux/appimage/Quantum/Quantum.AppDir/usr/share/quantum/resources/app/node_modules/rc/index.js is of architecture armhf
  export ARCH=x86_64
